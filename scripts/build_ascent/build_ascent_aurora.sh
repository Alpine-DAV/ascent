...
env CC=`which mpicc` CXX=`which mpicxx` FTN=`which mpifort` enable_sycl=ON enable_mpi=ON enable_find_mpi=OFF enable_fortran=ON enable_python=ON raja_enable_vectorization=ON enable_tests=OFF enable_verbose=OFF ./build_ascent_sycl.sh
