...
class Ascent(Package, CudaPackage):
    ...
    depends_on("vtk-h@0.5.9",             when="+vtkh")
    depends_on("vtk-h@0.5.9~openmp",      when="+vtkh~openmp")
    depends_on("vtk-h@0.5.9+cuda+openmp", when="+vtkh+cuda+openmp")
    depends_on("vtk-h@0.5.9+cuda~openmp", when="+vtkh+cuda~openmp")

    depends_on("vtk-h@0.5.9~shared",             when="~shared+vtkh")
    depends_on("vtk-h@0.5.9~shared~openmp",      when="~shared+vtkh~openmp")
    depends_on("vtk-h@0.5.9~shared+cuda",        when="~shared+vtkh+cuda")
    depends_on("vtk-h@0.5.9~shared+cuda~openmp", when="~shared+vtkh+cuda~openmp")
    ...
    depends_on("dray@0.1.2+mpi~test~utils+shared+cuda",        when="+dray+mpi+cuda+shared")
    depends_on("dray@0.1.2+mpi~test~utils+shared+openmp",      when="+dray+mpi+openmp+shared")
    depends_on("dray@0.1.2+mpi~test~utils+shared~openmp~cuda", when="+dray+mpi~openmp~cuda+shared")

    depends_on("dray@0.1.2+mpi~test~utils~shared+cuda",        when="+dray+mpi+cuda~shared")
    depends_on("dray@0.1.2+mpi~test~utils~shared+openmp",      when="+dray+mpi+openmp~shared")
    depends_on("dray@0.1.2+mpi~test~utils~shared~openmp~cuda", when="+dray+mpi~openmp~cuda~shared")

    # devil ray variants 2ithout mpi
    depends_on("dray@0.1.2~mpi~test~utils+shared+cuda",        when="+dray~mpi+cuda+shared")
    depends_on("dray@0.1.2~mpi~test~utils+shared+openmp",      when="+dray~mpi+openmp+shared")
    depends_on("dray@0.1.2~mpi~test~utils+shared~openmp~cuda", when="+dray~mpi~openmp~cuda+shared")
