...
export SPACK_SPEC="%gcc+mpi+cuda+vtkh+dray+mfem~occa~python~openmp~shared cuda_arch=80"
# constraints
# note: silo static build with spack will fail unless you disable python
export SPACK_SPEC="${SPACK_SPEC} ^raja+cuda~openmp cuda_arch=80 ^umpire+cuda~openmp cuda_arch=80 ^camp+cuda~openmp cuda_arch=80 ^hdf5~mpi ^mfem~cuda~petsc~sundials~slepc ^hypre~cuda ^viskores+cuda~kokkos cuda_arch=80 ^silo~python~mpi"
