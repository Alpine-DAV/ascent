...
# constraints
export SPACK_SPEC="${SPACK_SPEC} ^raja@0.14.0+cuda cuda_arch=70  ^dray~openmp+cuda~test~utils cuda_arch=70 ^hdf5~mpi ^mfem~cuda~petsc~sundials~slepc ^hypre~cuda ^vtk-h+cuda cuda_arch=70 ^vtk-m+cuda~kokkos@1.8.0 cuda_arch=70"
# config
