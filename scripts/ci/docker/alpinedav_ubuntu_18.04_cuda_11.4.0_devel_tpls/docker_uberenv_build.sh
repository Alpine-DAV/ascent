...
# constraints
export SPACK_SPEC="${SPACK_SPEC} ^hdf5~mpi ^dray@test~openmp+cuda ^mfem~cuda cuda_arch=none ^hypre~cuda cuda_arch=none ^conduit@develop ^vtk-h@develop+cuda ^vtk-m+cuda ^cmake~openssl~ncurses"
# config
