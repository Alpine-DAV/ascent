...
class Viskores(CMakePackage, CudaPackage, ROCmPackage):
    ...
    version(
        "1.1.1",
        tag="v1.1.1",
        commit="1d38698019f56ef2636d7283f4cac9bc2ee6c819",
        preferred=True,
    )
    version(
        "1.0.0",
    ...
    variant("logging", default=False, when="@1.0.0:", description="build logging support")
    variant("mpi", default=False, when="@1.0.0:", description="build mpi support")
    ...
    variant("testlib", default=False, when="@1.0.0:", description="build test library")
    variant("fpic", default=False, description="build fpic support")
    variant("examples", default=True, when="@1.0.0:", description="Install builtin examples")

    # Device variants
    # CudaPackage provides cuda variant
    # ROCmPackage provides rocm variant
    variant("kokkos", default=False, when="@1.0.0:", description="build using Kokkos backend")
    variant(
    ...
    depends_on("rocthrust", when="@1.0.0: +kokkos+rocm ^cmake@3.24:")
    ...
    def cmake_args(self):
        ...
        with working_dir("spack-build", create=True):
            ...
            if "+cuda_native" in spec:
                ...
                if spec.satisfies("@1.0.0:") and spec.satisfies("^cmake@3.18:"):
                    ...
                    try:
                        ...
                        raise InstallError(
                            f"cuda_arch={num_cuda_arch} needs cmake>=3.18 & Viskores>=1.9.0"
                        )
    ...
    def test_smoke_test(self):
        ...
        with working_dir(testdir, create=True):
            ...
            cmake(self.prefix.share.doc.Viskores.examples.smoke_test, f"-DViskores_ROOT={self.prefix}")
            cmake("--build", ".")
            ctest("--verbose")
