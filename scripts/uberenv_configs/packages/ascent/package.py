...
class Ascent(CMakePackage, CudaPackage):
    ...
    with when("+raja"):
        depends_on("raja")
        depends_on("raja@2024.02.1:2026.12.99", when="@0.9.3:")
    ...
    with when("+umpire"):
        ...
        depends_on("umpire@2024.02.1:2026.12.99", when="@0.9.3:")
    ...
    #######################
    with when("+vtkh"):
        depends_on("viskores +doubleprecision ~64bitids")
        depends_on("viskores@2.1:", when="@0.9.3:")
        depends_on("viskores@2.0:", when="@0.9.2:")
        # 2.1 support needs commit e52b7bb8c9fd131f2fd49edf58037cc5ef77a166
        depends_on("viskores@:2.0", when="@:0.9.2")
        depends_on("viskores@1.9", when="@0.9.0:0.9.1")

        depends_on("viskores~tbb", when="@0.9.0:")
        depends_on("viskores+openmp", when="@0.9.0: +openmp")
        depends_on("viskores~openmp", when="@0.9.0: ~openmp")
        depends_on("viskores~cuda", when="@0.9.0: ~cuda")
        depends_on("viskores+cuda", when="@0.9.0: +cuda")
        depends_on("viskores+fpic", when="@0.8.0:")
        depends_on("viskores~shared+fpic", when="@0.8.0: ~shared")
        # Ascent defaults to C++11
        depends_on("kokkos cxxstd=11", when="+vtkh ^viskores +kokkos")
        depends_on("kokkos@3.7.02", when="@0.9.3: +vtkh ^viskores +kokkos")
        ...
        depends_on("vtk-h~shared", when="@:0.8.0 ~shared")
        # When using VTK-h ascent also needs Viskores
        depends_on("viskores@:1.7", when="@:0.8.0")
        depends_on("viskores+testlib", when="@:0.8.0 +test")
