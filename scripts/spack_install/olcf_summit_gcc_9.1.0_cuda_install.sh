...
mkdir -p $DEST_DIR
python scripts/uberenv/uberenv.py --spec="%gcc ^conduit@develop ^vtk-h@develop" \
       --pull \
       --install \
...
rm -f public_env.sh
python scripts/spack_install/gen_public_install_env_script.py ${DEST_DIR} gcc/9.1.0 cuda
