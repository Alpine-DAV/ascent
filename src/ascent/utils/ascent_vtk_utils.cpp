...
uint32_t VTKutils::DATA_DECOMP[3] = {0,0,0};


vtkSmartPointer<vtkImageData> VTKutils::getImageData(char* data, uint32_t* dims)
{
...
vtkSmartPointer<vtkMarchingCubes> VTKutils::getSurface(char* data, double isoValue, uint32_t* dims)
{
...
//     vtkSmartPointer<vtkVolumeTextureMapper3D>::New();
  ...
  compositeOpacity->AddPoint(0.1,1.0);//0.7);
  compositeOpacity->AddPoint(80.1,1.0);//0.9);
...
//  camera->SetClippingRange(1300,1995.182540);
  ...
  std::cout << "Setting offscreen rendering..." << std::endl;
...
int VTKutils::arraytoImage(unsigned char* image, uint32_t* bound, char* filename)
{
  ...
  for(int y=0; y < VTKutils::IMAGE_Y_DIM-1; y++){
      for(int x=0; x < VTKutils::IMAGE_X_DIM-1; x++){
        int idx = x + y*VTKutils::IMAGE_X_DIM; 
        ...
        if(x >= bound[0] && x < bound[1] && y >= bound[2] && y < bound[3]){
          ...
          int myidx = (x-bound[0]) + (y-bound[2])*x_size;
          int myimgidx = myidx*3;
...
int VTKutils::arraytoImageFixedSize(unsigned char* image, uint32_t* bound, char* filename)
{
  vtkImageData *blankImage = vtkImageData::New();
...
//#ifdef BOUNDING_BOX_OPTIMIZATION
int VTKutils::composite(const std::vector<ImageData>& image_data, std::vector<VTKutils::ImageData>& out_image, 
                        int id, int x_factor, int y_factor, int n_out)
{
    ...
    for(int j=0; j < image_data.size(); j++){
      ...
      for(uint32_t y=bound[2]; y < bound[3]+1; y++){
        for(uint32_t x=bound[0]; x < bound[1]+1; x++){
          ...
          // uint32_t myrx = x-out_image[i].rend_bounds[0];
          // uint32_t myry = y-out_image[i].rend_bounds[2];
          ...
          if(out_image[i].zbuf[myidx] < image_data[j].zbuf[idx]){
             out_image[i].zbuf[myidx] = ( unsigned char ) -255+(image_data[j].zbuf[idx] *255);
             image_data[j].image[imgidx] = image_data[j].image[imgidx];
              out_image[i].image[imgmyidx] = out_image[i].image[imgmyidx];
...
#ifdef BOUNDING_BOX_OPTIMIZATION
int VTKutils::composite(const std::vector<SimpleImageData>& image_data, SimpleImageData& out_image, const int id)
{
...
int VTKCompositeRender::volumeRender(uint32_t* box, char* data, float isovalue, 
      VTKutils::SimpleImageData& image_data, int id)
{
  ...
  vtkSmartPointer<vtkImageData> surface = VTKutils::getImageData(data,dims);
 ...
 //ren->GetZbufferData(0,0,VTKutils::IMAGE_X_DIM-1,VTKutils::IMAGE_Y_DIM-1);
  ...
  memcpy(image_data.bounds, in_bounds, 4*sizeof(uint32_t));
...
int VTKCompositeRender::isosurfaceRender(uint32_t* box, char* data, float isovalue, 
      VTKutils::SimpleImageData& image_data, int id)
{
#if DISABLE_RENDERING
  uint32_t this_size = VTKutils::IMAGE_X_DIM*VTKutils::IMAGE_Y_DIM;
  ...
  vtkSmartPointer<vtkMarchingCubes> surface = VTKutils::getSurface(data, isovalue, dims);
...
int VTKCompositeRender::isosurfaceRender(uint32_t* box, char* data, float isovalue, 
      std::vector<VTKutils::ImageData>& images, int id, int x_factor, int y_factor)
{
#if DISABLE_RENDERING
  for(int i=0; i<2; i++){ 
...
int VTKCompositeRender::writeImage(unsigned char* image, uint32_t* bound, const char* filename)
{
    vtkImageData *blankImage = vtkImageData::New();
    blankImage->SetExtent(0,VTKutils::IMAGE_X_DIM-1,0,VTKutils::IMAGE_Y_DIM-1,0,1);
    blankImage->AllocateScalars(VTK_UNSIGNED_CHAR, 3);
