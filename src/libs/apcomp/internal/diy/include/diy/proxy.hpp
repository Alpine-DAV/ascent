...
namespace apcompdiy
{
  ...
  struct Master::Proxy
  {
    ...
    void                enqueue(const BlockID&  to,                                     //!< target block (gid,proc)
                                ...
                                void (*save)(BinaryBuffer&, const T&) = &::apcompdiy::save<T> //!< optional serialization function
                               ) const
    {
    ...
    //! previously enqueued so that apcompdiy knows its size when it is received.
    //! In this case, apcompdiy will allocate the receive buffer; the user does not need to do so.
    template<class T>
    void                dequeue(int             from,                                   //!< target block gid
                                T&              x,                                      //!< data (eg. STL vector)
                                void (*load)(BinaryBuffer&, T&) = &::apcompdiy::load<T>       //!< optional serialization function
                               ) const
    ...
    EnqueueIterator<T>  enqueuer(const T& x,
                                 void (*save)(BinaryBuffer&, const T&) = &::apcompdiy::save<T>) const
        ...
        void (*save)(BinaryBuffer&, const T&)) const
{
    ...
    if (save == (void (*)(BinaryBuffer&, const T&)) &::apcompdiy::save<T>)
        apcompdiy::save(bb, x, n);       // optimized for unspecialized types
    else
        ...
        void (*load)(BinaryBuffer&, T&)) const
{
    ...
    if (load == (void (*)(BinaryBuffer&, T&)) &::apcompdiy::load<T>)
        apcompdiy::load(bb, x, n);       // optimized for unspecialized types
    else
        for (size_t i = 0; i < n; ++i)
            load(bb, x[i]);
}
