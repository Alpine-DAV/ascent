  ...
  class NumPy: public BOV
  {
    public:
      ...
      unsigned          read_header()
      {
        ...
        if (fortran)
            throw std::runtime_error("apcompdiy::io::NumPy cannot read data in fortran order");
    ...
    private:
      ...
      void              save(apcompdiy::BinaryBuffer& bb, const std::string& s)               { bb.save_binary(s.c_str(), s.size()); }
      template<class T>
      inline void       convert_and_save(apcompdiy::BinaryBuffer& bb, const T& x)
      {
...
write_header(const S& shape)
{
    ...
    apcompdiy::MemoryBuffer dict;
    save(dict, "{'descr': '");
    apcompdiy::save(dict, detail::big_endian());
    apcompdiy::save(dict, detail::map_numpy_type<T>());
    ...
    apcompdiy::MemoryBuffer header;
    apcompdiy::save(header, (char) 0x93);
    save(header, "NUMPY");
    apcompdiy::save(header, (char) 0x01);  // major version of numpy format
    apcompdiy::save(header, (char) 0x00);  // minor version of numpy format
    apcompdiy::save(header, (unsigned short) dict.position);
...
char
apcompdiy::io::detail::big_endian()
{
  unsigned char x[] = {1,0};
  void* x_void = x;
