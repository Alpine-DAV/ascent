...
namespace apcompdiy
{
  ...
  struct Serialization< MemoryBuffer >
  {
    ...
    {
      apcompdiy::save(bb, x.position);
      if (x.position > 0)
          apcompdiy::save(bb, &x.buffer[0], x.position);
    }

    static void         load(BinaryBuffer& bb, MemoryBuffer& x)
    {
      apcompdiy::load(bb, x.position);
      x.buffer.resize(x.position);
      if (x.position > 0)
          apcompdiy::load(bb, &x.buffer[0], x.position);
    }
  ...
  struct Serialization< std::string >
  {
    ...
    {
      size_t sz = s.size();
      apcompdiy::save(bb, sz);
      apcompdiy::save(bb, s.c_str(), sz);
    }

    static void         load(BinaryBuffer& bb, String& s)
    {
      size_t sz;
      apcompdiy::load(bb, sz);
      s.resize(sz);
  ...
  struct Serialization< std::pair<X,Y> >
  {
    ...
    {
      apcompdiy::save(bb, p.first);
      apcompdiy::save(bb, p.second);
    }

    static void         load(BinaryBuffer& bb, Pair& p)
    {
      apcompdiy::load(bb, p.first);
      apcompdiy::load(bb, p.second);
    }
  };
  ...
  struct Serialization< std::tuple<Args...> >
  {
    ...
    static
    typename std::enable_if<I < sizeof...(Args), void>::type
                        save(BinaryBuffer& bb, const Tuple& t)          { apcompdiy::save(bb, std::get<I>(t)); save<I+1>(bb, t); }
    ...
    static
    typename std::enable_if<I < sizeof...(Args), void>::type
                        load(BinaryBuffer& bb, Tuple& t)                { apcompdiy::load(bb, std::get<I>(t)); load<I+1>(bb, t); }

  };
}

void
apcompdiy::MemoryBuffer::
