namespace apcompdiy
{
    ...
    namespace detail
    {
        template<typename T>  struct is_mpi_datatype< apcompdiy::detail::VectorWindow<T> > { typedef true_type type; };

        template <typename T>
        struct mpi_datatype< apcompdiy::detail::VectorWindow<T> >
        {
            using VecWin = apcompdiy::detail::VectorWindow<T>;
...
} // namespace apcompapcompdiy


/** InFlightRecv **/

apcompdiy::Master::InFlightRecv&
apcompdiy::Master::
...
apcompdiy::Master::InFlightSendsList&
apcompdiy::Master::inflight_sends()
{
...
recv(mpi::communicator& comm, const mpi::status& status)
{
    ...
    if (info.from == -1)            // uninitialized
    {
        ...
        apcompdiy::load_back(bb, info);
        info.nparts--;
        if (info.nparts > 0)        // multi-part message
        {
            size_t msg_size;
            apcompdiy::load(bb, msg_size);
