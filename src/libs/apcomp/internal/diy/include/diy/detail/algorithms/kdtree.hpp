...
{
    typedef     apcompdiy::RegularContinuousLink      RCLink;
    typedef     apcompdiy::ContinuousBounds           Bounds;
    ...
    void        operator()(Block* b, const apcompdiy::ReduceProxy& srp, const KDTreePartners& partners) const;

    int         divide_gid(int gid, bool lower, int round, int rounds) const;
    void        update_links(Block* b, const apcompdiy::ReduceProxy& srp, int dim, int round, int rounds, bool wrap, const Bounds& domain) const;
    void        split_to_neighbors(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const;
    apcompdiy::Direction
                find_wrap(const Bounds& bounds, const Bounds& nbr_bounds, const Bounds& domain) const;

    void        compute_local_histogram(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const;
    void        add_histogram(Block* b, const apcompdiy::ReduceProxy& srp, Histogram& histogram) const;
    void        receive_histogram(Block* b, const apcompdiy::ReduceProxy& srp,       Histogram& histogram) const;
    void        forward_histogram(Block* b, const apcompdiy::ReduceProxy& srp, const Histogram& histogram) const;

    void        enqueue_exchange(Block* b, const apcompdiy::ReduceProxy& srp, int dim, const Histogram& histogram) const;
    void        dequeue_exchange(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const;
...
struct apcompdiy::detail::KDTreePartners
{
  ...
  inline bool   active(int round, int gid, const apcompdiy::Master& m) const
  {
    if (round == (int) rounds())
        return true;
  ...
  inline void   incoming(int round, int gid, std::vector<int>& partners, const apcompdiy::Master& m) const
  {
    if (round == (int) rounds())
  ...
  inline void   outgoing(int round, int gid, std::vector<int>& partners, const apcompdiy::Master& m) const
  {
    if (round == (int) rounds())
  ...
  inline void   link_neighbors(int, int gid, std::vector<int>& partners, const apcompdiy::Master& m) const
  {
    int         lid  = m.lid(gid);
    apcompdiy::Link*  link = m.link(lid);
  ...
  apcompdiy::RegularDecomposer<apcompdiy::DiscreteBounds>   decomposer;

  apcompdiy::RegularAllReducePartners     histogram;
  apcompdiy::RegularSwapPartners          swap;
...
template<class Block, class Point>
void
apcompdiy::detail::KDTreePartition<Block,Point>::
operator()(Block* b, const apcompdiy::ReduceProxy& srp, const KDTreePartners& partners) const
{
    int dim;
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
update_links(Block* b, const apcompdiy::ReduceProxy& srp, int dim, int round, int rounds, bool wrap, const Bounds& domain) const
{
    int         gid  = srp.gid();
    ...
    // (gid, dir) -> i
    std::map<std::pair<int,apcompdiy::Direction>, int> link_map;
    for (int i = 0; i < link->size(); ++i)
    ...
    for (int i = 0; i < link->size(); ++i)
    {
        float split; apcompdiy::Direction dir(dim_,0);
    ...
    for (int i = 0; i < link->size(); ++i)
    {
        apcompdiy::Direction  dir      = link->direction(i);
        //apcompdiy::Direction  wrap_dir = link->wrap(i);     // we don't use existing wrap, but restore it from scratch
        if (dir[dim] != 0)
        {
            if ((dir[dim] < 0 && lower) || (dir[dim] > 0 && !lower))
            {
                int nbr_gid = divide_gid(link->target(i).gid, !lower, round, rounds);
                apcompdiy::BlockID nbr = { nbr_gid, srp.assigner().rank(nbr_gid) };
                ...
                if (wrap)
                    ...
                    new_link.add_wrap(apcompdiy::Direction(dim_,0));
            }
    ...
    apcompdiy::BlockID dual = { dual_gid, srp.assigner().rank(dual_gid) };
    ...
    new_link.add_wrap(apcompdiy::Direction(dim_,0));    // dual block cannot be wrapped

    if (lower)
    {
        apcompdiy::Direction right(dim_,0);
        right[dim] = 1;
        new_link.add_direction(right);
    } else
    {
        apcompdiy::Direction left(dim_,0);
        left[dim] = -1;
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
split_to_neighbors(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const
{
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
compute_local_histogram(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const
{
    int         lid  = srp.master()->lid(srp.gid());
    RCLink*     link = static_cast<RCLink*>(srp.master()->link(lid));
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
add_histogram(Block* b, const apcompdiy::ReduceProxy& srp, Histogram& histogram) const
{
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
receive_histogram(Block* b, const apcompdiy::ReduceProxy& srp, Histogram& histogram) const
{
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
forward_histogram(Block* b, const apcompdiy::ReduceProxy& srp, const Histogram& histogram) const
{
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
enqueue_exchange(Block* b, const apcompdiy::ReduceProxy& srp, int dim, const Histogram& histogram) const
{
    auto        log = get_logger();
...
void
apcompdiy::detail::KDTreePartition<Block,Point>::
dequeue_exchange(Block* b, const apcompdiy::ReduceProxy& srp, int dim) const
{
    int         lid  = srp.master()->lid(srp.gid());
    RCLink*     link = static_cast<RCLink*>(srp.master()->link(lid));
...
find_wrap(const Bounds& bounds, const Bounds& nbr_bounds, const Bounds& domain) const
{
    apcompdiy::Direction wrap(dim_,0);
    for (int i = 0; i < dim_; ++i)
    {
