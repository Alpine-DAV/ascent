...
//-----------------------------------------------------------------------------
bool check_for_file(const std::string &file_name, int mpi_comm_id)
{
    int rank = 0;
#ifdef ASCENT_MPI_ENABLED
    if (mpi_comm_id == -1)
...
//-----------------------------------------------------------------------------
void load_included_files_in_node_tree(conduit::Node &node, int mpi_comm_id)
{
    // This function recursively traverses the node tree searching for include
    // statements If one is found, the node attempts to load the file's
    // contents and add it to the node tree
    int comm_size = 1;
    int rank = 0;
...
//-----------------------------------------------------------------------------
void quiet_handler(const std::string &, const std::string &, int) {}

} // namespace detail

//-----------------------------------------------------------------------------
Ascent::Ascent()
    : m_runtime(NULL), m_verbose_msgs(true), m_forward_exceptions(false),
...
//-----------------------------------------------------------------------------
void CheckForSettingsFile(std::string file_name,
                          conduit::Node &node,
                          bool merge,
                          int mpi_comm_id)
{
...
//-----------------------------------------------------------------------------
void Ascent::open(const conduit::Node &options)
{
    try
    {
        if (m_runtime != NULL)
        ...
        CheckForSettingsFile(opts_file, processed_opts, true, comm_id);
        ...
        if (m_options.has_path("messages"))
        {
            ...
            else if (m_options["messages"].dtype().is_object())
            {
                ...
                {
                    std::string echo_thresh =
                        msg_ops["echo_threshold"].as_string();
                    if (echo_thresh == "none" || echo_thresh == "error" ||
                        echo_thresh == "warn")
                    {
                        m_verbose_msgs = false;
                    }
        ...
        // if ranks == "root"
        //   rank 0 is what is specified, echo_threshold = "none" for all
        //   others
        // if ranks == "all"
        //   echo_threshold = specified option used for all ranks (alreay
        //   handled above)
        ...
        {
            logger.set_log_threshold(
                logging_opts["log_threshold"].as_string());
            std::string file_pattern =
                logging_opts["file_pattern"].as_string();
#if defined(ASCENT_MPI_ENABLED)
            ASCENT_LOG_OPEN_RANK(file_pattern, par_rank) // mpi par
            ASCENT_LOG_DEBUG(conduit_fmt::format(
                "mpi info: rank={}, size={}", par_rank, par_size));
#else
...
#if defined(ASCENT_VTKH_ENABLED)
                std::string backend =
                    m_options["runtime/vtkm/backend"].as_string();
                if (backend == "serial")
                {
                    vtkh::ForceSerial();
                }
    ...
    {
        set_status("Ascent::open failed", e.message());
        ...
        {
            std::cout << "[Error] Ascent::open " << e.message() << std::endl;
        }
    }
}

//-----------------------------------------------------------------------------
void Ascent::publish(const conduit::Node &data)
{
    try
    {
        if (m_runtime != NULL)
    ...
    {
        set_status("Ascent::publish failed", e.message());
        ...
        else
        {
            if (m_runtime != NULL)
            {
                ...
                msg << "[Error] Ascent::publish " << e.message() << std::endl;
                m_runtime->DisplayError(msg.str());
            }
            else
            {
                std::cerr << "[Error] Ascent::publish " << e.message()
                          << std::endl;
            }
        }
    }
...
//-----------------------------------------------------------------------------
void Ascent::execute(const conduit::Node &actions)
{
    try
    {
        if (m_runtime != NULL)
        {
            ...
            detail::load_included_files_in_node_tree(processed_actions,
                                                     mpi_comm_id);
            ...
            CheckForSettingsFile(
                m_actions_file, processed_actions, false, mpi_comm_id);
    ...
    {
        set_status("Ascent::execute failed", e.message());
        ...
        else
        {
            ...
            {
                std::stringstream msg;
                msg << "[Error] Ascent::execute " << e.message() << std::endl;
                m_runtime->DisplayError(msg.str());
            }
            else
            {
                std::cerr << "[Error] Ascent::execute " << e.message()
                          << std::endl;
            }
        }
    }
...
//-----------------------------------------------------------------------------
void Ascent::info(conduit::Node &info_out)
{
    try
    {
        if (m_runtime != NULL)
    ...
    {
        set_status("Ascent::info failed", e.message());
        ...
        else
        {
            ...
            {
                std::stringstream msg;
                msg << "[Error] Ascent::info" << e.message() << std::endl;
                m_runtime->DisplayError(msg.str());
            }
            else
            {
                std::cerr << "[Error] Ascent::info" << e.message()
                          << std::endl;
            }
        }
    }
...
void Ascent::close()
{
    ...
    {
        set_status("Ascent::close failed", e.message());
        ...
        else
        {
            ...
            {
                std::stringstream msg;
                msg << "[Error] Ascent::close" << e.message() << std::endl;
                m_runtime->DisplayError(msg.str());
            }
            else
            {
                std::cerr << "[Error] Ascent::close " << e.message()
                          << std::endl;
            }
        }
        ASCENT_LOG_CLOSE();
...
//---------------------------------------------------------------------------//
void Ascent::set_status(const std::string &msg)
{
    m_status.reset();
...
//---------------------------------------------------------------------------//
void Ascent::set_status(const std::string &msg, const std::string &details)
{
    m_status.reset();
    std::ostringstream oss;
    oss << msg << " at " << timestamp();
...
void about(conduit::Node &n)
{
    ...
    {
        n["version"] =
            n["version"].as_string() + "-" + n["git_sha1_abbrev"].as_string();
    }
...
// Callback maps
static std::map<std::string, void (*)(conduit::Node &, conduit::Node &)>
    m_void_callback_map;
                                                 ...
                                                 conduit::Node &))
{
    ...
    {
        ASCENT_ERROR(
            "cannot register more than one void callback under the name '"
            << callback_name << "'");
    }
    else if (m_bool_callback_map.count(callback_name) != 0)
    {
        ASCENT_ERROR("cannot register both a void and bool callback under the "
                     "same name '"
                     << callback_name << "'");
    }
    m_void_callback_map.insert(
        std::make_pair(callback_name, callback_function));
}
                       ...
                       bool (*callback_function)(void))
{
    ...
    {
        ASCENT_ERROR(
            "cannot register more than one bool callback under the name '"
            << callback_name << "'");
    }
    else if (m_void_callback_map.count(callback_name) != 0)
    {
    ...
    m_bool_callback_map.insert(
        std::make_pair(callback_name, callback_function));
}

//-----------------------------------------------------------------------------
void execute_callback(const std::string &callback_name,
                      conduit::Node &params,
                      conduit::Node &output)
{
    if (m_void_callback_map.count(callback_name) != 1)
    {
        ASCENT_ERROR("requested void callback '" << callback_name
...
//-----------------------------------------------------------------------------
bool execute_callback(const std::string &callback_name)
{
    if (m_bool_callback_map.count(callback_name) != 1)
    {
        ASCENT_ERROR("requested bool callback '" << callback_name
                                                 << "' was never registered");
    }
...
//-----------------------------------------------------------------------------
void get_void_callbacks(std::vector<std::string> &callback_names)
{
...
//-----------------------------------------------------------------------------
void get_bool_callbacks(std::vector<std::string> &callback_names)
{
