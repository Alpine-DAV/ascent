...
char *PyString_AsString(PyObject *py_obj)
{
    ...
    {
        PyObject *temp_bytes =
            PyUnicode_AsEncodedString(py_obj,
                                      "ASCII",
...
{
    PyObject_HEAD Ascent
        *ascent; // NoteIterator is light weight, we can deal with copies
};

//---------------------------------------------------------------------------//
// Helper that promotes ascent error to python error
//---------------------------------------------------------------------------//
static void PyAscent_MPI_Ascent_Error_To_PyErr(const conduit::Error &e)
{
...
// Helper that promotes ascent error to python error
//---------------------------------------------------------------------------//
static void PyAscent_MPI_Cpp_Error_To_PyErr(const char *msg)
{
    std::ostringstream oss;
...
//---------------------------------------------------------------------------//
static PyObject *PyAscent_MPI_Ascent_new(PyTypeObject *type,
...
//---------------------------------------------------------------------------//
static void PyAscent_MPI_Ascent_dealloc(PyAscent_MPI_Ascent *self)
{
    if (self->ascent != NULL)
    {
        delete self->ascent;
...
//---------------------------------------------------------------------------//
static int PyAscent_MPI_Ascent_init(PyAscent_MPI_Ascent *self,
                                    PyObject *, // args -- unused
                                    PyObject *) // kwds -- unused
{
...
//-----------------------------------------------------------------------------
static PyObject *PyAscent_MPI_Ascent_open(PyAscent_MPI_Ascent *self,
                                          PyObject *args,
                                          PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(
            args, kwargs, "|O", const_cast<char **>(kwlist), &py_node))
    {
        return NULL;
    }
...
//-----------------------------------------------------------------------------
static PyObject *PyAscent_MPI_Ascent_publish(PyAscent_MPI_Ascent *self,
                                             PyObject *args,
                                             PyObject *kwargs)
{
...
//-----------------------------------------------------------------------------
static PyObject *PyAscent_MPI_Ascent_execute(PyAscent_MPI_Ascent *self,
                                             PyObject *args,
                                             PyObject *kwargs)
{

    static const char *kwlist[] = {"actions", NULL};
...
//-----------------------------------------------------------------------------
static PyObject *PyAscent_MPI_Ascent_info(PyAscent_MPI_Ascent *self,
                                          PyObject *args,
                                          PyObject *kwargs)
{
...
//---------------------------------------------------------------------------//
static PyObject *PyAscent_MPI_Ascent_close(PyAscent_MPI_Ascent *self)
{
    self->ascent->close();
    Py_RETURN_NONE;
}
...
//---------------------------------------------------------------------------//
static PyObject *PyAscent_MPI_Ascent_str(PyAscent_MPI_Ascent *self)
{
...
static PyMethodDef PyAscent_MPI_Ascent_METHODS[] = {
    ...
    {"info",
     ...
     "{todo}"                                                                },
    //-----------------------------------------------------------------------//
    {"close",   (PyCFunction)PyAscent_MPI_Ascent_close, METH_NOARGS, "{todo}"},
    //-----------------------------------------------------------------------//
...
static PyTypeObject PyAscent_MPI_Ascent_TYPE = {
    PyVarObject_HEAD_INIT(NULL, 0) "Ascent",
...
// ascent::about
//---------------------------------------------------------------------------//
static PyObject *PyAscent_MPI_about()
{
...
//---------------------------------------------------------------------------//
static PyObject *PyAscent_MPI_execute_callback(PyObject *self, PyObject *args)
{
    char *callback_name;
    PyObject *py_params = NULL;
    PyObject *py_output = NULL;

    if (!PyArg_ParseTuple(args, "sOO", &callback_name, &py_params, &py_output))
    {
        return NULL;
    }
...
static PyMethodDef ascent_mpi_python_funcs[] = {
    //-----------------------------------------------------------------------//
    {"about",            (PyCFunction)PyAscent_MPI_about, METH_NOARGS,  NULL},
    //-----------------------------------------------------------------------//
    {"execute_callback",
...
#if defined(IS_PY3K)
//---------------------------------------------------------------------------//
static int ascent_mpi_python_traverse(PyObject *m, visitproc visit, void *arg)
{
...
//---------------------------------------------------------------------------//
static int ascent_mpi_python_clear(PyObject *m)
{
...
//---------------------------------------------------------------------------//
#if defined(IS_PY3K)
    PyObject *ASCENT_MPI_PYTHON_API
    PyInit_ascent_mpi_python(void)
#else
    void ASCENT_MPI_PYTHON_API
    initascent_mpi_python(void)
#endif
//---------------------------------------------------------------------------//
{
    //-----------------------------------------------------------------------//
...
#else
    PyObject *py_module =
        Py_InitModule((char *)"ascent_mpi_python", ascent_mpi_python_funcs);
#endif
    ...
    st->error =
        PyErr_NewException((char *)"ascent_mpi_python.Error", NULL, NULL);
    if (st->error == NULL)
    {
        Py_DECREF(py_module);
        PY_MODULE_INIT_RETURN_ERROR;
    ...
    PyModule_AddObject(
        py_module, "Ascent", (PyObject *)&PyAscent_MPI_Ascent_TYPE);
