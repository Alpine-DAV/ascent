...
WebInterface::WebInterface()
    : m_enabled(false), m_ms_poll(100), m_ms_timeout(100), m_doc_root("")
{
}
...
//-----------------------------------------------------------------------------
void WebInterface::SetDocumentRoot(const std::string &path)
{
    m_doc_root = path;
}
...
WebSocket *WebInterface::Connection()
{
    ...
    if (!m_server.is_running())
    {
        ...
        std::string default_root = conduit::utils::join_file_path(
            web_client_root_directory(), "ascent");
        ...
        if (m_doc_root != default_root)
        {
            ...
            if (conduit::utils::is_file(
                    conduit::utils::join_file_path(m_doc_root, "index.html")))
            {
                ...
                {
                    ASCENT_ERROR(
                        "Failed to load compiled resources for ascent_web");
                }

                ascent::resources::expand_resource_tree_to_file_system(
                    ascent_rc, m_doc_root);
            }
        }
    ...
    WebSocket *wsock =
        m_server.websocket((index_t)m_ms_poll, (index_t)m_ms_timeout);
    return wsock;
}

//-----------------------------------------------------------------------------
void WebInterface::PushMessage(const Node &msg)
{
    //  Don't do any more work unless we have a valid client connection
...
//-----------------------------------------------------------------------------
void WebInterface::PushRenders(const Node &renders)
{
    //  Don't do any more work unless we have a valid client connection
    // (also handles case where stream is not enabled)
    WebSocket *wsock = Connection();
    ...
    while (itr.has_next())
    {
        ...
        EncodeImage(curr.as_string(), msg["renders"].append());
    }
...
//-----------------------------------------------------------------------------
void WebInterface::EncodeImage(const std::string &png_image_path,
                               conduit::Node &out)
{
    out.reset();

    std::ifstream file(png_image_path.c_str(), std::ios::binary);
    ...
    utils::base64_encode(
        png_raw_ptr, png_raw_bytes, png_data["encoded"].data_ptr());
