...
//-----------------------------------------------------------------------------
void RoverXRay::declare_interface(Node &i)
{
    i["type_name"] = "xray";
...
//-----------------------------------------------------------------------------
bool RoverXRay::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    bool res = true;
    ...
    {
        info["errors"].append() =
            "Missing required string parameter 'absorption'";
        res = false;
    }
    ...
    {
        info["errors"].append() =
            "Missing required string parameter 'filename'";
        res = false;
    }
    ...
    {
        info["errors"].append() =
            "Optional parameter 'emission' must be a string";
        res = false;
    }
    ...
    if (params.has_path("image_params"))
    {
        ...
        {
            info["errors"].append() = "Missing required image parameter "
                                      "'log_scale' must be a string";
            res = false;
        }
    ...
    {
        info["errors"].append() =
            "Optional parameter 'precision' must be a string";
        std::string prec = params["precision"].as_string();
        if (prec != "single" || prec != "double")
        {
            info["errors"].append() =
                "Parameter 'precision' must be 'single' or 'double'";
        }
        res = false;
    }
...
void RoverXRay::execute()
{
    ...
    {
        settings.m_energy_settings.m_unit_scalar =
            params()["unit_scalar"].to_float64();
    }
    ...
    if (params().has_path("blueprint"))
    {
        ...
        mesh_blueprint_save(
            multi_domain, filename, protocol, -1, extra_opts, result_path);
    }
    ...
    if (params().has_path("image_params"))
    {
        ...
        bool log_scale =
            params()["image_params/log_scale"].as_string() == "true";
...
//-----------------------------------------------------------------------------
RoverVolume::RoverVolume() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void RoverVolume::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool RoverVolume::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
    bool res = true;

    if (!params.has_child("field") || !params["field"].dtype().is_string())
    {
...
void RoverVolume::execute()
{
    ...
    {
        settings.m_volume_settings.m_num_samples =
            params()["samples"].to_int32();
    }

    if (params().has_path("min_value"))
    {
        settings.m_volume_settings.m_scalar_range.Min =
            params()["min_value"].to_float32();
    }

    if (params().has_path("max_value"))
    {
        settings.m_volume_settings.m_scalar_range.Max =
            params()["max_value"].to_float32();
    }
