...
namespace detail
{
std::string check_color_table_surprises(const conduit::Node &color_table)
{
    ...
    if (color_table.has_path("control_points"))
    {
        ...
        {
            // Valid path options for the compressed control points input
            // format
            std::vector<std::string> c_valid_paths;
...
std::string check_renders_surprises(const conduit::Node &renders_node)
{
    std::string surprises;
    ...
    for (int i = 0; i < num_renders; ++i)
    {
        ...
        surprises +=
            surprise_check(r_valid_paths, r_ignore_paths, render_node);

        if (render_node.has_path("phi_theta_positions"))
        {
            const conduit::Node &phi_theta_positions =
                render_node["phi_theta_positions"];
...
public:
    RendererContainer() : m_valid(false) {};
    RendererContainer(std::string key,
        ...
        : m_key(key), m_registry(r), m_collection(collection),
          m_topo_name(topo_name), m_valid(true), m_data(data_object)
    {
...
public:
    AscentScene(flow::Registry *r) : m_registry(r), m_renderer_count(0) {}
    ...
    void Execute(std::vector<vtkh::Render> &renders)
    {
        ...
        for (int i = 0; i < m_renderer_count; i++)
        {
            ...
            oss << "key_" << i;
            vtkh::Renderer *r =
                m_registry->fetch<RendererContainer>(oss.str())->Fetch();
            scene.AddRenderer(r);
        }
                          ...
                          const std::string &image_name)
{
    ...
    //
    vtkh::Render render =
        vtkh::MakeRender(image_width, image_height, bounds, image_name);
    ...
    if (!annot_all_off && render_node.has_path("world_annotations"))
    {
        ...
        {
            ASCENT_ERROR(
                "render/world_annotations node must be a string value");
        }
    ...
    if (!annot_all_off && render_node.has_path("screen_annotations"))
    {
        ...
        {
            ASCENT_ERROR(
                "render/screen_annotations node must be a string value");
        }
        const std::string annot =
            render_node["screen_annotations"].as_string();
    ...
    if (render_node.has_path("color_bar_position"))
    {
        if (!render_node["color_bar_position"].dtype().is_number() ||
            render_node["color_bar_position"].dtype().number_of_elements() %
                    4 !=
                0)
        {
            ASCENT_ERROR("render/color_bar_position must be an array of 4 "
                         "values for each color bar");
        }

        int positions =
            render_node["color_bar_position"].dtype().number_of_elements() / 4;
...
public:
    CinemaManager(vtkm::Bounds bounds,
        ...
        : m_bounds(bounds), m_image_name(image_name), m_time(0.f)
    {
        if (render_node.has_path("phi_theta_positions"))
        {
            const conduit::Node &positions =
                render_node["phi_theta_positions"];
            for (int p = 0; p < positions.number_of_children(); ++p)
            {
                float64_accessor phi_theta =
                    positions[p].as_float64_accessor();
                if (phi_theta.number_of_elements() != 2)
                {
                    ASCENT_ERROR("Cinema camera phi_theta_positions must be "
                                 "an array of tuples");
                }
        ...
        else
        {
            ...
            if (render_node.has_path("phi"))
            {
                ...
                this->create_angles_from_range(
                    m_phi_values, m_phi_min, m_phi_inc, m_phi);
            }
            else if (render_node.has_path("phi_angles"))
            {
                float64_accessor phi_angles =
                    render_node["phi_angles"].as_float64_accessor();
                for (int p = 0; p < phi_angles.number_of_elements(); ++p)
                {
                    m_phi_values.push_back(phi_angles[p]);
                }
            }
            else if (render_node.has_path("phi_range") &&
                     render_node.has_path("dphi"))
            {
                float64_accessor phi_range =
                    render_node["phi_range"].as_float64_accessor();
                if (phi_range[0] >= phi_range[1])
                {
                    ASCENT_ERROR("Cinema camera phi_range[0] must be less "
                                 "that phi_range[1]");
                }
                m_phi_min = phi_range[0];
                m_phi_inc = render_node["dphi"].to_float64();
                m_phi = int(floor((phi_range[1] - phi_range[0] + 1.0e-6) /
                                  m_phi_inc)) +
                        1;
                this->create_angles_from_range(
                    m_phi_values, m_phi_min, m_phi_inc, m_phi);
                     ...
                     render_node.has_path("phi_num_angles"))
            {
                float64_accessor phi_range =
                    render_node["phi_range"].as_float64_accessor();
                if (phi_range[0] >= phi_range[1])
            ...
            if (render_node.has_path("theta"))
            {
                ...
                this->create_angles_from_range(
                    m_theta_values, m_theta_min, m_theta_inc, m_theta);
            }
            else if (render_node.has_path("theta_angles"))
            {
                float64_accessor theta_angles =
                    render_node["theta_angles"].as_float64_accessor();
                for (int t = 0; t < theta_angles.number_of_elements(); ++t)
                {
                    m_theta_values.push_back(theta_angles[t]);
                }
            }
            else if (render_node.has_path("theta_range") &&
                     render_node.has_path("dtheta"))
            {
                float64_accessor theta_range =
                    render_node["theta_range"].as_float64_accessor();
                if (theta_range[0] >= theta_range[1])
                {
                    ASCENT_ERROR("Cinema camera theta_range[0] must be less "
                                 "that theta_range[1]");
                }
                m_theta_min = theta_range[0];
                m_theta_inc = render_node["dtheta"].to_float64();
                m_theta =
                    int(floor((theta_range[1] - theta_range[0] + 1.0e-6) /
                              m_theta_inc)) +
                    1;
                this->create_angles_from_range(
                    m_theta_values, m_theta_min, m_theta_inc, m_theta);
                     ...
                     render_node.has_path("theta_num_angles"))
            {
                float64_accessor theta_range =
                    render_node["theta_range"].as_float64_accessor();
                if (theta_range[0] >= theta_range[1])
                ...
                m_theta_inc =
                    (theta_range[1] - theta_range[0]) / double(m_theta - 1);
                this->create_angles_from_range(
                    m_theta_values, m_theta_min, m_theta_inc, m_theta);
            }
            else
            {
                ASCENT_ERROR(
                    "Cinema camera must specify theta or theta_range");
            }
    ...
    void add_time_step()
    {
        ...
        if (rank == 0 && !conduit::utils::is_directory(m_db_path))
        {
            ...
            {
                ASCENT_ERROR(
                    "Failed to load compiled resources for cinema_web");
            }
                      ...
                      const conduit::Node &render_node)
    {
        ...
        if (render_copy.has_path("camera/zoom"))
        {
            ...
            {
                ASCENT_ERROR(
                    "Zoom must be greater than zero. Default zoom is 1.\n");
            }
        ...
        std::string tmp_name = "";
        vtkh::Render render =
            detail::parse_render(render_copy, m_bounds, tmp_name);
        ...
        for (int i = 0; i < num_renders; ++i)
        {
            ...
            std::string image_name =
                conduit::utils::join_file_path(m_image_path, m_image_names[i]);
    ...
    void write_metadata()
    {
        ...
        for (int a = 0; a < m_camera_angles.size(); ++a)
        {
            ...
            csv << theta << ",";
            csv << current_time << ",";
            csv << current_time << "/" << phi << "_" << theta << "_"
                << m_image_name << ".png\n";
        }
...
private:
    void create_angles_from_range(std::vector<float> &angles,
                                  const float min,
                                  const float inc,
                                  const int n_angles)
    {
    ...
    void create_cinema_angles()
    {
        for (int p = 0; p < m_phi_values.size(); ++p)
        {
            ...
            {
                std::tuple<float, float> angles(m_phi_values[p],
                                                m_theta_values[t]);
                m_camera_angles.push_back(angles);
...
public:
                          ...
                          std::string path)
    {
        ...
        m_databases.emplace(std::make_pair(
            db_name, CinemaManager(bounds, render_node, db_name, path)));
    }
...
//-----------------------------------------------------------------------------
DefaultRender::DefaultRender() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DefaultRender::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DefaultRender::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
...
void DefaultRender::execute()
{
    ...
    if (params().has_path("renders"))
    {
        ...
        for (int i = 0; i < num_renders; ++i)
        {
            ...
            if (is_cinema)
            {
...
#ifdef ASCENT_MPI_ENABLED
                    MPI_Comm mpi_comm =
                        MPI_Comm_f2c(Workspace::default_mpi_comm());
                    MPI_Comm_rank(mpi_comm, &rank);
#endif
                    // create a folder if it doesn't exist
                    if (rank == 0 &&
                        !conduit::utils::is_directory(output_path))
                    {
                ...
                if (!exists)
                {
                    detail::CinemaDatabases::create_db(
                        *bounds, render_node, db_name, output_path);
                }

                detail::CinemaManager &manager =
                    detail::CinemaDatabases::get_db(db_name);
                ...
                if (!graph().workspace().registry().has_entry("extract_list"))
                {
                    ...
                    graph().workspace().registry().add<Node>(
                        "extract_list",
                        extract_list,
                        -1); // TODO keep forever?
                }
            ...
            else
            {
                ...
                {
                    std::stringstream ss;
                    ss << expand_family_name(
                        render_node["image_prefix"].as_string(), cycle);
                    image_name = ss.str();
                    image_name = output_dir(image_name);
                }
                else
                ...
                {
                    DataObject *source =
                        graph().workspace().registry().fetch<DataObject>(
                            "source_object");

                    std::shared_ptr<VTKHCollection> collection =
                        source->as_vtkh_collection();
                    ...
                    std::string field_name =
                        render_node["auto_camera/field"].as_string();
                    std::string metric =
                        render_node["auto_camera/metric"].as_string();
                    int samples =
                        render_node["auto_camera/samples"].as_int32();
                    ...
                    std::string topo_name =
                        collection->field_topology(field_name);
                    vtkh::DataSet &dataset =
                        collection->dataset_by_topology(topo_name);
                    ...
                    vtkm::rendering::Camera *camera =
                        new vtkm::rendering::Camera;
                    *camera = auto_cam.GetCamera();
                    vtkh::Render render = vtkh::MakeRender(
                        width, height, scene_bounds, *camera, image_name);
                ...
                else
                {
                    ...
                    vtkh::Render render = detail::parse_render(
                        render_node, scene_bounds, image_name);
                    renders->push_back(render);
                }
            }
        }
    ...
    else
    {
        ...
        vtkh::Render render =
            vtkh::MakeRender(1024, 1024, scene_bounds, image_name);
...
//-----------------------------------------------------------------------------
void VTKHBounds::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
VTKHUnionBounds::VTKHUnionBounds() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHUnionBounds::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
void AddPlot::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
void CreatePlot::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool CreatePlot::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
    ...
    {
        surprises +=
            detail::check_color_table_surprises(params["color_table"]);
    }
...
void CreatePlot::execute()
{
    ...
    {
        std::string fpath = filter_to_path(this->name());
        ASCENT_INFO(fpath << " " << type
                          << " plot yielded no data, i.e., no cells remain");
    }
    ...
    if (plot_params.has_path("color_table"))
    {
        vtkm::cont::ColorTable color_table =
            parse_color_table(plot_params["color_table"]);
        if (type != "mesh")
        {
            if (plot_params["color_table"].has_path("annotation"))
            {
                if (plot_params["color_table/annotation"].as_string() ==
                    "false")
                {
            ...
            if (type != "volume")
            {
                ...
                {
                    if (plot_params["color_table/discrete"].as_string() ==
                        "true")
                    {
    ...
    if (type == "mesh")
    {
        vtkh::MeshRenderer *mesh =
            dynamic_cast<vtkh::MeshRenderer *>(renderer);
        if (!plot_params.has_path("field"))
        {
...
//-----------------------------------------------------------------------------
CreateScene::CreateScene() : Filter() {}

//-----------------------------------------------------------------------------
CreateScene::~CreateScene() {}

//-----------------------------------------------------------------------------
void CreateScene::declare_interface(Node &i)
{
...
{
    detail::AscentScene *scene =
        new detail::AscentScene(&graph().workspace().registry());
    set_output<detail::AscentScene>(scene);
}

//-----------------------------------------------------------------------------
ExecScene::ExecScene() : Filter() {}

//-----------------------------------------------------------------------------
ExecScene::~ExecScene() {}

//-----------------------------------------------------------------------------
void ExecScene::declare_interface(conduit::Node &i)
{
...
void generate_camera_meshes(conduit::Node &image_data)
{
    ...
    double x_scene_bounds[] = {scene_bounds[0],
                               scene_bounds[0],
                               scene_bounds[0],
                               scene_bounds[0],
                               scene_bounds[3],
                               ...
                               scene_bounds[3],
                               scene_bounds[3]};
    double y_scene_bounds[] = {scene_bounds[1],
                               scene_bounds[1],
                               scene_bounds[4],
                               scene_bounds[4],
                               scene_bounds[1],
                               ...
                               scene_bounds[4],
                               scene_bounds[4]};
    double z_scene_bounds[] = {scene_bounds[2],
                               scene_bounds[5],
                               scene_bounds[5],
                               scene_bounds[2],
                               scene_bounds[2],
    ...
    cam_frust["topologies/scene_bounds_topo/elements/connectivity"] = {
        0, 1, 1, 2, 2, 3, 3, 0, 4, 5, 5, 6,
        6, 7, 7, 4, 0, 4, 1, 5, 2, 6, 3, 7};

    // Initializing look vector from position to the "look_at" point of
    // interest
    float64_accessor position = camera["position"].value();
    float64_accessor look_at = camera["look_at"].value();
    vtkm::Vec<vtkm::Float64, 3> vtkm_look_at(
        look_at[0], look_at[1], look_at[2]);
    vtkm::Vec<vtkm::Float64, 3> vtkm_position(
    ...
    double angle_between =
        vtkm::ACos(vtkm::Dot(forward, vtkm_look)) / vtkm::Pi() * 180;

    // If the look vector has been rotated by a certain angle, adjust the
    // camera up vector to match
    if (vtkm::Abs(angle_between) >= 0.001)
    {
        vtkm::Vec<vtkm::Float64, 3> axisOfRotation =
            vtkm::Cross(vtkm_look, forward);
        vtkm_up = vtkm::Transform3DVector(
            vtkm::Transform3DRotate(-angle_between, axisOfRotation), vtkm_up);
    }
    vtkm::Normalize(vtkm_up);

    // Identifying points where the look vector intersects with the near and
    // far frustum planes
    ...
    vtkm::Vec<vtkm::Float64, 3> look_near_pt =
        (vtkm_look * near_dist) + vtkm_position;
    vtkm::Vec<vtkm::Float64, 3> look_far_pt =
        (vtkm_look * far_dist) + vtkm_position;
    ...
    // Near frustum
    double frust_near_height =
        near_dist * vtkm::Tan(fov * 0.5 * vtkm::Pi() / 180.0) / zoom;
    double frust_near_width = frust_near_height;
    vtkm::Vec<vtkm::Float64, 3> near_frust_ll =
        look_near_pt + (-1 * vtkm_up * frust_near_height) +
        (vtkm_side * frust_near_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> near_frust_lr =
        look_near_pt + (-1 * vtkm_up * frust_near_height) +
        (-1 * vtkm_side * frust_near_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> near_frust_ur =
        look_near_pt + (vtkm_up * frust_near_height) +
        (-1 * vtkm_side * frust_near_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> near_frust_ul =
        look_near_pt + (vtkm_up * frust_near_height) +
        (vtkm_side * frust_near_width * image_aspect);
    // Far frustum
    double frust_far_height =
        far_dist * vtkm::Tan(fov * 0.5 * vtkm::Pi() / 180.0) / zoom;
    double frust_far_width = frust_far_height;
    vtkm::Vec<vtkm::Float64, 3> far_frust_ll =
        look_far_pt + (-1 * vtkm_up * frust_far_height) +
        (vtkm_side * frust_far_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> far_frust_lr =
        look_far_pt + (-1 * vtkm_up * frust_far_height) +
        (-1 * vtkm_side * frust_far_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> far_frust_ur =
        look_far_pt + (vtkm_up * frust_far_height) +
        (-1 * vtkm_side * frust_far_width * image_aspect);
    vtkm::Vec<vtkm::Float64, 3> far_frust_ul =
        look_far_pt + (vtkm_up * frust_far_height) +
        (vtkm_side * frust_far_width * image_aspect);

    // Assembling frustum mesh
    vtkm::Vec<vtkm::Float64, 3> up_vector_pt =
        vtkm_up * (far_dist - near_dist) * 0.5 + look_near_pt;
    double x_val_frust[] = {near_frust_ll[0],
                            near_frust_lr[0],
                            near_frust_ur[0],
                            near_frust_ul[0],
                            far_frust_ll[0],
                            ...
                            look_far_pt[0],
                            up_vector_pt[0]};
    double y_val_frust[] = {near_frust_ll[1],
                            near_frust_lr[1],
                            near_frust_ur[1],
                            near_frust_ul[1],
                            far_frust_ll[1],
                            ...
                            look_far_pt[1],
                            up_vector_pt[1]};
    double z_val_frust[] = {near_frust_ll[2],
                            near_frust_lr[2],
                            near_frust_ur[2],
                            near_frust_ul[2],
                            far_frust_ll[2],
    ...
    cam_frust["topologies/camera_frustum_topo/coordset"] =
        "camera_frustum_coords";
    cam_frust["topologies/camera_frustum_topo/elements/shape"] = "line";
    cam_frust["topologies/camera_frustum_topo/elements/connectivity"] = {
    ...
    cam_frust["topologies/clipping_planes_topo/coordset"] =
        "camera_frustum_coords";
    cam_frust["topologies/clipping_planes_topo/elements/shape"] = "quad";
    cam_frust["topologies/clipping_planes_topo/elements/connectivity"] = {
        0, 1, 2, 3,  // Near plane
        4, 5, 6, 7,  // Far plane
        0, 1, 5, 4,  // Lower plane
        2, 3, 7, 6,  // Upper plane
...
void ExecScene::execute()
{
    ...
    {
        ASCENT_ERROR(
            "'renders' must be a std::vector<vtkh::Render> * instance");
    }
    ...
    conduit::Node *image_list =
        graph().workspace().registry().fetch<Node>("image_list");
    for (int i = 0; i < renders->size(); ++i)
    {
        ...
        if (renders->at(i).GetCamera().GetMode() ==
        ...
        {
            image_data["camera/position"].set(
                &renders->at(i).GetCamera().GetPosition()[0], 3);
            image_data["camera/look_at"].set(
                &renders->at(i).GetCamera().GetLookAt()[0], 3);
            image_data["camera/up"].set(
                &renders->at(i).GetCamera().GetViewUp()[0], 3);
            image_data["camera/zoom"] = renders->at(i).GetCamera().GetZoom();
            image_data["camera/fov"] =
                renders->at(i).GetCamera().GetFieldOfView();
            image_data["camera/near_plane"] =
                renders->at(i).GetCamera().GetClippingRange().Min;
            image_data["camera/far_plane"] =
                renders->at(i).GetCamera().GetClippingRange().Max;
        }
