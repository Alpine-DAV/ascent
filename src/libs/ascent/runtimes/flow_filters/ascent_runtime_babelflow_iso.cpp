...
namespace BabelFlow
{
extern int relay_message(std::vector<Payload> &inputs,
...
struct IsoSurfaceData
{
    ...
    // IsoSurfaceData( vtkh::DataSet* data_set, const std::string& fld_name,
    // const double* iso_vals, int num_iso_vals ) : m_DataSet( data_set ),
    // m_FieldName( fld_name ), m_IsoVals( iso_vals, iso_vals + num_iso_vals )
    // {}
    ...
    BabelFlow::Payload serialize() const
    {
        ...
        uint32_t payl_size =
            sizeof(vtkh::DataSet *) + sizeof(size_t) + m_FieldName.size() +
            sizeof(size_t) + iso_vals_buff_size + 2 * sizeof(int) +
            sizeof(size_t) + m_FileName.size() + payl_bounds.size();

        char *out_buffer = new char[payl_size];
        uint32_t offset = 0;
        memcpy(out_buffer + offset,
               (const char *)&m_DataSet,
               sizeof(vtkh::DataSet *));
        offset += sizeof(vtkh::DataSet *);
        size_t fld_name_size = m_FieldName.size();
        memcpy(
            out_buffer + offset, (const char *)&fld_name_size, sizeof(size_t));
        offset += sizeof(size_t);
        memcpy(out_buffer + offset, m_FieldName.c_str(), m_FieldName.size());
        offset += m_FieldName.size();
        size_t num_iso_vals = m_IsoVals.size();
        memcpy(
            out_buffer + offset, (const char *)&num_iso_vals, sizeof(size_t));
        offset += sizeof(size_t);
        memcpy(out_buffer + offset,
               (const char *)m_IsoVals.data(),
               iso_vals_buff_size);
    ...
    void deserialize(BabelFlow::Payload payload)
    {
        ...
        memcpy((char *)&m_DataSet,
               payload.buffer() + offset,
               sizeof(vtkh::DataSet *));
        offset += sizeof(vtkh::DataSet *);

        memcpy((char *)(&fld_name_len),
               payload.buffer() + offset,
               sizeof(size_t));
        offset += sizeof(size_t);
        ...
        memcpy(
            (char *)&file_name_len, payload.buffer() + offset, sizeof(size_t));
        offset += sizeof(size_t);

        m_FileName.assign(payload.buffer() + offset, file_name_len);
        offset += m_FileName.size();

        m_bounds.deserialize(BabelFlow::Payload(payload.size() - offset,
                                                payload.buffer() + offset));
    }
};
                   ...
                   BabelFlow::TaskId task_id)
{
    ...
    marcher.SetIsoValues(iso_surf_data.m_IsoVals.data(),
                         iso_surf_data.m_IsoVals.size());
    marcher.Update();
                   ...
                   BabelFlow::TaskId task_id)
{
    ...
    vtkh::Render render =
        vtkh::MakeRender(iso_surf_data.m_Width,
    ...
    // {
    //   auto ranges = iso_surf_data.m_DataSet->GetGlobalRange(
    //   iso_surf_data.m_FieldName ); int num_components =
    //   ranges.GetNumberOfValues(); std::cout << "vtkm_rendering -- range num
    //   components = " << num_components << std::endl; vtkm::Range
    //   global_range = ranges.ReadPortal().Get(0);
...
#ifdef BFLOW_ISO_DEBUG
    {
        ...
        std::cout << "Num domains: "
                  << iso_surf_data.m_DataSet->GetNumberOfDomains()
                  << std::endl;
        std::cout << "Color table: " << renderer.GetColorTable().GetName()
                  << std::endl;
    ...
    const vtkm::cont::Field &field =
        data_set.GetField(renderer.GetFieldName());
    ...
    mapper->RenderCells(
        cellset, coords, field, renderer.GetColorTable(), cam, range);
    ...
    assert(color_portal.GetNumberOfValues() ==
           iso_surf_data.m_Width * iso_surf_data.m_Height);

    bflow_comp::ImageData input_img;
    input_img.image = new bflow_comp::ImageData::PixelType
        [iso_surf_data.m_Width * iso_surf_data.m_Height *
         bflow_comp::ImageData::sNUM_CHANNELS];
    input_img.zbuf =
        new bflow_comp::ImageData::PixelType[iso_surf_data.m_Width *
                                             iso_surf_data.m_Height];
    ...
    input_img.bounds[3] = input_img.rend_bounds[3] =
        iso_surf_data.m_Height - 1;
    ...
    for (vtkm::Id index = 0; index < color_portal.GetNumberOfValues(); ++index)
    {
        ...
        // input_img.image[img_offset + 0] = (unsigned char)(cur_color[0] *
        // 255.f); input_img.image[img_offset + 1] = (unsigned
        // char)(cur_color[1] * 255.f); input_img.image[img_offset + 2] =
        // (unsigned char)(cur_color[2] * 255.f); input_img.image[img_offset +
        // 3] = (unsigned char)(cur_color[3] * 255.f);
              ...
              BabelFlow::TaskId task_id)
{
    ...
    {
        // X axis
        bounds_red.m_rangeVec[0] =
            std::min(bounds_red.m_rangeVec[0], bounds_v[i].m_rangeVec[0]);
        bounds_red.m_rangeVec[1] =
            std::max(bounds_red.m_rangeVec[1], bounds_v[i].m_rangeVec[1]);
        // Y axis
        bounds_red.m_rangeVec[2] =
            std::min(bounds_red.m_rangeVec[2], bounds_v[i].m_rangeVec[2]);
        bounds_red.m_rangeVec[3] =
            std::max(bounds_red.m_rangeVec[3], bounds_v[i].m_rangeVec[3]);
        // Z axis
        bounds_red.m_rangeVec[4] =
            std::min(bounds_red.m_rangeVec[4], bounds_v[i].m_rangeVec[4]);
        bounds_red.m_rangeVec[5] =
            std::max(bounds_red.m_rangeVec[5], bounds_v[i].m_rangeVec[5]);
    }

#ifdef BFLOW_ISO_DEBUG
    {
        std::cout << "allreduce -- task_id = " << task_id
                  << ", inputs = " << inputs.size()
...
public:
    BabelIsoGraph(const IsoSurfaceData &iso_surf_data,
        ...
        : BabelCompRadixK(bflow_comp::ImageData(),
                          img_name,
                          rank_id,
                          n_blocks,
                          2,
    ...
    virtual void Initialize() override
    {
        ...
        BabelFlow::TaskGraph::registerCallback(
            0, BabelFlow::RadixKExchange::LEAF_TASK_CB, allreduce);
        BabelFlow::TaskGraph::registerCallback(
            0, BabelFlow::RadixKExchange::MID_TASK_CB, allreduce);
        BabelFlow::TaskGraph::registerCallback(
            0, BabelFlow::RadixKExchange::ROOT_TASK_CB, allreduce);

        BabelFlow::TaskGraph::registerCallback(
            1, BabelFlow::SingleTaskGraph::SINGLE_TASK_CB, marching_cubes);

        BabelFlow::TaskGraph::registerCallback(
            2, BabelFlow::SingleTaskGraph::SINGLE_TASK_CB, vtkm_rendering);

        BabelFlow::TaskGraph::registerCallback(
            3,
            BabelFlow::RadixKExchange::LEAF_TASK_CB,
            bflow_comp::volume_render_radixk);
        BabelFlow::TaskGraph::registerCallback(
            3,
            BabelFlow::RadixKExchange::MID_TASK_CB,
            bflow_comp::composite_radixk);
        BabelFlow::TaskGraph::registerCallback(
            3,
            BabelFlow::RadixKExchange::ROOT_TASK_CB,
            bflow_comp::composite_radixk);

        BabelFlow::TaskGraph::registerCallback(
            4,
            BabelFlow::KWayReduction::LEAF_TASK_CB,
            BabelFlow::relay_message);
        BabelFlow::TaskGraph::registerCallback(
            4,
            BabelFlow::KWayReduction::MID_TASK_CB,
            bflow_comp::gather_results_radixk);
        BabelFlow::TaskGraph::registerCallback(
            4,
            BabelFlow::KWayReduction::ROOT_TASK_CB,
            bflow_comp::write_results_radixk);

        // m_isoGrConnector_1 = BabelFlow::DefGraphConnector( &m_redAllGr, 0,
        // &m_isoCalcTaskGr, 1 ); m_isoGrConnector_2 =
        // BabelFlow::DefGraphConnector( &m_isoCalcTaskGr, 1,
        // &m_isoRenderTaskGr, 2 ); m_isoGrConnector_3 =
        // BabelFlow::DefGraphConnector( &m_isoRenderTaskGr, 2, &m_radixkGr, 3
        // ); m_defGraphConnector = BabelFlow::DefGraphConnector( &m_radixkGr,
        // 3, &m_gatherTaskGr, 4 );
        ...
        //                                                            &m_defGraphConnector
        //                                                            };
        std::vector<BabelFlow::TaskGraph *> gr_vec{&m_redAllGr,
                                                   &m_isoCalcTaskGr,
                                                   &m_isoRenderTaskGr,
                                                   &m_radixkGr,
                                                   &m_gatherTaskGr};
        std::vector<BabelFlow::TaskMap *> task_maps{&m_redAllMp,
                                                    &m_isoCalcTaskMp,
                                                    &m_isoRenderTaskMp,
                                                    &m_radixkMp,
                                                    &m_gatherTaskMp};
        ...
        if (m_rankId == 0)
        {
            m_isoCalcTaskGr.outputGraphHtml(
                m_nRanks, &m_isoCalcTaskMp, "iso-gr.html");
            m_isoRenderTaskGr.outputGraphHtml(
                m_nRanks, &m_isoRenderTaskMp, "gather-task.html");
            m_radixkGr.outputGraphHtml(m_nRanks, &m_radixkMp, "radixk.html");
            // m_gatherTaskGr.outputGraphHtml( m_nRanks, &m_gatherTaskMp,
            // "gather-task.html" );
            m_radGatherGraph.outputGraphHtml(
                m_nRanks, &m_radGatherTaskMap, "bflow-iso.html");
        }
#endif
        ...
        m_master.initialize(
            m_radGatherGraph, &m_radGatherTaskMap, m_comm, &m_contMap);

        m_inputs[BabelFlow::TaskId(m_rankId, 0)] =
            m_isoSurfData.m_bounds.serialize();
...
bool ascent::runtime::filters::BFlowIso::verify_params(
...
void ascent::runtime::filters::BFlowIso::execute()
{
    ...
    iso_surf_data.m_IsoVals.assign(
        n_iso_vals_dbls.as_double_ptr(),
        n_iso_vals_dbls.as_double_ptr() +
...
#ifdef BFLOW_ISO_DEBUG
    {
        if (my_rank == 0)
        {
            ...
            std::cout << "Field name = " << iso_surf_data.m_FieldName
                      << std::endl;
            std::cout << "Iso vals = ";
            ...
            std::cout << "File name = " << iso_surf_data.m_FileName
                      << std::endl;
            auto ranges =
                iso_surf_data.m_DataSet->GetRange(iso_surf_data.m_FileName);
    ...
    bflow_iso::BabelIsoGraph bflow_iso_gr(
        iso_surf_data, image_name, my_rank, n_ranks, mpi_comm, radix_v);
