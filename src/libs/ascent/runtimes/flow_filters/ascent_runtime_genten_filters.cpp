...
//-----------------------------------------------------------------------------
///
/// file: ascent_runtime_genten_filters.cpp
///
//-----------------------------------------------------------------------------
    ...
    std::vector<std::vector<std::pair<std::string, double>>> &mesh_data)
{
    ...
    for (int i = 0; i < input.number_of_children() && all_good; ++i)
    {
        ...
        // create the "small" version, which is a single element mesh per
        // domain this is a uniform grid that spans the spatial extends of the
        // input mesh.
        //
        ...
        std::string coords_name =
            dom["topologies/" + topo + "/coordset"].as_string();
        ...
        if (in_coords["type"].as_string() == "uniform")
        {
            ...
            out_coords["spacing/dx"] =
                in_i * in_coords["spacing/dx"].to_float64();
            out_coords["spacing/dy"] =
                in_j * in_coords["spacing/dy"].to_float64();
            out_coords["spacing/dz"] =
                in_k * in_coords["spacing/dz"].to_float64();

            std::vector<std::pair<std::string, double>> &dom_scalars =
                mesh_data[i];
    ...
    conduit::relay::io::blueprint::save_mesh(
        output, output_path + "_small", "hdf5");
}
                  ...
                  double *fmms) // output of size fields
{
    ...
    for (int i = 0; i < nfields; i++)
    {
        ...
        eigenvalues[i] =
            std::sqrt(std::abs(eigenvalues[i])); // Need to include cmath.h
    ...
    for (int j = 0; j < nfields; j++)
    {
        for (int i = 0; i < nfields; i++)
        {
            fmms[i] += eigenvalues[j] *
                       (kvecs[i + j * nfields] * kvecs[i + j * nfields]);
        }
    }
...
//-----------------------------------------------------------------------------
void Learn::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool Learn::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
...
void Learn::execute()
{
    ...
    for (int i = 0; i < n_input->number_of_children(); ++i)
    {
        ...
        for (int f = 0; f < num_fields; ++f)
        {
            ...
            if (f == 0)
            {
                ...
                topo = f_topo;
                field_sizes[i] =
                    dom[fpath + "/values"].dtype().number_of_elements();
            }
            else
            {
    ...
    for (int f = 0; f < num_fields; ++f)
    {
        ...
        if (rank == 0)
        {
            std::cout << field_selection[f] << " min " << field_mins[f]
                      << " len " << field_lengths[f] << "\n";
        }
    }
    ...
    double *kVecs =
        new double[num_fields * num_fields]; // TODO: need one per domain!!!!
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        // double *fmms = FormRawMomentTensor(A, field_sizes[0], num_fields,
        // order);

        double *pvecs = new double[num_fields * num_fields];
        double *pvals = new double[num_fields];
        ComputePrincipalKurtosisVectors(
            A, field_sizes[i], num_fields, pvecs, pvals);
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        if (norm_eigv[i] > 1e-18)
        {
            ...
            debug << "keep domain " << i << " fmms " << fmms[offset] << " "
                  << fmms[offset + 1] << " ";
    ...
    MPI_Allreduce(
        local_sum, average_fmms, num_fields, MPI_DOUBLE, MPI_SUM, mpi_comm);
#else
    for (int f = 0; f < num_fields; f++)
    {
    ...
    if (rank == 0)
    {
        for (int f = 0; f < num_fields; f++)
        {
            std::cout << field_selection[f] << " average fmms "
                      << average_fmms[f] << "\n";
        }
    }

    // Compute the spatial anomaly metric for each domain. If metric is above
    // threshold = 0.7 the domain is anomalous, so paint all its cells 'red'
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        // This threshold is user specified, and a "hyper parameter" (fudge
        // factor)
        if (spatial_metric[i] > threshold)
        {
            // TO DO: Take whatever actions, e.g. painting all cells of this
            // domain
            triggered = true;
        }
    }
    ...
    conduit::relay::mpi::io::blueprint::save_mesh(
        *n_input, output_path, "hdf5", mpi_comm);
#else
    conduit::relay::io::blueprint::save_mesh(*n_input, output_path, "hdf5");
#endif
