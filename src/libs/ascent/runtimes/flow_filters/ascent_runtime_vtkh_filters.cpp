                                      ...
                                      conduit::Node &info)
{
    ...
    {
        info["errors"].append() =
            "Missing required numeric parameter. Contour must"
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
VTKHExternalSurfaces::VTKHExternalSurfaces() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHExternalSurfaces::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHExternalSurfaces::verify_params(const conduit::Node &params,
                                         conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void VTKHExternalSurfaces::execute()
{
    if (!input(0).check_type<DataObject>())
    {
    ...
    std::string topo_name = detail::resolve_topology(
        params(), this->name(), collection, throw_error);
    if (topo_name == "")
    {
        // this creates a data object with an invalid soource
        set_output<DataObject>(new DataObject());
...
void VTKH3Slice::execute()
{
    ...
    const float eps =
        1e-5; // ensure that the slice is always inside the data set
...
//-----------------------------------------------------------------------------
bool VTKHSlice::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    ...
    {
        res =
            check_numeric("sphere/center/x", params, info, true, true) && res;
        res =
            check_numeric("sphere/center/y", params, info, true, true) && res;
        res =
            check_numeric("sphere/center/z", params, info, true, true) && res;
        res = check_numeric("sphere/radius", params, info, true, true) && res;
    }
    else if (params.has_child("cylinder"))
    {
        res = check_numeric("cylinder/center/x", params, info, true, true) &&
              res;
        res = check_numeric("cylinder/center/y", params, info, true, true) &&
              res;
        res = check_numeric("cylinder/center/z", params, info, true, true) &&
              res;
        res =
            check_numeric("cylinder/axis/x", params, info, true, true) && res;
        res =
            check_numeric("cylinder/axis/y", params, info, true, true) && res;
        res =
            check_numeric("cylinder/axis/z", params, info, true, true) && res;
        res =
            check_numeric("cylinder/radius", params, info, true, true) && res;
    }
    ...
    {
        info["errors"].append() = "Cannot specify the plane point as both an "
                                  "offset and explicit point";
        res = false;
    }
...
//-----------------------------------------------------------------------------
VTKHAutoSliceLevels::VTKHAutoSliceLevels() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHAutoSliceLevels::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHAutoSliceLevels::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
    ...
    {
        info["errors"].append() = "AutoSliceLevels must specify number of "
                                  "slices to consider via 'levels'.";
        res = false;
    }
...
void VTKHAutoSliceLevels::execute()
{
    ...
    //   << " "; std::cerr<<std::endl; vtkm::Vec<vtkm::Float32,3> normal =
    //   slicer.GetNormal(); std::cerr << "normal: " << normal[0] << " " <<
    //   normal[1] << " " << normal[2] << std::endl; vtkm::Float32 radius =
    //   slicer.GetRadius(); std::cerr << "radius: " << radius << std::endl;
...
//-----------------------------------------------------------------------------
VTKHAddRanks::VTKHAddRanks() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHAddRanks::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHAddRanks::verify_params(const conduit::Node &params,
                                 conduit::Node &info)
{
    info.reset();
...
void VTKHAddRanks::execute()
{
    ...
    {
        bool throw_error = false;
        topo_name = detail::resolve_topology(
            params(), this->name(), collection, throw_error);
...
//-----------------------------------------------------------------------------
VTKHAddDomains::VTKHAddDomains() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHAddDomains::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHAddDomains::verify_params(const conduit::Node &params,
                                   conduit::Node &info)
{
    info.reset();
                                  ...
                                  conduit::Node &info)
{
    ...
    {
        info["errors"].append() =
            "Missing required parameter. Threshold must specify 'field', "
            "'sphere', 'cylinder', 'box', or 'plane'";
        res = false;
    }
    else
    {
        if (params.has_child("sphere"))
        {
            res = check_numeric("sphere/center/x", params, info, true, true) &&
                  res;
            res = check_numeric("sphere/center/y", params, info, true, true) &&
                  res;
            res = check_numeric("sphere/center/z", params, info, true, true) &&
                  res;
            res = check_numeric("sphere/radius", params, info, true, true) &&
                  res;
        }
        else if (params.has_child("cylinder"))
        {
            res =
                check_numeric("cylinder/center/x", params, info, true, true) &&
                res;
            res =
                check_numeric("cylinder/center/y", params, info, true, true) &&
                res;
            res =
                check_numeric("cylinder/center/z", params, info, true, true) &&
                res;
            res = check_numeric("cylinder/axis/x", params, info, true, true) &&
                  res;
            res = check_numeric("cylinder/axis/y", params, info, true, true) &&
                  res;
            res = check_numeric("cylinder/axis/z", params, info, true, true) &&
                  res;
            res = check_numeric("cylinder/radius", params, info, true, true) &&
                  res;
        }
        else if (params.has_child("box"))
        {
        ...
        {
            res = check_numeric("plane/point/x", params, info, true, true) &&
                  res;
            res = check_numeric("plane/point/y", params, info, true, true) &&
                  res;
            res = check_numeric("plane/point/z", params, info, true, true) &&
                  res;
            res = check_numeric("plane/normal/x", params, info, true, true) &&
                  res;
            res = check_numeric("plane/normal/y", params, info, true, true) &&
                  res;
            res = check_numeric("plane/normal/z", params, info, true, true) &&
                  res;
        }
        else if (params.has_child("multi_plane"))
        {
            res = check_numeric(
                      "multi_plane/point1/x", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/point1/y", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/point1/z", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal1/x", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal1/y", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal1/z", params, info, true, true) &&
                  res;

            res = check_numeric(
                      "multi_plane/point2/x", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/point2/y", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/point2/z", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal2/x", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal2/y", params, info, true, true) &&
                  res;
            res = check_numeric(
                      "multi_plane/normal2/z", params, info, true, true) &&
                  res;
        }
    }
...
void VTKHThreshold::execute()
{
    ...
    if (params().has_child("field"))
    {
        ...
        if (!collection->has_field(field_name))
        {
            bool throw_error = false;
            detail::field_error(
                field_name, this->name(), collection, throw_error);
            // this creates a data object with an invalid source
            set_output<DataObject>(new DataObject());
            return;
        }
...
bool VTKHClip::verify_params(const conduit::Node &params, conduit::Node &info)
{
    ...
    {
        info["errors"].append() =
            "Missing required parameter. Clip must specify a 'sphere', "
            "'cylinder', 'box', 'plane', or 'mulit_plane'";
        res = false;
    }
    else
    {
...
void VTKHClipWithField::execute()
{
    ...
    vtkm::Float64 clip_value =
        get_float64(params()["clip_value"], data_object);
...
void VTKHLog::execute()
{
    ...
    {
        double min_value =
            get_float64(params()["clamp_min_value"], data_object);
...
//-----------------------------------------------------------------------------
void VTKHLog10::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHLog10::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void VTKHLog2::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHLog2::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
...
void VTKHHistSampling::execute()
{
    ...
    if (params().has_path("bins"))
    {
        ...
        if (bins <= 0.f)
        {
            ASCENT_ERROR("vtkh_hist_sampling 'bins' value '"
                         << bins << "'"
                         << " must be positive");
        }
    }
...
VTKHUniformGrid::VTKHUniformGrid() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHUniformGrid::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHUniformGrid::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
...
void VTKHUniformGrid::execute()
{
    ...
    vtkm::Float64 z_extents =
        d_bounds.Z.Length() + 1; //(not cells) in each dim
...
//-----------------------------------------------------------------------------
bool VTKHStats::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
...
void VTKHProject2d::execute()
{
    ...
    if (params().has_path("dataset_bounds"))
    {
        ...
        if (num_bounds != 6)
        {
            ...
            ASCENT_ERROR("project_2d ("
                         << fpath << ")" << " only provided " << num_bounds
...
void VTKHCompositeVector::execute()
{
    ...
    if (!collection->has_field(field_name1))
    {
        bool throw_error = false;
        detail::field_error(
            field_name1, this->name(), collection, throw_error);
        // this creates a data object with an invalid soource
        set_output<DataObject>(new DataObject());
        return;
    }
    ...
    if (!collection->has_field(field_name2))
    {
        bool throw_error = false;
        detail::field_error(
            field_name2, this->name(), collection, throw_error);
        // this creates a data object with an invalid soource
        set_output<DataObject>(new DataObject());
        return;
    }
    ...
    if (params().has_path("field3"))
    {
        ...
        if (!collection->has_field(field_name3))
        {
            bool throw_error = false;
            detail::field_error(
                field_name3, this->name(), collection, throw_error);
            // this creates a data object with an invalid soource
            set_output<DataObject>(new DataObject());
            return;
        }
...
//-----------------------------------------------------------------------------
bool VTKHScale::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
...
VTKHTransform::VTKHTransform() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHTransform::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHTransform::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    ...
    */
    ...
    std::vector<std::string> modes = {
        "scale", "translate", "rotate", "reflect", "matrix"};
    ...
    if (mode_count > 1)
    {
        info["errors"].append() = "transform only supports one of: scale, "
                                  "translate, rotate, reflect, or matrix";
        res = false;
    }

    if (mode_count == 0)
    {
        info["errors"].append() = "transform requires parameters for: scale, "
                                  "translate, rotate, reflect, or matrix";
        res = false;
    }
    ...
    if (params.has_child("scale"))
    {
        ...
        if (!p_vals.has_child("x") && !p_vals.has_child("y") &&
            !p_vals.has_child("z"))
        {
            res = false;
            info["errors"].append() =
                "scale transform requires: scale/x, scale/y, and/or scale/z";
        }
    ...
    if (params.has_child("translate"))
    {
            ...
            !p_vals.has_child("z"))
        {
            res = false;
            info["errors"].append() =
                "translate transform requires: translate/x, translate/y, "
                "and/or translate/z";
        }
        res &= check_numeric("x", p_vals, info, false, true);
        res &= check_numeric("y", p_vals, info, false, true);
    ...
    if (params.has_child("rotate"))
    {
        ...
        if (p_vals.has_child("axis"))
        {
            ...
            if (!p_axis.has_child("x") && !p_axis.has_child("y") &&
                !p_axis.has_child("z"))
            {
                rotate_ok = false;
            }
        ...
        if (!rotate_ok)
        {
            res = false;
            info["errors"].append() =
                "rotate transform requires: rotate/angle and rotate/axis/x, "
                "rotate/axis/y, and/or rotate/axis/z";
        }
    }
    ...
    if (params.has_child("reflect"))
    {
        ...
        {
            res = false;
            info["errors"].append() = "reflect transform requires: reflect/x, "
                                      "reflect/y, and/or reflect/z";
        }
        res &= check_numeric("x", p_vals, info, false, true);
        res &= check_numeric("y", p_vals, info, false, true);
        res &= check_numeric("z", p_vals, info, false, true);
    ...
    if (params.has_child("matrix"))
    {
        ...
        if (res)
        {
            ...
            if (matrix_len != 16)
            {
                res = false;
                info["errors"].append() =
                    "matrix must an array with 16 entries (representing a 4x4 "
                    "transform matrix)";
            }
        }
    }
...
void VTKHTransform::execute()
{
    ...
    for (auto &topo : topo_names)
    {
        ...
        if (use_scale)
        {
            transform.SetScale(t_scale[0], t_scale[1], t_scale[2]);
        }

        if (use_translate)
        {
            transform.SetTranslation(
                t_translate[0], t_translate[1], t_translate[2]);
        }
        ...
        if (use_reflect)
        {
            transform.SetReflect(t_reflect[0], t_reflect[1], t_reflect[2]);
        }
                                          ...
                                          conduit::Node &info)
{
    ...
    {
        info["errors"].append() = "Missing required parameter. Particle "
                                  "Advection must specify seeds";
        res = false;
    }
    else
    {
        ...
        {
            info["errors"].append() = "Missing required parameter. Particle "
                                      "Advection must specify seed type";
            res = false;
        }
        else
        {
            ...
            else if (type == "box")
            {
                ...
                string sampling_type =
                    seed_params["sampling_type"].as_string();
                if (sampling_type == "uniform")
                {
                    res &=
                        check_numeric("num_seeds_x", seed_params, info, true);
                    res &=
                        check_numeric("num_seeds_y", seed_params, info, true);
                    res &=
                        check_numeric("num_seeds_z", seed_params, info, true);
                }
                else
                {
                    res &= check_numeric("num_seeds", seed_params, info, true);
            ...
            {
                info["errors"].append() =
                    "Unrecognized parameter. Particle Advection supports seed "
                    "types 'point', 'point_list', 'line', or 'box'.";
                res = false;
            }
        }
    }
...
void VTKHParticleAdvection::execute()
{
    ...
    else if (seed_type == "line")
    {
        ...
        else
        {
            ...
            std::default_random_engine generator(0);
            float zero(0), one(1);
            std::uniform_real_distribution<vtkm::FloatDefault> distribution(
                zero, one);
            for (int i = 0; i < num_seeds; ++i)
            {
    ...
    else if (seed_type == "box")
    {
        ...
        {
            ASCENT_ERROR("Particle Advection box seeds accepts either "
                         "'uniform' or 'random' as the 'sampling_type'");
        }
            ...
            if (sampling_type == "uniform")
            {
                ...
                for (int i = 0; i < num_seeds_x; ++i)
                {
                    ...
                    for (int j = 0; j < num_seeds_z; ++j)
                    {
                        double z = z_min + dz * j;
                        // std::cerr << "seed point" << ": " << x << " " <<
                        // y_min << " " << z << std::endl; std::cerr << "seed
                        // point" << ": " << x << " " << y_max << " " << z <<
                        // std::endl;
                        // std::cerr << "seed_count: " << seed_count <<
                        // std::endl;
                        seeds.push_back(
                            vtkm::Particle({x, y_min, z}, seed_count++));
                        seeds.push_back(
                            vtkm::Particle({x, y_max, z}, seed_count++));
                    }
                }
                for (int j = 0; j < num_seeds_y; ++j)
                {
                    ...
                    {
                        double z = z_min + dz * k;
                        // std::cerr << "seed point" << ": " << x_min << " " <<
                        // y << " " << z << std::endl; std::cerr << "seed
                        // point" << ": " << x_max << " " << y << " " << z <<
                        // std::endl; std::cerr << "seed_count: " << seed_count
                        // << std::endl;
                        seeds.push_back(
                            vtkm::Particle({x_min, y, z}, seed_count++));
                        seeds.push_back(
                            vtkm::Particle({x_max, y, z}, seed_count++));
                    }
                }
            }
            else // random
        ...
        {
            ASCENT_ERROR("Particle Advection box seeds accepts either "
                         "'interior' or 'boundary' as the 'sampling_space'");
        }
    }
    ...
    // float seedBBox[6];
    // seedBBox[0] = get_float32(params()["seed_bounding_box_xmin"],
    // data_object); seedBBox[1] =
    // get_float32(params()["seed_bounding_box_xmax"], data_object);
    // seedBBox[2] = get_float32(params()["seed_bounding_box_ymin"],
    // data_object); seedBBox[3] =
    // get_float32(params()["seed_bounding_box_ymax"], data_object);
    // seedBBox[4] = get_float32(params()["seed_bounding_box_zmin"],
    // data_object); seedBBox[5] =
    // get_float32(params()["seed_bounding_box_zmax"], data_object);
    ...
    //   std::cerr << "seed " << i << ": " << x << " " << y << " " << z <<
    //   std::endl; seeds.push_back(vtkm::Particle({x,y,z}, i));
    // }
        ...
        if (draw_tubes)
        {
            ...
            if (params().has_path("rendering/output_field"))
            {
                std::string output_field =
                    params()["rendering/output_field"].as_string();
            ...
            {
                double tube_value =
                    params()["rendering/tube_value"].as_float64();
                sl.SetTubeValue(tube_value);
            }
            if (params().has_path("rendering/tube_size"))
            {
                double tube_size =
                    params()["rendering/tube_size"].as_float64();
...
VTKHWarpXStreamline::VTKHWarpXStreamline() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHWarpXStreamline::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHWarpXStreamline::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void VTKHWarpXStreamline::execute()
{
    ...
    if (draw_tubes)
    {
        ...
        {
            std::string output_field =
                b_field + "_" + e_field + "_streamlines";
            sl.SetOutputField(output_field);
        }
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
VTKHVTKFileExtract::VTKHVTKFileExtract() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void VTKHVTKFileExtract::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHVTKFileExtract::verify_params(const conduit::Node &params,
                                       conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void VTKHVTKFileExtract::execute()
{
    ...
    std::string output_file_pattern =
        conduit::utils::join_path(output_files_dir, "domain_{:08d}.vtk");
    ...
    {
        vtkh_dset.GetDomain(idx, vtkm_dset, domain_id);
        local_domain_ids[idx] = domain_id;
        vtkm::io::VTKDataSetWriter writer(
            conduit_fmt::format(output_file_pattern, domain_id));
...
#ifdef ASCENT_MPI_ENABLED
    Node n_recv;
    conduit::relay::mpi::all_gather_using_schema(
        n_local_domain_ids, n_recv, mpi_comm);
    ...
    index_t idx = 0;
    for (index_t chld_idx = 0; chld_idx < n_recv.number_of_children();
         chld_idx++)
    {
        const Node &cld = n_recv.child(chld_idx);
        index_t_array cld_vals = cld.value();
        for (index_t local_idx = 0; local_idx < cld_vals.number_of_elements();
             local_idx++)
        {
    ...
    if (par_rank == 0)
    {
        ...
        else
        {
            ...
            std::string tmp;
            utils::rsplit_path(output_files_dir, output_files_dir_rel, tmp);

            std::string output_file_pattern_rel = conduit::utils::join_path(
                output_files_dir_rel, "domain_{:08d}.vtk");
    ...
    conduit::relay::mpi::max_all_reduce(n_local_err, n_global_err, mpi_comm);
...
//-----------------------------------------------------------------------------
void VTKHMIR::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool VTKHMIR::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
