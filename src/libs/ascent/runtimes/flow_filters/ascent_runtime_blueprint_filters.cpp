...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
BlueprintVerify::BlueprintVerify() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void BlueprintVerify::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool BlueprintVerify::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
    bool res = true;
    ...
    {
        info["errors"].append() =
            "Missing required string parameter 'protocol'";
    }
...
void BlueprintVerify::execute()
{
    ...
    {
        if (!conduit::blueprint::verify(protocol, *n_input, v_info))
        {
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
ConduitExtract::ConduitExtract() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ConduitExtract::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ConduitExtract::verify_params(const conduit::Node &params,
                                   conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
BlueprintPartition::BlueprintPartition() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void BlueprintPartition::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool BlueprintPartition::verify_params(const conduit::Node &params,
                                       conduit::Node &info)
{
    info.reset();
    bool res = true;
    ...
    if (params.has_child("selections"))
    {
        ...
        res &= (check_string("selections/domain_id", params, info, false) ||
                check_numeric(
                    "selections/domain_id", params, info, false, false));
...
//-----------------------------------------------------------------------------
void BlueprintPartition::execute()
{
    if (!input(0).check_type<DataObject>())
    {
    ...
    {
        conduit::blueprint::mesh::partition(*n_input, n_options, tmp);
    }
    else
    {
        conduit::blueprint::mpi::mesh::partition(
            *n_input, n_options, tmp, mpi_comm);
    }
#else
    conduit::blueprint::mesh::partition(*n_input, n_options, tmp);
#endif
...
//-----------------------------------------------------------------------------
//-----------------------------------------------------------------------------
DataBinning::DataBinning() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DataBinning::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DataBinning::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
void DataBinning::execute()
{
    ...
    for (int i = 0; i < num_axes; ++i)
    {
        ...
        {
            ASCENT_ERROR("Data Binning: axis "
                         << i << " is missing `field` parameter");
        }
...
//-----------------------------------------------------------------------------
void AddFields::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool AddFields::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void PowerOfField::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool PowerOfField::verify_params(const conduit::Node &params,
                                 conduit::Node &info)
{
    info.reset();
    bool res = true;
...
void PowerOfField::execute()
{
    ...
    DataObject *d_output = new DataObject();
    d_output->reset(n_input);
    expressions::derived_field_power_of_field(
        *n_input.get(), field, exponent, out_field);
    set_output<DataObject>(d_output);
}
