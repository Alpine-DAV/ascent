...
void ascent::runtime::filters::BFlowCompose::declare_interface(
    conduit::Node &i)
{
...
bool ascent::runtime::filters::BFlowCompose::verify_params(
...
void ascent::runtime::filters::BFlowCompose::execute()
{
    ...
    {
        ASCENT_ERROR("BabelFlow comp extract could not find "
                     "coordsets/coords/type or type is not 'uniform'");
    }
    ...
    {
        ASCENT_ERROR(
            "BabelFlow comp extract requires spacing of 1 along the X-axis");
    }
    ...
    {
        ASCENT_ERROR(
            "BabelFlow comp extract requires spacing of 1 along the Y-axis");
    }

    int32_t img_width =
        data_node["coordsets/coords/dims/i"].as_int32(); // Need -1 ??
    int32_t img_height =
        data_node["coordsets/coords/dims/j"].as_int32(); // Need -1 ??
    ...
    {
        ASCENT_ERROR("BabelFlow comp extract cannot find the given color or "
                     "depth fields");
    }
    ...
    {
        ASCENT_ERROR("BabelFlow comp extract requires element association in "
                     "pixel and zbuf fields");
    }
    ...
    if (pixel_vals_node.dtype().number_of_elements() !=
            img_width * img_height * bflow_comp::ImageData::sNUM_CHANNELS ||
        depth_vals_node.dtype().number_of_elements() != img_width * img_height)
    {
        std::cerr << "BFlowCompose: pixels_arr num elems = "
                  << pixel_vals_node.dtype().number_of_elements() << std::endl;
        std::cerr << "BFlowCompose: zbuff_arr num elems = "
                  << depth_vals_node.dtype().number_of_elements() << std::endl;
        ASCENT_ERROR("BabelFlow comp extract pixel array or zbuf array "
                     "element count problem");
    }
    ...
    input_img.image = new bflow_comp::ImageData::PixelType
        [img_width * img_height * bflow_comp::ImageData::sNUM_CHANNELS];
    input_img.zbuf =
        new bflow_comp::ImageData::PixelType[img_width * img_height];
    memcpy(input_img.image,
           pixel_data,
           pixel_vals_node.dtype().number_of_elements() *
               sizeof(bflow_comp::ImageData::PixelType));
    memcpy(input_img.zbuf,
           depth_data,
           depth_vals_node.dtype().number_of_elements() *
               sizeof(bflow_comp::ImageData::PixelType));
    ...
    CompositingType compositing_flag =
        CompositingType(p["compositing"].as_int64());
...
#ifdef BFLOW_COMP_DEBUG
    {
        ...
        img_name << "img_data_" << my_rank << "_" << input_img.rend_bounds[0]
                 << "_" << input_img.rend_bounds[1] << "_"
                 << input_img.rend_bounds[2] << "_" << input_img.rend_bounds[3]
                 << ".png";
    ...
    {
        bflow_comp::BabelCompReduce red_graph(
            input_img, image_name, my_rank, n_ranks, fanin, mpi_comm);
    ...
    {
        bflow_comp::BabelCompBinswap binswap_graph(
            input_img, image_name, my_rank, n_ranks, fanin, mpi_comm);
    ...
    case CompositingType::RADIX_K:
    {
        ...
        bflow_comp::BabelCompRadixK radixk_graph(
            input_img, image_name, my_rank, n_ranks, fanin, mpi_comm, radix_v);
