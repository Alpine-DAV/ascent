...
{
public:
    ParallelMergeTree(FunctionType *data_ptr,
                      int32_t task_id,
    ...
    static int DownSizeGhosts(std::vector<BabelFlow::Payload> &inputs,
                              std::vector<BabelFlow::Payload> &output,
                              BabelFlow::TaskId task);
    static void ComputeGhostOffsets(uint32_t low[3],
                                    uint32_t high[3],
                                    uint32_t &dnx,
                                    uint32_t &dny,
                                    uint32_t &dnz,
...
template <typename T> static inline void serialize_elem(char *&buff, T val)
{
    memcpy(buff, &val, sizeof(T));
    buff += sizeof(T);
}

template <typename T> static inline void deserialize_elem(char *&buff, T &val)
{
...
{
public:
    using StreamingStatisticPtr =
        std::shared_ptr<Statistics::StreamingStatistic>;
    ...
    StatisticsVec &getAndInitStatsVec(GlobalIndexType id)
    {
        ...
        if (stats_vec.size() == 0)
        {
            ...
            for (unsigned int j = 0; j < StatisticsMap::sRequestedStats.size();
                 ++j)
                stats_vec[j] = StreamingStatisticPtr(mStatsFac.make_statistic(
                    StatisticsMap::sRequestedStats[j]));
        }
        return stats_vec;
    }
    ...
    std::unordered_map<GlobalIndexType, StatisticsVec>::const_iterator
    beginIter() const
    {
        return mStats.begin();
    }
    ...
    {
        bool within[3] = {mLow[0] <= coords[0] && coords[0] <= mHigh[0],
    ...
    void deserialize(const BabelFlow::Payload &pl)
    {
        ...
        for (uint32_t i = 0; i < map_sz; ++i)
        {
            ...
            for (uint32_t j = 0; j < stats_vec.size(); ++j)
            {
                ...
                stats_vec[j] = StreamingStatisticPtr(mStatsFac.make_statistic(
                    Statistics::StreamingStatisticType(stat_type)));
...
public:
    ...
    BabelFlow::Payload serialize()
    {
        ...
        int32_t buff_sz =
            2 * sizeof(uint32_t) + stat_map_payl.size() + tree_payl.size();
        char *full_buff = new char[buff_sz];
    ...
    void demux(const BabelFlow::Payload &pl,
               BabelFlow::Payload &stat_pl,
               BabelFlow::Payload &tree_pl)
    {
        char *buff_ptr = pl.buffer();
...
void split_stats(const StatisticsMap &stats_map,
                 std::vector<StatisticsMap> &out_stats_map)
{
    // Got over stats map, for each feature:
    //   Translate feature label (id) to location
    //   Determine in which part of the full extent the feature location
    //   belongs Add the statistic to that part stat map
    ...
    GlobalIndexType extent[3] = {(high[0] - low[0]) + 1,
                                 (high[1] - low[1]) + 1,
                                 (high[2] - low[2]) + 1};
    int delta[3] = {0, 0, 0};
    GlobalIndexType max_extent =
        std::max(std::max(extent[0], extent[1]), extent[2]);
    ...
    {
        GlobalIndexType l[3] = {low[0] + delta[0] * i,
                                low[1] + delta[1] * i,
                                low[2] + delta[2] * i};
    ...
    // Split given statistic map according to feature locations -- belonging to
    // each one of the output extents
    ...
    for (; iter != end_iter; ++iter)
    {
        ...
        coords[1] =
            (feat_label / MergeTree::sDimension[0]) % MergeTree::sDimension[1];
        coords[2] =
            feat_label / (MergeTree::sDimension[0] * MergeTree::sDimension[1]);
...
void merge_stat_maps(StatisticsMap &merged_stats_map,
                     std::vector<BabelFlow::Payload> &inputs)
{
    ...
    for (unsigned int i = 0; i < inputs.size(); ++i)
    {
        ...
        for (; iter != end_iter; ++iter)
        {
            ...
            StatisticsMap::StatisticsVec &sv =
                merged_stats_map.getStatsVec(feat_label);
            if (sv.size() == 0)
            {
                sv = iter->second;
            }
                      ...
                      BabelFlow::TaskId task)
{
    ...
    // std::cout << "task " << task << "  boundary " <<
    // std::bitset<8>(block_boundary_type.t) << std::endl;

    LocalIndexType local_extent[3] = {(high[0] - low[0]) + 1,
                                      (high[1] - low[1]) + 1,
                                      (high[2] - low[2]) + 1};
    ...
    for (unsigned int i = 0; i < aug_t.sampleCount(); ++i)
    {
        ...
        // std::cout << "i " << i << "  " <<
        // std::bitset<8>(origin_boundary_type.t) << std::endl;
        ...
        StatisticsMap::StatisticsVec &stats_vec =
            stats_map.getAndInitStatsVec(aug_t.label(i));
...
/////
int merge_trees_algorithm(std::vector<Payload> &inputs,
                          MergeTree &join_tree,
                          TaskId task)
{
    ...
    std::unordered_map<BabelFlow::GlobalIndexType, BabelFlow::LocalIndexType>
        tree_map;
    ...
    for (MergeTree &mt : input_trees)
    {
        for (const TreeNode &node : mt.nodes())
        {
            ...
            {
                TreeNode *new_node = join_tree.addNode(
                    node.id(), node.value(), node.boundary());
                tree_map[node.id()] = new_node->index();
            }
        }

        std::map<BabelFlow::GlobalIndexType, BabelFlow::GlobalIndexType>
            edge_map;
                             ...
                             BabelFlow::TaskId task)
{
    ...
    for (int i = 0; i < inputs.size(); ++i)
        stat_tree_combined.demux(
            inputs[i], stats_input_v[i], trees_input_v[i]);
                     ...
                     BabelFlow::TaskId task)
{
    ...
    features.resize(tree_sz,
                    TopologyFileFormat::FeatureElement(
    ...
    for (BabelFlow::LocalIndexType i = 0; i < tree_sz; ++i)
    {
        ...
        features[i].addLink((node->down() == NULL) ? BabelFlow::LNULL
                                                   : node->down()->index());
    ...
    // We have merge trees in which case we always only have a single
    // represetative
    ...
    for (BabelFlow::LocalIndexType i = 0; i < tree_sz; ++i)
    {
        TreeNode *node = stat_tree_combined.mTree.getNode(i);
        StatisticsMap::StatisticsVec &sv =
            stat_tree_combined.mStatMap.getStatsVec(node->id());
...
// TopologyFileFormat::Data<BabelFlow::GlobalIndexType> idx_arr(
// merged_stats_map.mapSize() );
...
//   StatisticsMap::StatisticsVec& sv = merged_stats_map.getStatsVec(
//   feat_label ); assert( sv.size() > 0 ); stats_arr[i] = sv[0]->value();
...
void ParallelMergeTree::ComputeGhostOffsets(uint32_t low[3],
                                            uint32_t high[3],
                                            uint32_t &dnx,
                                            uint32_t &dny,
                                            uint32_t &dnz,
...
int ParallelMergeTree::DownSizeGhosts(std::vector<BabelFlow::Payload> &inputs,
                                      std::vector<BabelFlow::Payload> &output,
                                      BabelFlow::TaskId task)
{
    ...
    ParallelMergeTree::ComputeGhostOffsets(
        low, high, dnx, dny, dnz, dpx, dpy, dpz);
            ...
            for (uint32_t y = 0; y < ysize; ++y)
            {
                if (y >= dny && y < ysize - dpy)
                {
                    FunctionType *data_ptr =
                        block_data + dnx + y * xsize + z * ysize * xsize;
                    memcpy(n_block_data + offset,
                           (char *)data_ptr,
                           numx * sizeof(FunctionType));
    ...
    output.resize(1);
    output[0] = make_local_block(
        (FunctionType *)n_block_data, n_low, n_high, threshold);
             ...
             BabelFlow::TaskId task)
{
    ...
    // printf("this is where the preprocessing supposed to happend for Task
    // %d\n", task);
    //  output = inputs;
    //  return 1;
}
...
void ParallelMergeTree::Initialize()
{
    ...
    m_preProcTaskMp = BabelFlow::ModuloMap(
        mpi_size, m_nBlocks[0] * m_nBlocks[1] * m_nBlocks[2]);
    ...
    m_gatherTaskMp =
        BabelFlow::KWayReductionTaskMap(mpi_size, &m_gatherTaskGr);
    ...
    BabelFlow::TaskGraph::registerCallback(
        0, BabelFlow::SingleTaskGraph::SINGLE_TASK_CB, pre_proc);

    BabelFlow::TaskGraph::registerCallback(
        1, KWayMerge::LOCAL_COMP_CB, local_compute);
    BabelFlow::TaskGraph::registerCallback(1, KWayMerge::JOIN_COMP_CB, join);
    BabelFlow::TaskGraph::registerCallback(
        1, KWayMerge::LOCAL_CORR_CB, local_correction);
    BabelFlow::TaskGraph::registerCallback(
        1, KWayMerge::WRITE_RES_CB, write_results);
    BabelFlow::TaskGraph::registerCallback(
        1, KWayMerge::RELAY_CB, BabelFlow::relay_message);

    BabelFlow::TaskGraph::registerCallback(
        2, BabelFlow::RadixKExchange::LEAF_TASK_CB, compute_loc_stats);
    BabelFlow::TaskGraph::registerCallback(
        2, BabelFlow::RadixKExchange::MID_TASK_CB, merge_stats);
    // BabelFlow::TaskGraph::registerCallback( 2,
    // BabelFlow::RadixKExchange::ROOT_TASK_CB, write_stats );
    BabelFlow::TaskGraph::registerCallback(
        2, BabelFlow::RadixKExchange::ROOT_TASK_CB, merge_stats);

    // BabelFlow::TaskGraph::registerCallback( 3,
    // BabelFlow::KWayReduction::LEAF_TASK_CB, BabelFlow::relay_message );
    // BabelFlow::TaskGraph::registerCallback( 3,
    // BabelFlow::KWayReduction::MID_TASK_CB,  merge_stats );
    // BabelFlow::TaskGraph::registerCallback( 3,
    // BabelFlow::KWayReduction::ROOT_TASK_CB, write_stats_topo );

    BabelFlow::TaskGraph::registerCallback(
        3, BabelFlow::KWayReduction::LEAF_TASK_CB, init_tree_stats);
    BabelFlow::TaskGraph::registerCallback(
        3, BabelFlow::KWayReduction::MID_TASK_CB, merge_tree_stats);
    BabelFlow::TaskGraph::registerCallback(
        3, BabelFlow::KWayReduction::ROOT_TASK_CB, write_stats_topo);

#ifdef BFLOW_PMT_DEBUG
    if (my_rank == 0)
    {
        m_kWayMergeGr.outputGraphHtml(
            mpi_size, &m_kWayTaskMp, "orig_pmt_gr.html");
    }
#endif

    if (StatisticsMap::sRequestedStats.size())
    {
        // m_defGraphConnector_1 = BabelFlow::DefGraphConnector(
        // &m_preProcTaskGr, 0, &m_kWayMergeGr, 1 ); m_defGraphConnector_2 =
        // BabelFlow::DefGraphConnector( &m_kWayMergeGr, 1, &m_treeStatsGr, 2
        // ); m_defGraphConnector_3 = BabelFlow::DefGraphConnector(
        // &m_treeStatsGr, 2, &m_gatherTaskGr, 3 );
        ...
        std::vector<BabelFlow::TaskGraph *> gr_vec{
            &m_preProcTaskGr, &m_kWayMergeGr, &m_treeStatsGr, &m_gatherTaskGr};
        std::vector<BabelFlow::TaskMap *> task_maps{
            &m_preProcTaskMp, &m_kWayTaskMp, &m_treeStatsMp, &m_gatherTaskMp};

        m_fullGraph =
            BabelFlow::ComposableTaskGraph(gr_vec /*, gr_connectors*/);
        m_fullTaskMap = BabelFlow::ComposableTaskMap(task_maps);
    }
    else
    {
        // m_defGraphConnector_1 = BabelFlow::DefGraphConnector(
        // &m_preProcTaskGr, 0, &m_kWayMergeGr, 1 );

        // std::vector<BabelFlow::TaskGraphConnector*> gr_connectors{
        // &m_defGraphConnector_1 };
        std::vector<BabelFlow::TaskGraph *> gr_vec{&m_preProcTaskGr,
                                                   &m_kWayMergeGr};
        std::vector<BabelFlow::TaskMap *> task_maps{&m_preProcTaskMp,
                                                    &m_kWayTaskMp};
...
void ParallelMergeTree::ExtractSegmentation(FunctionType *output_data_ptr)
{
    ...
    // Output data includes ghost cells so we have to embed the smaller
    // segmentation part in the output data. The first step is to intialize all
    // output
    std::fill(output_data_ptr,
              output_data_ptr + xsize * ysize * zsize,
              (FunctionType)BabelFlow::GNULL);

    uint32_t dnx, dny, dnz, dpx, dpy, dpz;
    ParallelMergeTree::ComputeGhostOffsets(
        m_low, m_high, dnx, dny, dnz, dpx, dpy, dpz);
    ...
    // assert( m_fullGraph.task(m_fullGraph.gId(iter->first)).callbackId() ==
    // KWayMerge::WRITE_RES_CB );
    ...
    for (uint32_t z = dnz; z < zsize - dpz; ++z)
    {
        for (uint32_t y = dny; y < ysize - dpy; ++y)
        {
            for (uint32_t x = dnx; x < xsize - dpx; ++x)
            {
                ...
                output_data_ptr[out_data_idx] =
                    (FunctionType)t.label(label_idx);
                ++label_idx;
            }
        }
    }
    ...
    : m_dataPtr(data_ptr), m_threshold(threshold), m_comm(mpi_comm)
{
    ...
    memcpy(m_dataSize,
           reinterpret_cast<const uint32_t *>(data_size),
           3 * sizeof(uint32_t));
    memcpy(m_nBlocks,
           reinterpret_cast<const uint32_t *>(n_blocks),
           3 * sizeof(uint32_t));
    memcpy(
        m_low, reinterpret_cast<const uint32_t *>(low), 3 * sizeof(uint32_t));
    memcpy(m_high,
           reinterpret_cast<const uint32_t *>(high),
           3 * sizeof(uint32_t));
    m_fanin = static_cast<uint32_t>(fanin);

    // Store the local data as a static pointer so that it could be passed
    // later to computeSegmentation function -- probably a better solution is
    // needed
    sLocalData = data_ptr;
    ...
    {
        StatisticsMap::sRequestedStats[i] =
            Statistics::StreamingStatisticType(stat_types_v[i]);
    }
}
...
void ascent::runtime::filters::BFlowPmt::execute()
{
...
#ifdef BFLOW_PMT_DEBUG // DEBUG printouts
    {
        ...
        while (itr_dnode.has_next())
        {
            ...
            // std::cout << " dnode name " <<cld_dname  << std::endl; //<< ": "
            // << cld.to_json()
            ...
            while (itr.has_next())
            {
                ...
                std::cout << "\tname " << cld_name
                          << std::endl; //<< ": " << cld.to_json()
            }

            std::cout << world_rank << ": dnode name " << cld_dname
                      << " coordtype "
                      << data_node["coordsets/coords/type"].as_string()
                      << " uniform "
                      << data_node.has_path("coordsets/coords/spacing/x")
                      << std::endl;
    ...
    {
        std::string coordSetType =
            data_node["coordsets/coords/type"].as_string();
        if (coordSetType != "uniform")
        {
            uniform_color = 0;
            // error
            // ASCENT_ERROR("BabelFlow filter currenlty only works with uniform
            // grids");
        }
        else
        {
    ...
    // std::cout << world_rank << ": " <<
    // data_node["coordsets/coords/type"].as_string() << " color " << color
    // <<std::endl;
    ...
    if (uniform_color)
    {
        ...
        MPI_Allgather(&myspacing,
                      1,
                      MPI_DOUBLE,
                      uniform_spacing.data(),
                      1,
                      MPI_DOUBLE,
                      uniform_comm);
        ...
        if (p.has_path("ugrid_select"))
            selected_spacing = *std::next(uniform_spacing.begin(),
                                          p["ugrid_select"].as_int64());
        else
            selected_spacing = *std::next(uniform_spacing.begin(), 0);

        color = fabs(myspacing - selected_spacing) < EPSILON;

        // std::cout << "Selected spacing "<< selected_spacing << " rank " <<
        // world_rank << " contributing " << color <<"\n";
    }
    ...
    conduit::DataArray<double> array_mag =
        field_node["values"].as_float64_array();

    if (color)
    {
        // std::cout << rank << ": comm size " << comm_size << " color " <<
        // color << std::endl; data_node["coordsets/coords"].print();
        // data_node["topologies"].print();

        const int ndims =
            data_node.has_path("coordsets/coords/dims/k") ? 3 : 2;
        ...
        if (data_node.has_path("coordsets/coords/spacing"))
        {
            ...
            {
                spacing[0] =
                    data_node["coordsets/coords/spacing/x"].to_float64();
                spacing[1] =
                    data_node["coordsets/coords/spacing/y"].to_float64();
                if (ndims > 2)
                    spacing[2] =
                        data_node["coordsets/coords/spacing/z"].to_float64();
            ...
            {
                spacing[0] =
                    data_node["coordsets/coords/spacing/dx"].to_float64();
                spacing[1] =
                    data_node["coordsets/coords/spacing/dy"].to_float64();
                if (ndims > 2)
                    spacing[2] =
                        data_node["coordsets/coords/spacing/dz"].to_float64();
            }
        }
        ...
        for (int i = 0; i < ndims; i++)
        {
            ...
            MPI_Allreduce(
                &high[i], &global_high[i], 1, MPI_INT, MPI_MAX, comm);
#else
            global_low[i] = low[i];
        ...
        // int32_t dn[3], dp[3];
        // dn[0] = (low[0] == data_size[0]) ? 0 :
        // ParallelMergeTree::o_ghosts[0] - ParallelMergeTree::n_ghosts[0];
        // dn[1] = (low[1] == data_size[1]) ? 0 :
        // ParallelMergeTree::o_ghosts[1] - ParallelMergeTree::n_ghosts[1];
        // dn[2] = (low[2] == data_size[2]) ? 0 :
        // ParallelMergeTree::o_ghosts[2] - ParallelMergeTree::n_ghosts[2];
        // dp[0] = (high[0] == data_size[0] - 1) ? 0 :
        // ParallelMergeTree::o_ghosts[3] - ParallelMergeTree::n_ghosts[3];
        // dp[1] = (high[1] == data_size[1] - 1) ? 0 :
        // ParallelMergeTree::o_ghosts[4] - ParallelMergeTree::n_ghosts[4];
        // dp[2] = (high[2] == data_size[2] - 1) ? 0 :
        // ParallelMergeTree::o_ghosts[5] - ParallelMergeTree::n_ghosts[5];
        ...
        // get the data handle
        FunctionType *array =
            reinterpret_cast<FunctionType *>(array_mag.data_ptr());

        // conduit::DataArray<float> array =
        // data_node[p["data_path"].as_string()].as_float32_array();

#ifdef BFLOW_PMT_DEBUG
        {
            std::stringstream ss;
            ss << "block_" << dims[0] << "_" << dims[1] << "_" << dims[2]
               << "_low_" << low[0] << "_" << low[1] << "_" << low[2]
               << ".raw";
            std::fstream fil;
            fil.open(ss.str().c_str(), std::ios::out | std::ios::binary);
            fil.write(reinterpret_cast<char *>(array),
                      (dims[0] * dims[1] * dims[2]) * sizeof(FunctionType));
            fil.close();
            // MPI_Barrier(comm);
        }
#endif
        ...
        {
            conduit::DataArray<int64_t> radices_arr =
                p["radices"].as_int64_array();
            radix_v.resize(radices_arr.number_of_elements());
            for (uint32_t i = 0; i < radix_v.size(); ++i)
                radix_v[i] = (uint32_t)radices_arr[i];
        }
        ...
        {
            assert(!gen_field);
            conduit::DataArray<int64_t> stat_types_arr =
                p["stream_stat_types"].as_int64_array();
            stream_stat_v.resize(stat_types_arr.number_of_elements());
            for (uint32_t i = 0; i < stream_stat_v.size(); ++i)
                stream_stat_v[i] = (uint32_t)stat_types_arr[i];
        }
        ...
        // Reduce all of the local sums into the global sum
        {
            ...
            ofs << "origin " << origin[0] << " " << origin[1] << " "
                << origin[2] << std::endl;
            ofs << "spacing " << spacing[0] << " " << spacing[1] << " "
                << spacing[2] << std::endl;
            ofs << "dims " << dims[0] << " " << dims[1] << " " << dims[2]
                << std::endl;
            ofs << "low " << low[0] << " " << low[1] << " " << low[2]
                << std::endl;
            ofs << "high " << high[0] << " " << high[1] << " " << high[2]
                << std::endl;

            uint32_t dnx, dny, dnz, dpx, dpy, dpz;
            uint32_t loc_low[3] = {
                (uint32_t)low[0], (uint32_t)low[1], (uint32_t)low[2]};
            uint32_t loc_high[3] = {
                (uint32_t)high[0], (uint32_t)high[1], (uint32_t)high[2]};
            ParallelMergeTree::ComputeGhostOffsets(
                loc_low, loc_high, dnx, dny, dnz, dpx, dpy, dpz);
            ...
            if (rank == 0)
            {
                ofs << "*data_size " << data_size[0] << " " << data_size[1]
                    << " " << data_size[2] << std::endl;
                ofs << "*global_low " << global_low[0] << " " << global_low[1]
                    << " " << global_low[2] << std::endl;
                ofs << "*global_high " << global_high[0] << " "
                    << global_high[1] << " " << global_high[2] << std::endl;
                ofs << "*n_blocks " << n_blocks[0] << " " << n_blocks[1] << " "
                    << n_blocks[2] << std::endl;
            }
        ...
        if (gen_field)
        {
            ...
            data_node["fields/segment/association"] =
                field_node["association"].as_string();
            data_node["fields/segment/topology"] =
                field_node["topology"].as_string();

            // New field data
            std::vector<FunctionType> seg_data(dims[0] * dims[1] * dims[2],
                                               0.f);
            ...
            {
                std::stringstream ss;
                ss << "segment_data_" << rank << "_" << dims[0] << "_"
                   << dims[1] << "_" << dims[2] << "_low_" << low[0] << "_"
                   << low[1] << "_" << low[2] << ".raw";
                std::ofstream bofs(ss.str(), std::ios::out | std::ios::binary);
                bofs.write(reinterpret_cast<char *>(seg_data.data()),
                           dims[0] * dims[1] * dims[2] * sizeof(FunctionType));
                bofs.close();
            }
#endif
            ...
            //   std::cout << "BP with new field verify -- successful" <<
            //   std::endl;
            // else
            //   std::cout << "BP with new field verify -- failed" <<
            //   std::endl;
    ...
    else
    {
        ...
        if (gen_field)
        {
            ...
            std::vector<FunctionType> seg_data(array_mag.number_of_elements(),
                                               (FunctionType)BabelFlow::GNULL);
...
bool ascent::runtime::filters::BFlowPmt::verify_params(
