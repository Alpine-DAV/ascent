...
{
    return b1.m_ranges[0].min() == b2.m_ranges[0].min() &&
...
std::string dray_color_table_surprises(const conduit::Node &color_table)
{
    ...
    if (color_table.has_path("control_points"))
    {
        ...
        if (control_points_node.dtype().is_list())
        {
            ...
            const conduit::Node &control_points =
                color_table["control_points"];
...
std::string dray_load_balance_surprises(const conduit::Node &load_balance)
{
    std::string surprises;
...
dray::Vec<float, 3> planes(const conduit::Node &params,
                           const dray::AABB<3> bounds)
{
    ...
    if (params.has_path("x_offset"))
    {
        ...
        point[0] =
            bounds.m_ranges[0].min() + t * (bounds.m_ranges[0].length());
    }
    ...
    if (params.has_path("y_offset"))
    {
        ...
        t = std::max(0.f + eps, std::min(1.f - eps, t));
        point[1] =
            bounds.m_ranges[1].min() + t * (bounds.m_ranges[1].length());
    }
    ...
    if (params.has_path("z_offset"))
    {
        ...
        t = std::max(0.f + eps, std::min(1.f - eps, t));
        point[2] =
            bounds.m_ranges[2].min() + t * (bounds.m_ranges[2].length());
    }

    return point;
}

void parse_plane(const conduit::Node &plane, dray::PlaneDetector &plane_d)
{
...
void parse_camera(const conduit::Node camera_node, dray::Camera &camera)
{
    typedef dray::Vec<float, 3> Vec3f;
    //
...
dray::ColorTable parse_color_table(const conduit::Node &color_table_node)
{
    // default name
    ...
    if (color_table_node.has_child("name"))
    {
        ...
        else
        {
            ...
            ASCENT_INFO("Invalid color table name '"
                        << name << "'. Defaulting to " << color_map_name
                        << ". known names: " << ss.str());
        }
    }
    ...
    {
        const Node &control_points_node =
            color_table_node.fetch("control_points");
        ...
        if (control_points_node.dtype().is_list())
        {
            ...
            while (itr.has_next())
            {
                ...
                if (!peg.has_child("position"))
                {
                    ...
                    ASCENT_WARN(
                        "Color map control point must have a position");
                }
        ...
        else if (control_points_node.dtype().is_object())
        {
            ...
            float64_array pos_vals =
                control_points_node.fetch("position").value();
            ...
            {
                ASCENT_ERROR(
                    "Color map color channels should all be of the same size");
            }

            for (index_t i = 0; i < r_vals.number_of_elements(); i++)
            {
                dray::Vec<dray::float32, 3> ecolor(
            ...
            if (control_points_node.has_child("a"))
            {
                ...
                if (pos_vals.number_of_elements() !=
                    alpha_vals.number_of_elements())
                {
                    ASCENT_ERROR("Color map alpha channel should have same "
                                 "size as color channels");
                }

                for (index_t i = 0; i < alpha_vals.number_of_elements(); i++)
                {
                    color_table.add_alpha(
                        pos_vals[i],
                        std::min(1.f, std::max(alpha_vals[i], 0.f)));
                }
            }
        }
    }
...
public:
    DrayCinemaManager(
        ...
        : m_bounds(bounds), m_phi(phi), m_theta(theta),
          m_image_name(image_name), m_time(0.f),
          m_additional_params(additional_params)
    {
        ...
        for (const auto &param : m_additional_params)
        {
            ...
            {
                ASCENT_ERROR(
                    "Additional cinema parameter must have at least 1 value");
            }
        }
        m_csv += "time,FILE\n";
    ...
    DrayCinemaManager() : m_phi(0), m_theta(0)
    {
                      ...
                      const conduit::Node &n_camera)
    {
        ...
        if (m_additional_params.size() == 1)
        {
            ...
            for (const auto &param : m_additional_params)
            {
                ...
                for (int p = 0; p < param_size; ++p)
                {
                    ...
                    std::string p_string =
                        get_string(param.second[p], precision);
                    for (int i = 0; i < num_renders; ++i)
                    {
                        dray::Camera camera = m_cameras[i];
                        camera.set_zoom(zoom);
                        cameras.push_back(camera);
                        // image names do not include the additional parameter
                        // so we add it here
                        std::string image_name =
                            p_string + "_" + m_image_names[i];
                        image_name = conduit::utils::join_file_path(
                            m_image_path, image_name);
        ...
        else
        {
            for (int i = 0; i < num_renders; ++i)
            {
                ...
                camera.set_zoom(zoom);
                cameras.push_back(camera);
                std::string image_name = conduit::utils::join_file_path(
                    m_image_path, m_image_names[i]);
                image_names.push_back(image_name);
            }
        }
    }
    ...
    void write_metadata()
    {
        ...
        for (const auto &param : m_additional_params)
        {
            ...
            {
                const int precision = 2;
                add_param["values"].append().set(
                    get_string(param.second[i], precision));
            }
                ...
                if (m_additional_params.size() > 0)
                {
                    ...
                    for (const auto &param : m_additional_params)
                    {
                        ...
                        for (int i = 0; i < param_size; ++i)
                        {
                            ...
                            csv << current_time << ",";
                            csv << param_val << ",";
                            csv << current_time << "/" << param_val << "_"
                                << phi << "_" << theta << "_" << m_image_name
                                << ".png\n";
                        }
                    }
                }
...
public:
            ...
            std::map<std::string, std::vector<float>>())
    {
        ...
        {
            ASCENT_ERROR(
                "Creation failed: dray cinema database already exists");
        }

        m_databases.emplace(std::make_pair(
            db_name,
            DrayCinemaManager(
                bounds, phi, theta, db_name, path, additional_params)));
    }
...
void parse_params(const conduit::Node &params,
                  ...
                  std::vector<std::string> &image_names)
{
    ...
    if (params.has_path("camera"))
    {
        ...
        if (is_cinema)
        {
            ...
            if (!exists)
            {
                detail::DrayCinemaDatabases::create_db(
                    bounds, phi, theta, db_name, output_path);
            }

            detail::DrayCinemaManager &manager =
                detail::DrayCinemaDatabases::get_db(db_name);
...
//-----------------------------------------------------------------------------
DRayPseudocolor::DRayPseudocolor() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DRayPseudocolor::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayPseudocolor::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
...
void DRayPseudocolor::execute()
{
    ...
    detail::parse_params(
        params(), &faces, &meta, cameras, color_map, field_name, image_names);
...
//-----------------------------------------------------------------------------
void DRay3Slice::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRay3Slice::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
...
void DRay3Slice::execute()
{
    ...
    detail::parse_params(
        params_copy, dcol, &meta, cameras, color_map, field_name, image_names);
    ...
    if (params().has_path("sweep"))
    {
        ...
        {
            ASCENT_ERROR(
                "Dray 3slice param sweep requires string parameter 'axis'");
        }

        if (!n_sweep.has_path("count"))
        {
            ASCENT_ERROR(
                "Dray 3slice param sweep requires integer parameter 'count'");
        }
        ...
        {
            ASCENT_ERROR("Dray 3slice param 'axis' invalid value '"
                         << s_axis << "'"
                         << " Valid values are 'x', 'y', or 'z'");
        }

        // this is kinda weird but lets start here. This supports a count of 1
        // which will put the value at the center of the range. This method
        // will always exclude the min and max value, and spread the values out
        // equally. With an infinite count, then first and last values will
        // converge to the min and max of the range. My initial assumption is
        // that the extremes will likely contain the least interesting data.
    ...
    if (is_cinema)
    {
        ...
        if (!exists)
        {
            ...
            else
            {
                ...
                std::string s_axis =
                    params()["sweep/axis"].as_string() + "_offset";
                add_params[s_axis] = axis_sweep;
                detail::DrayCinemaDatabases::create_db(
                    bounds, phi, theta, db_name, output_path, add_params);
            }
        }
        ...
        if (is_sweep)
        {
            ...
            for (int i = 0; i < size; ++i)
            {
                ...
                float value = min_val + bounds.m_ranges[axis].length() *
                                            axis_sweep[sweep_idx];
...
//-----------------------------------------------------------------------------
void DRayVolume::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayVolume::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
    ...
    {
        surprises +=
            detail::dray_load_balance_surprises(params["load_balancing"]);
    }
...
void DRayVolume::execute()
{
    ...
    detail::parse_params(
        params(), dcol, &meta, cameras, color_map, field_name, image_names);
    ...
    if (params().has_path("load_balancing"))
    {
        ...
        bool enabled = true;
        if (load.has_path("enabled") && load["enabled"].as_string() == "false")
        {
            enabled = false;
        }
...
//-----------------------------------------------------------------------------
DRayReflect::DRayReflect() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DRayReflect::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayReflect::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
DRayProject2d::DRayProject2d() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DRayProject2d::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayProject2d::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
...
void DRayProject2d::execute()
{
    ...
    if (params().has_path("fields"))
    {
        ...
        {
            ASCENT_ERROR(
                "dray_project_2d  field selection list must be non-empty");
        }
        for (int i = 0; i < num_fields; ++i)
        {
            ...
            {
                ASCENT_ERROR("relay_io_save field selection list values must "
                             "be a string");
            }
...
//-----------------------------------------------------------------------------
DRayProjectColors2d::DRayProjectColors2d() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DRayProjectColors2d::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayProjectColors2d::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void DRayProjectColors2d::execute()
{
    if (!input(0).check_type<DataObject>())
    {
        ASCENT_ERROR("dray_project2d input must be a DataObject");
    ...
    {
        ASCENT_ERROR(
            "DrayProjectColors: only one camera is supported (no cinema)");
    }
...
//-----------------------------------------------------------------------------
DRayVectorComponent::DRayVectorComponent() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void DRayVectorComponent::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool DRayVectorComponent::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
...
//-----------------------------------------------------------------------------
void DRayVectorComponent::execute()
{
