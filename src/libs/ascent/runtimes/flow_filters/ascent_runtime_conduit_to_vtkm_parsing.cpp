                  ...
                  vtkm::rendering::Camera &camera)
{
    ...
    if (camera_node.has_child("2d"))
    {
        ...
        camera.SetViewRange2D(
            view_vals[0], view_vals[1], view_vals[2], view_vals[3]);
    }
    ...
    if (camera_node.has_child("xpan") || camera_node.has_child("ypan"))
    {
        vtkm::Float64 xpan = 0.;
        vtkm::Float64 ypan = 0.;
        if (camera_node.has_child("xpan"))
            xpan = camera_node["xpan"].to_float64();
        if (camera_node.has_child("ypan"))
            xpan = camera_node["ypan"].to_float64();
...
//-----------------------------------------------------------------------------
vtkm::cont::ColorTable parse_color_table(const conduit::Node &color_table_node)
{
    // default name
    ...
    if (color_table_node.has_child("control_points"))
    {
        ...
        if (control_points_node.dtype().is_list())
        {
            ...
            while (itr.has_next())
            {
                ...
                if (peg["type"].as_string() == "rgb")
                {
                    ...
                    vtkm::Vec<vtkm::Float64, 3> ecolor(
        ...
        else if (control_points_node.dtype().is_object())
        {
            ...
            {
                float64_array alpha_vals =
                    control_points_node.fetch("a").value();
                ...
                {
                    color_table.AddPointAlpha(
                        pos_vals[i],
                        std::min(1., std::max(alpha_vals[i], 0.)));
                }
            }
        }
    }
