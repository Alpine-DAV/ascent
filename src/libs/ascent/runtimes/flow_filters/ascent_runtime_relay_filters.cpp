...
//-----------------------------------------------------------------------------
void filter_topos(const conduit::Node &input,
                  conduit::Node &output,
    ...
    for (int d = 0; d < num_doms; ++d)
    {
        ...
        for (auto field_name : field_names)
        {
            if (dom.has_path("fields/" + field_name))
            {
                ...
                {
                    const std::string mopo =
                        dom[fpath + "/matset"].as_string();
                    matsets.insert(mopo);
                }
                ...
                if (!out_dom.has_path(tpath))
                {
                    ...
                    if (dom.has_path(tpath + "/grid_function"))
                    {
                        const std::string gf_name =
                            dom[tpath + "/grid_function"].as_string();
                    ...
                    if (dom.has_path(tpath + "/boundary_topology"))
                    {
                        const std::string bname =
                            dom[tpath + "/boundary_topology"].as_string();
        ...
        if (dom.has_path("nestsets"))
        {
            ...
            const std::vector<std::string> nest_names =
                dom["nestsets"].child_names();
        ...
        if (dom.has_path("matsets"))
        {
            ...
            const std::vector<std::string> matt_names =
                dom["matsets"].child_names();
        ...
        if (meta.has_path("ghost_field"))
        {
            ...
            for (int i = 0; i < num_ghosts; ++i)
            {
                    ...
                    !out_dom.has_path("fields/" + ghost))
                {
                    ...
                    const std::string ghost_topo =
                        ghost_field["topology"].as_string();
...
//-----------------------------------------------------------------------------
void post_filter_check_for_data(const conduit::Node &output)
{
    const int num_out_doms = output.number_of_children();
    bool has_data = false;
    // check to see if this resulted in any data
...
//-----------------------------------------------------------------------------
bool verify_io_params(const conduit::Node &params, conduit::Node &info)
{
    bool res = true;
    ...
    if (params.has_child("protocol"))
    {
        ...
        {
            info["errors"].append() =
                "optional entry 'protocol' must be a string";
            res = false;
        }
    ...
    if (params.has_child("num_files"))
    {
        ...
        {
            info["errors"].append() =
                "optional entry 'num_files' must be an integer";
            res = false;
        }
        else
        {
    ...
    if (params.has_child("refinement_level"))
    {
        ...
        {
            info["errors"].append() =
                "optional entry 'refinement_level' must be an integer";
            res = false;
        }
        else
        {
    ...
    if (params.has_child("hdf5_options"))
    {
        ...
        res &= check_object("compact_storage", params_hdf5_opts, info, false);

        res &= check_bool(
            "compact_storage/enabled", params_hdf5_opts, info, false);

        res &= check_numeric(
            "compact_storage/threshold", params_hdf5_opts, info, false);

        res &= check_object("chunking", params_hdf5_opts, info, false);

        res &= check_bool("chunking/enabled", params_hdf5_opts, info, false);

        res &=
            check_numeric("chunking/threshold", params_hdf5_opts, info, false);

        res &= check_numeric(
            "chunking/chunk_size", params_hdf5_opts, info, false);

        res &= check_object(
            "chunking/compression", params_hdf5_opts, info, false);

        res &= check_string(
            "chunking/compression/method", params_hdf5_opts, info, false);

        res &= check_numeric(
            "chunking/compression/level", params_hdf5_opts, info, false);
    }
#endif
                         ...
                         std::string &root_file_out)
{
    ...
    bool using_hdf5_opts =
        (file_protocol == "hdf5" && extra_opts.number_of_children() > 0);
    Node hdf5_opts_orig;
    if (using_hdf5_opts)
    {
...
#    endif
    ...
    else
    {
        ...
        conduit::relay::mpi::io::blueprint::save_mesh(
            data, path, file_protocol, opts, mpi_comm);
#else
        conduit::relay::io::blueprint::save_mesh(
            data, path, file_protocol, opts);
#endif
    }
...
//-----------------------------------------------------------------------------
RelayIOSave::RelayIOSave() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void RelayIOSave::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool RelayIOSave::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
...
void RelayIOSave::execute()
{
    ...
    if (protocol == "overlink")
    {
        ...
        if (params().has_path("topologies"))
        {
            ...
            {
                ASCENT_ERROR(
                    "relay_io_save Overlink save requires a single topology; "
        ...
        else
        {
            ...
            relay::mpi::gather_using_schema(
                local_unique_topos, global_topo_names, root, mpi_comm);
#else
            ...
            if (rank == root)
            {
                ...
                {
                    const conduit::Node &rank_topo_names =
                        global_topo_names.child(rank_id);
                ...
                {
                    error = 1;
                    std::vector<std::string> unique_topo_names =
                        unique_topos.child_names();
                    error_oss
                        << "relay_io_save Overlink save requires a single "
                           "topology; "
                           "there are "
                        << num_topos
                        << " topologies in the input mesh. "
                           "The current topologies are "
                        << std::accumulate(unique_topo_names.begin(),
                        ...
                        << "\nYou can select which topologies to save using "
                           "the "
                           "\"topologies\" parameter like in the following "
                           "example:\n"
                           "extracts[\"<extract_name>/params/"
                           "topologies\"].append() = \"<topo_name>\"";
                }
            }
    ...
    else
    {
        ...
        if (params().has_path("fields"))
        {
            ...
            {
                ASCENT_ERROR(
                    "relay_io_save field selection list must be non-empty");
            }
            for (int i = 0; i < num_fields; ++i)
            {
                ...
                {
                    ASCENT_ERROR("relay_io_save field selection list values "
                                 "must be a string");
                }
        ...
        if (params().has_path("topologies"))
        {
            ...
            {
                ASCENT_ERROR(
                    "relay_io_save topology selection list must be non-empty");
            }
            for (int i = 0; i < num_topos; ++i)
            {
                ...
                {
                    ASCENT_ERROR("relay_io_save topology selection list "
...
#if defined(ASCENT_HDF5_ENABLED)
    else if (protocol == "blueprint" || protocol == "blueprint/mesh/hdf5" ||
             protocol == "hdf5")
    {
        mesh_blueprint_save(
            selected, path, "hdf5", num_files, extra_opts, result_path);
    }
#endif
    else if (protocol == "blueprint" || protocol == "blueprint/mesh/yaml" ||
             protocol == "yaml")
    {
        mesh_blueprint_save(
            selected, path, "yaml", num_files, extra_opts, result_path);
    }
    else if (protocol == "blueprint/mesh/json" || protocol == "json")
    {
        mesh_blueprint_save(
            selected, path, "json", num_files, extra_opts, result_path);
    }
    else if (protocol == "silo" || protocol == "overlink")
    {
#ifdef CONDUIT_RELAY_IO_SILO_ENABLED
        mesh_blueprint_save(
            selected, path, protocol, num_files, extra_opts, result_path);
#else
        ASCENT_ERROR("Ascent's Conduit was not built with Silo support.");
#endif
    }
...
//-----------------------------------------------------------------------------
RelayIOLoad::RelayIOLoad() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void RelayIOLoad::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool RelayIOLoad::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    return verify_io_params(params, info);
}
...
//-----------------------------------------------------------------------------
BlueprintFlatten::BlueprintFlatten() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void BlueprintFlatten::declare_interface(Node &i)
{
    i["type_name"] = "false";
    i["port_names"].append() = "in";
    i["output_port"] = "false";
...
//-----------------------------------------------------------------------------
bool BlueprintFlatten::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;

    if (!params.has_child("path") || !params["path"].dtype().is_string())
    {
...
void BlueprintFlatten::execute()
{
    ...
    if (params().has_path("fields"))
    {
        ...
        {
            ASCENT_ERROR(
                "Blueprint flatten field selection list must be non-empty");
        }
        for (int i = 0; i < num_fields; ++i)
        {
            const conduit::Node &f = flist.child(i);
            if (!f.dtype().is_string())
            {
                ASCENT_ERROR("Blueprint flatten field selection list values "
                             "must be a string");
            }
            field_selection.push_back(f.as_string());
        }
    ...
    blueprint::mpi::mesh::flatten(selected, params(), output, mpi_comm);
#else
    blueprint::mesh::flatten(selected, params(), output);
