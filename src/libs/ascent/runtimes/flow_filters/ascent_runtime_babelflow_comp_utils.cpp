...
static inline void
compute_union(const uint32_t *a, const uint32_t *b, uint32_t *c)
{
    c[0] = std::min(a[0], b[0]);
    c[1] = std::max(a[1], b[1]);
    c[2] = std::min(a[2], b[2]);
...
static inline bool
compute_intersection(const uint32_t *a, const uint32_t *b, uint32_t *c)
{
    c[0] = std::max(a[0], b[0]);
    c[1] = std::min(a[1], b[1]);
    c[2] = std::max(a[2], b[2]);
...
void ImageData::writeImage(const char *filename, uint32_t *extent)
{
    ...
    for (int y = 0; y < y_extent; ++y)
    {
        for (int x = 0; x < x_extent; ++x)
        {
            ...
            if ((x + extent[0] < rend_bounds[0]) ||
                (x + extent[0] > rend_bounds[1]) ||
                (y + extent[2] < rend_bounds[2]) ||
                (y + extent[2] > rend_bounds[3]))
            {
                continue;
            }

            uint32_t myidx = (x + extent[0] - rend_bounds[0]) +
                             (y + extent[2] - rend_bounds[2]) * x_size;
            uint32_t myimgidx = myidx * ImageData::sNUM_CHANNELS;

            pixel_buff[imgidx + 3] = ImageData::sOPAQUE;
            memcpy(pixel_buff + imgidx,
                   image + myimgidx,
                   ImageData::sNUM_CHANNELS * sizeof(PixelType));
        }
    }
...
void ImageData::writeDepth(const char *filename, uint32_t *extent)
{
    ...
    for (int y = 0; y < y_extent; ++y)
    {
        for (int x = 0; x < x_extent; ++x)
        {
            ...
            PixelType pval =
                (zbuf[myidx] - min_depth) / (max_depth - min_depth);
...
{
    uint32_t zsize = (rend_bounds[1] - rend_bounds[0] + 1) *
                     (rend_bounds[3] - rend_bounds[2] + 1) * sizeof(PixelType);
                     ...
                     bool union_box_as_extent)
{
    ...
    if (union_box_as_extent)
    {
        ...
        std::cout << "union_box: " << union_box[0] << " " << union_box[1]
                  << " " << union_box[2] << " " << union_box[3] << std::endl;
#endif
    }
    else
    {
    ...
    uint32_t split_size[2] = {x_size / (uint32_t)out_images.size(),
                              y_size / (uint32_t)out_images.size()};
    ...
    for (int i = 0; i < out_images.size(); ++i)
    {
        ...
        uint32_t zsize = (outimg.rend_bounds[1] - outimg.rend_bounds[0] + 1) *
                         (outimg.rend_bounds[3] - outimg.rend_bounds[2] + 1);
        outimg.image =
            new ImageData::PixelType[zsize * ImageData::sNUM_CHANNELS]();
        // Initialize alpha channel to 255 (fully opaque)
        if (ImageData::sNUM_CHANNELS > 3)
        {
            for (uint32_t j = 0; j < zsize; ++j)
                outimg.image[j * ImageData::sNUM_CHANNELS + 3] =
                    ImageData::sOPAQUE;
        }
        outimg.zbuf = new ImageData::PixelType[zsize];
        std::fill(outimg.zbuf,
                  outimg.zbuf + zsize,
    ...
    for (uint32_t j = 0; j < input_images.size();
         ++j) // Blend every input image
    {
        ...
        for (uint32_t i = 0; i < out_images.size(); ++i)
        {
            ...
            for (uint32_t y = bound[2]; y < bound[3] + 1; ++y)
            {
                for (uint32_t x = bound[0]; x < bound[1] + 1; ++x)
                {
                    ...
                    {
                        // std::cout << "split_and_blend, shouldn't be here, x
                        // = " << x << ", y = " << y << std::endl;
                        continue;
                    }
                    ...
                    if (skip_z_check || outimg.zbuf[myidx] > inimg.zbuf[idx])
                    {
                        ...
                        memcpy(outimg.image + imgmyidx,
                               inimg.image + imgidx,
                               ImageData::sNUM_CHANNELS *
                                   sizeof(ImageData::PixelType));
                    }
                }
            }
        }
                    ...
                    bool union_box_as_extent)
{
    ...
    {
        bflow_comp::compute_union(
            union_box, in_images[i].rend_bounds, union_box);
    }

    bflow_comp::split_and_blend(in_images,
                                out_images,
                                union_box,
                                flip_split_side,
                                skip_z_check,
                      ...
                      bool union_box_as_extent)
{
    ...
    // The inputs are image fragments from child nodes -- read all into one
    // array
    ...
    bflow_comp::compose_images(images,
                               out_images,
                               task_id,
                               flip_split_side,
                               skip_z_check,
    ...
    for (uint32_t i = 0; i < out_images.size(); ++i)
    {
...
#ifdef BFLOW_COMP_UTIL_DEBUG // DEBUG -- write local rendering result to a file
        {
            ...
            out_images[i].writeImage(filename.str().c_str(),
                                     out_images[i].rend_bounds);

            std::stringstream filename1;
            filename1 << "comp_dep_" << task_id << "_" << i << ".png";
            out_images[i].writeDepth(filename1.str().c_str(),
                                     out_images[i].rend_bounds);
        }
#endif
...
int write_results_red(std::vector<BabelFlow::Payload> &inputs,
                      ...
                      BabelFlow::TaskId task_id)
{
    // Writing results is the root of the composition reduce tree, so now we
    // have to do one last composition step
    std::vector<BabelFlow::Payload> comp_outputs(1);
    composite_red(inputs,
                  comp_outputs,
                  task_id); // will also free the memory for input buffers
                          ...
                          BabelFlow::TaskId task_id)
{
    ...
    std::stringstream filename;
    filename << BabelGraphWrapper::sIMAGE_NAME << "_" << x << "_" << y
             << ".png";
                         ...
                         BabelFlow::TaskId task_id)
{
    ...
    // Compositing -- don't flip the split direction (as in traditional
    // k-radix) and skip the zbuf check (only one input)
...
void BabelCompReduce::Initialize()
{
    ...
    m_reduceTaskMp =
        BabelFlow::KWayReductionTaskMap(m_nRanks, &m_reduceTaskGr);

    m_preProcTaskGr.setGraphId(0);
    m_reduceTaskGr.setGraphId(1);

    // m_defGraphConnector = BabelFlow::DefGraphConnector( &m_preProcTaskGr, 0,
    // &m_reduceTaskGr, 1 );
    ...
    BabelFlow::TaskGraph::registerCallback(
        0, BabelFlow::SingleTaskGraph::SINGLE_TASK_CB, bflow_comp::pre_proc);
    BabelFlow::TaskGraph::registerCallback(
        1,
        BabelFlow::KWayReduction::LEAF_TASK_CB,
        bflow_comp::volume_render_red);
    BabelFlow::TaskGraph::registerCallback(
        1, BabelFlow::KWayReduction::MID_TASK_CB, bflow_comp::composite_red);
    BabelFlow::TaskGraph::registerCallback(
        1,
        BabelFlow::KWayReduction::ROOT_TASK_CB,
        bflow_comp::write_results_red);
...
void BabelCompBinswap::Initialize()
{
    ...
    BabelFlow::TaskGraph::registerCallback(
        0, BabelFlow::SingleTaskGraph::SINGLE_TASK_CB, bflow_comp::pre_proc);
    BabelFlow::TaskGraph::registerCallback(1,
                                           BabelFlow::BinarySwap::LEAF_TASK_CB,
                                           bflow_comp::volume_render_binswap);
    BabelFlow::TaskGraph::registerCallback(
        1, BabelFlow::BinarySwap::MID_TASK_CB, bflow_comp::composite_binswap);
    BabelFlow::TaskGraph::registerCallback(1,
                                           BabelFlow::BinarySwap::ROOT_TASK_CB,
                                           bflow_comp::write_results_binswap);
...
void BabelCompRadixK::InitGatherGraph()
{
    ...
    m_gatherTaskMp =
        BabelFlow::KWayReductionTaskMap(m_nRanks, &m_gatherTaskGr);
}
...
void BabelCompRadixK::Initialize()
{
    ...
    // m_defGraphConnector = BabelFlow::DefGraphConnector( &m_radixkGr, 0,
    // &m_gatherTaskGr, 1 );
    ...
    BabelFlow::TaskGraph::registerCallback(
        ...
        bflow_comp::composite_radixk);
    BabelFlow::TaskGraph::registerCallback(
        1, BabelFlow::KWayReduction::LEAF_TASK_CB, BabelFlow::relay_message);
    BabelFlow::TaskGraph::registerCallback(
        1,
    ...
    if (m_rankId == 0)
    {
        m_preProcTaskGr.outputGraphHtml(
            m_nRanks, &m_preProcTaskMp, "pre-proc.html");
        m_radixkGr.outputGraphHtml(m_nRanks, &m_radixkMp, "radixk.html");
        m_gatherTaskGr.outputGraphHtml(
            m_nRanks, &m_gatherTaskMp, "gather-task.html");
        m_radGatherGraph.outputGraphHtml(
            m_nRanks, &m_radGatherTaskMap, "radixk-gather.html");
    }
#endif
