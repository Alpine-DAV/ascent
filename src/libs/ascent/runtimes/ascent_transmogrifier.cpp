...
void Transmogrifier::to_poly(conduit::Node &doms, conduit::Node &to_vtkh)
{
    ...
    for (int i = 0; i < num_domains; i++)
    {
        ...
        // we know it must have a child "topologies" b/c otherwise it is not
        // valid blueprint
        conduit::NodeConstIterator itr = dom["topologies"].children();
        while (itr.has_next())
        {
        ...
        for (int i = 0; i < poly_topos.size(); i++)
        {
            ...
            //
            fields_tmp.rename_child("original_vertex_ids",
                                    poly_topos[i] + "_original_vertex_ids");
            fields_tmp.rename_child("original_element_ids",
                                    poly_topos[i] + "_original_element_ids");
