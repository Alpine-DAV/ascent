...
template <typename T> const T *GetNodePointer(const conduit::Node &node);

template <> const float64 *GetNodePointer<float64>(const conduit::Node &node)
{
    return node.as_float64_ptr();
}

template <> const float32 *GetNodePointer<float32>(const conduit::Node &node)
{
    return node.as_float32_ptr();
}

template <typename T>
void CopyArray(vtkm::cont::ArrayHandle<T> &vtkm_handle,
               const T *vals_ptr,
               const int size,
               bool zero_copy)
{
...
template <typename T>
void BlueprintIndexArrayToVTKmIdArray(const conduit::Node &n,
                                      bool zero_copy,
                                      vtkm::cont::ArrayHandle<T> &vtkm_handle)
{
    ...
    {
        // unsupported!
        ASCENT_ERROR("BlueprintIndexArrayToVTKmIdArray does not support "
                     "2-byte index arrays");
    }
                            ...
                            bool zero_copy)
{
    ...
    // bool is_interleaved =
    // blueprint::mcarray::is_interleaved(n_coords["values"]);
    ...
    else
    {
        ...
        const T *x_verts_ptr = n_coords["values/x"].value();
        vtkm::cont::ArrayHandle<T> x_source_array =
            vtkm::cont::make_ArrayHandle<T>(
                x_verts_ptr, x_verts_expanded, copy);
    ...
    else
    {
        ...
        const T *y_verts_ptr = n_coords["values/y"].value();
        vtkm::cont::ArrayHandle<T> y_source_array =
            vtkm::cont::make_ArrayHandle<T>(
                y_verts_ptr, y_verts_expanded, copy);
    ...
    else
    {
        ...
        const T *z_verts_ptr = n_coords["values/z"].value();
        vtkm::cont::ArrayHandle<T> z_source_array =
            vtkm::cont::make_ArrayHandle<T>(
                z_verts_ptr, z_verts_expanded, copy);
                           ...
                           bool zero_copy)
{
    ...
    vtkm::cont::Field::Association vtkm_assoc =
    ...
    {
        ASCENT_ERROR("Cannot add field association "
                     << assoc_str << " from field " << field_name);
    }
    ...
    if (element_stride == 1)
    {
        field = vtkm::cont::make_Field(
            field_name, vtkm_assoc, values_ptr, num_vals, copy);
    }
    else
    {
        //
        ...
        vtkm::cont::ArrayHandle<T> source_array =
            vtkm::cont::make_ArrayHandle(values_ptr, num_vals_expanded, copy);
        vtkm::cont::ArrayHandleStride<T> stride_array(
            source_array, num_vals, element_stride, 0);
        field = vtkm::cont::Field(field_name, vtkm_assoc, stride_array);
    }
                   ...
                   bool zero_copy)
{
    ...
    if (dims != 2 && dims != 3)
    {
        ASCENT_ERROR("Extract vector: only 2 and 3 dims supported given "
                     << dims);
    }
    ...
    if (dims == 2)
    {
        ...
        auto composite = make_ArrayHandleSOA(x_handle, y_handle);
    ...
    if (dims == 3)
    {
        ...
        auto composite = make_ArrayHandleSOA(x_handle, y_handle, z_handle);
                     ...
                     vtkm::cont::Field &vfs)
{
    ...
    vtkm::cont::Field::Association vtkm_assoc_c =
    ...
    length = vtkm::cont::make_Field(
        length_name, vtkm_assoc_c, length_ptr, neles, copy);

    const T *offsets_ptr = v_offsets.data();

    offsets = vtkm::cont::make_Field(
        offsets_name, vtkm_assoc_c, offsets_ptr, neles, copy);
    // calc vfs and mat ids
    vtkm::cont::Field::Association vtkm_assoc_w =
    ...
    if (node.has_child("element_ids"))
    {
        ...
        for (index_t i = 0; i < num_materials; ++i)
        {
            ...
            for (index_t j = 0; j < num_vals; ++j)
            {
                ...
                v_ids[offset + length] = i + 1; // material ids can't start at
                                                // 0
            }
        }
    }
    ...
    ids =
        vtkm::cont::make_Field(ids_name, vtkm_assoc_w, ids_ptr, l_total, copy);

    const S *vfs_ptr = v_vfs.data();

    vfs =
        vtkm::cont::make_Field(vfs_name, vtkm_assoc_w, vfs_ptr, l_total, copy);
}
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        for (int t = 0; t < topo_names.size(); ++t)
        {
            ...
            vtkm::cont::DataSet *dset =
                BlueprintToVTKmDataSet(dom, zero_copy, topo_name);
                                        ...
                                        bool zero_copy)
{
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        vtkm::cont::DataSet *dset =
            VTKHDataAdapter::BlueprintToVTKmDataSet(dom, zero_copy, topo_name);
        int domain_id = dom["state/domain_id"].to_int();
                                        ...
                                        const std::string &topo_name_str)
{
    ...
    {
        result = UniformBlueprintToVTKmDataSet(
            coords_name, n_coords, topo_name, n_topo, neles, nverts);
    }
    ...
    if (node.has_child("fields"))
    {
        ...
        while (itr.has_next())
        {
            ...
            {
                ASCENT_INFO("skipping field " << field_name << " with "
                                              << num_children << " comps");
            }
        }
    }
    ...
    if (node.has_child("matsets"))
    {
        ...
        while (itr.has_next())
        {
            ...
            AddMatSets(
                matset_name, n_matset, topo_name, neles, result, zero_copy);
        }
    }
    return result;
}
...
public:
        ...
        int &neles)
    {
        ...
        if (conn_size < indices)
            ASCENT_ERROR("Connectivity array size "
                         << conn_size << " must be at least size " << indices);
        if (conn_size % indices != 0)
            ASCENT_ERROR("Connectivity array size "
                         << conn_size << " be evenly divided by indices size"
                         << indices);
...
vtkm::cont::DataSet *VTKHDataAdapter::UniformBlueprintToVTKmDataSet(
    ...
    int &nverts)                    // output, number of verts
{
    ...
    vtkm::Vec<vtkm::Float32, 3> origin(origin_x, origin_y, origin_z);

    vtkm::Vec<vtkm::Float32, 3> spacing(spacing_x, spacing_y, spacing_z);

    vtkm::Id3 dims(dims_i, dims_j, dims_k);

    // todo, use actually coordset and topo names?
    result->AddCoordinateSystem(vtkm::cont::CoordinateSystem(
        coords_name.c_str(), dims, origin, spacing));
...
vtkm::cont::DataSet *VTKHDataAdapter::RectilinearBlueprintToVTKmDataSet(
    const std::string &coords_name, // input string with coordset name
    ...
    bool zero_copy)               // attempt to zero copy
{
    ...
    if (zero_copy)
    {
        x_coords_handle = vtkm::cont::make_ArrayHandle(
            x_coords_ptr, x_npts, vtkm::CopyFlag::Off);
        y_coords_handle = vtkm::cont::make_ArrayHandle(
            y_coords_ptr, y_npts, vtkm::CopyFlag::Off);
    }
    else
    {
    ...
    if (ndims == 3)
    {
        if (zero_copy)
        {
            z_coords_handle = vtkm::cont::make_ArrayHandle(
                z_coords_ptr, z_npts, vtkm::CopyFlag::Off);
        }
        else
        {
    ...
    static_assert(
        std::is_same<vtkm::FloatDefault, double>::value,
    ...
    coords = vtkm::cont::make_ArrayHandleCartesianProduct(
        x_coords_handle, y_coords_handle, z_coords_handle);
    ...
    {
        vtkm::cont::CellSetStructured<2> cell_set;
        cell_set.SetPointDimensions(vtkm::make_Vec(x_npts, y_npts));
        vtkm::Id2 origin2(topo_origin[0], topo_origin[1]);
        cell_set.SetGlobalPointIndexStart(origin2);
        result->SetCellSet(cell_set);
    }
    ...
    {
        vtkm::cont::CellSetStructured<3> cell_set;
        cell_set.SetPointDimensions(vtkm::make_Vec(x_npts, y_npts, z_npts));
        cell_set.SetGlobalPointIndexStart(topo_origin);
        result->SetCellSet(cell_set);
    }
...
vtkm::cont::DataSet *VTKHDataAdapter::StructuredBlueprintToVTKmDataSet(
    const std::string &coords_name, // input string with coordset name
    const Node &n_coords, // input mesh bp coordset (assumed rectilinear)
    const std::string &topo_name, // input string with topo name
    const Node &n_topo,           // input mesh bp topo
    ...
    bool zero_copy)               // attempt to zero copy
{
    ...
    else
    {
        ...
        {
            vtkm::cont::CellSetStructured<2> cell_set;
            cell_set.SetPointDimensions(
                vtkm::make_Vec(x_elems + 1, y_elems + 1));
            vtkm::Id2 origin2(topo_origin[0], topo_origin[1]);
            cell_set.SetGlobalPointIndexStart(origin2);
            result->SetCellSet(cell_set);
            neles = x_elems * y_elems;
        ...
        else
        {
            ...
            vtkm::cont::CellSetStructured<3> cell_set;
            cell_set.SetPointDimensions(
                vtkm::make_Vec(x_elems + 1, y_elems + 1, z_elems + 1));
            cell_set.SetGlobalPointIndexStart(topo_origin);
            result->SetCellSet(cell_set);
            neles = x_elems * y_elems * z_elems;
        }
...
vtkm::cont::DataSet *VTKHDataAdapter::PointsImplicitBlueprintToVTKmDataSet(
    const std::string &coords_name, // input string with coordset name
...
//-----------------------------------------------------------------------------
vtkm::cont::DataSet *VTKHDataAdapter::UnstructuredBlueprintToVTKmDataSet(
    const std::string &coords_name, // input string with coordset name
    const Node &n_coords, // input mesh bp coordset (assumed unstructured)
    const std::string &topo_name, // input string with topo name
    const Node &n_topo,           // input mesh bp topo
    ...
    bool zero_copy)               // attempt to zero copy
{
    ...
    if (n_coords["values/x"].dtype().is_float64())
    {
        ...
        {
            coords =
                detail::GetExplicitCoordinateSystem<float64>(n_coords,
                                                             coords_name,
                                                             ndims,
                                                             x_element_stride,
                                                             y_element_stride,
    ...
    else if (n_coords["values/x"].dtype().is_float32())
    {
        ...
        {
            coords =
                detail::GetExplicitCoordinateSystem<float32>(n_coords,
                                                             coords_name,
                                                             ndims,
                                                             x_element_stride,
                                                             y_element_stride,
    ...
    if (ele_shape == "mixed")
    {
        ...
        if (!CheckShapeMapVsVTKmShapeIds(n_topo_eles["shape_map"]))
        {
            ...
            ASCENT_ERROR(
                ...
                << n_topo_eles["shape_map"].to_yaml() << std::endl
                << "Supported Shape Map:" << std::endl
                << ref_map.to_yaml());
        }

        index_t num_ids =
            n_topo_eles["connectivity"].dtype().number_of_elements();
        ...
        detail::BlueprintIndexArrayToVTKmIdArray(
            n_topo_eles["connectivity"], zero_copy, vtkm_conn);

        // shapes
        vtkm::cont::ArrayHandle<vtkm::UInt8> vtkm_shapes;
        detail::BlueprintIndexArrayToVTKmIdArray(
            n_topo_eles["shapes"], zero_copy, vtkm_shapes);

        // offsets
        vtkm::cont::ArrayHandle<vtkm::Id> vtkm_offsets;
        detail::BlueprintIndexArrayToVTKmIdArray(
            n_topo_eles["offsets"], zero_copy, vtkm_offsets);
        ...
        vtkm::cont::ArrayHandle<vtkm::Id>::WritePortalType
            vtkm_offsets_full_wp = vtkm_offsets_full.WritePortal();
        vtkm::cont::ArrayHandle<vtkm::Id>::ReadPortalType vtkm_offsets_rp =
            vtkm_offsets.ReadPortal();
...
void VTKHDataAdapter::AddField(const std::string &field_name,
                               const Node &n_field,
                               const std::string &topo_name,
                               int neles,
                               int nverts,
                               ...
                               bool zero_copy) // attempt to zero copy
{
    ...
    {
        ASCENT_INFO("VTKm conversion does not support field assoc "
                    << assoc_str << ". Skipping");
        return;
    }
    ...
    const Node &n_vals =
        is_values ? n_field["values"] : n_field["values"].child(0);
    ...
    if (assoc_str == "element" && neles != num_vals)
    {
        ...
        {
            ASCENT_INFO("Field '"
                        << field_name << "' (topology: '" << topo_name
                        << "') number of values " << num_vals
                        << " does not match the number of elements " << neles
                        << ". Skipping");
        }
        return;
    }
    ...
    try
    {
        ...
        // vtk-m can stride as long as the strides are a multiple of the native
        // stride
        ...
        else if (n_vals.dtype().is_uint8())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::uint8> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
            ...
            {
                dset->AddField(
                    vtkm::cont::Field(field_name.c_str(),
        ...
        else if (n_vals.dtype().is_uint16())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::uint16> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_uint32())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::uint32> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_uint64())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::uint64> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_int8())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::int8> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_int16())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::int16> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_int32())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::int32> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        else if (n_vals.dtype().is_int64())
        {
            ...
            vtkm::cont::ArrayHandle<conduit::int64> input_arr =
                vtkm::cont::make_ArrayHandle(
                    input, num_vals, vtkm::CopyFlag::Off);
        ...
        //   Execution failed with vtkm: Could not find appropriate cast for
        //   array in CastAndCall. Array: valueType=x
        //   storageType=N4vtkm4cont15StorageTagBasicE 27 values occupying 216
        //   bytes [0 1 2 ... 24 25 26] TypeList: N4vtkm4ListIJfdEEE
        // ***********************************************************************
        // ***********************************************************************
        ...
        // vtk-m cant support zero copy for this layout or was not compiled to
        // expose this datatype use float64 by default
        if (!supported_type)
        {

            // std::cout << "WE ARE IN UNSUPPORTED DATA TYPE: "
            //           << n_vals.dtype().name() << std::endl;
            //  convert to float64, we use this as a compromise to cover the
            //  widest range
            vtkm::cont::ArrayHandle<vtkm::Float64> vtkm_arr;
            vtkm_arr.Allocate(num_vals);
...
void VTKHDataAdapter::AddVectorField(const std::string &field_name,
                                     const Node &n_field,
                                     const std::string &topo_name,
                                     int neles,
                                     int nverts,
        ...
        if (interleaved)
        {
            if (dims == 3)
            {
                ...
                if (u.dtype().is_float32())
                {
                    ...
                    const Vec3f32 *vec_ptr =
                        reinterpret_cast<const Vec3f32 *>(u.as_float32_ptr());
                ...
                else if (u.dtype().is_float64())
                {
                    ...
                    const Vec3f64 *vec_ptr =
                        reinterpret_cast<const Vec3f64 *>(u.as_float64_ptr());
            ...
            else if (dims == 2)
            {
                ...
                if (u.dtype().is_float32())
                {
                    ...
                    const Vec2f32 *vec_ptr =
                        reinterpret_cast<const Vec2f32 *>(u.as_float32_ptr());
                ...
                else if (u.dtype().is_float64())
                {
                    ...
                    const Vec2f64 *vec_ptr =
                        reinterpret_cast<const Vec2f64 *>(u.as_float64_ptr());
...
void VTKHDataAdapter::AddMatSets(const std::string &matset_name,
                                 const Node &n_matset,
                                 const std::string &topo_name,
                                 int neles,
                                 vtkm::cont::DataSet *dset,
                                 bool zero_copy) // attempt to zero copy
{
    ...
    if (!n_matset.has_child("volume_fractions"))
        ASCENT_ERROR(
            "No volume fractions were defined for matset: " << matset_name);
        ...
        try
        {
            ...
            if (n_matset["volume_fractions"].dtype().is_float32())
            {
                ...
                if (n_material_ids.dtype().is_int32())
                {
                    ...
                    const vector<conduit::int32> vec_ids(n_ids,
                                                         n_ids + num_vals);
                    bool zeroes =
                        std::any_of(vec_ids.begin(),
                                    vec_ids.end(),
                                    [](int value) { return value <= 0; });
                    if (zeroes) // need to make a copy and increment all
                                // material ids
                    {
                        ...
                        vtkm::cont::Field field_copy =
                            detail::GetField<int32>(n_mat_ids,
                                                    ids_name,
                                                    "whole",
                                                    topo_name,
                                                    index_t(1),
                    ...
                    {
                        vtkm::cont::Field field_copy =
                            detail::GetField<int32>(n_material_ids,
                                                    ids_name,
                                                    "whole",
                                                    topo_name,
                                                    index_t(1),
                ...
                else if (n_material_ids.dtype().is_int64())
                {
                    ...
                    const vector<conduit::int64> vec_ids(n_ids,
                                                         n_ids + num_vals);
                    bool zeroes =
                        std::any_of(vec_ids.begin(),
                                    vec_ids.end(),
                                ...
                                // material ids
                    {
                        ...
                        vtkm::cont::Field field_copy =
                            detail::GetField<int64>(n_mat_ids,
                                                    ids_name,
                                                    "whole",
                                                    topo_name,
                                                    index_t(1),
                    ...
                    {
                        vtkm::cont::Field field_copy =
                            detail::GetField<int64>(n_material_ids,
                                                    ids_name,
                                                    "whole",
                                                    topo_name,
                                                    index_t(1),
                ...
                const conduit::Node &n_volume_fractions =
                    n_matset["volume_fractions"];
    ...
    else if (n_matset.has_child(
                 "element_ids")) // matset is "sparse_by_material"
    {
        int num_ids = n_matset["element_ids"].number_of_children();
        if (num_ids == 0)
        {
            ASCENT_ERROR(
                "No element ids were defined for matset: " << matset_name);
        }

        int num_materials = n_matset["volume_fractions"].number_of_children();
        if (num_materials == 0)
        {
            ASCENT_ERROR("No volume fractions were defined for matset: "
                         << matset_name);
        }

        if (num_materials != num_ids)
        {
            ASCENT_ERROR("Number of materials ("
                         << num_materials
    ...
    else // matset is "full"
    {
        ...
        if (num_vals != neles)
        {
            ASCENT_ERROR("Number of vf values "
                         << num_vals << " for material " << material_name
                         << " does not equal number of cells " << neles);
        }
        try
        {
            bool supported_type = false;
...
std::string GetBlueprintCellName(vtkm::UInt8 shape_id)
{
    std::string name;
...
inline index_t vtkm_shape_size(vtkm::Id shape_id)
{
    switch (shape_id)
    {
    // point
...
void generate_sizes_from_shapes(const conduit::Node &shapes,
                                conduit::Node &sizes)
{
...
bool VTKHDataAdapter::VTKmTopologyToBlueprint(
    conduit::Node &output,
    ...
    bool zero_copy)
{
    ...
    int topo_dims;
    bool is_structured =
        vtkh::VTKMDataSetInfo::IsStructured(data_set, topo_dims);
    ...
    if (is_uniform)
    {
        ...
        output["coordsets/" + coords_name + "/spacing/dx"] =
            (double)spacing[0];
        output["coordsets/" + coords_name + "/spacing/dy"] =
            (double)spacing[1];
        output["coordsets/" + coords_name + "/spacing/dz"] =
            (double)spacing[2];
    }
    else if (is_rectilinear)
    {
        ...
        vtkm::FloatDefault *x_ptr =
            const_cast<vtkm::FloatDefault *>(x_portal.GetArray());
        vtkm::FloatDefault *y_ptr =
            const_cast<vtkm::FloatDefault *>(y_portal.GetArray());
        vtkm::FloatDefault *z_ptr =
            const_cast<vtkm::FloatDefault *>(z_portal.GetArray());
        ...
        if (zero_copy)
        {
            output["coordsets/" + coords_name + "/values/x"].set_external(
                x_ptr, x_portal.GetNumberOfValues());
            output["coordsets/" + coords_name + "/values/y"].set_external(
                y_ptr, y_portal.GetNumberOfValues());
            output["coordsets/" + coords_name + "/values/z"].set_external(
                z_ptr, z_portal.GetNumberOfValues());
        }
        else
        {
            output["coordsets/" + coords_name + "/values/x"].set(
                x_ptr, x_portal.GetNumberOfValues());
            output["coordsets/" + coords_name + "/values/y"].set(
                y_ptr, y_portal.GetNumberOfValues());
            output["coordsets/" + coords_name + "/values/z"].set(
                z_ptr, z_portal.GetNumberOfValues());
        }
    }
    else
    {
        ...
        using Coords32 =
            vtkm::cont::ArrayHandleSOA<vtkm::Vec<vtkm::Float32, 3>>;
        using Coords64 =
            vtkm::cont::ArrayHandleSOA<vtkm::Vec<vtkm::Float64, 3>>;

        using CoordsVec32 =
            vtkm::cont::ArrayHandle<vtkm::Vec<vtkm::Float32, 3>>;
        using CoordsVec64 =
            vtkm::cont::ArrayHandle<vtkm::Vec<vtkm::Float64, 3>>;
        ...
        if (coordsHandle.CanConvert<Coords32>())
        {
            ...
            {
                output["coordsets/" + coords_name + "/values/x"].set_external(
                    vtkh::GetVTKMPointer(x_handle), point_dims[0]);
                output["coordsets/" + coords_name + "/values/y"].set_external(
                    vtkh::GetVTKMPointer(y_handle), point_dims[1]);
                output["coordsets/" + coords_name + "/values/z"].set_external(
                    vtkh::GetVTKMPointer(z_handle), point_dims[2]);
            }
            else
            {
                output["coordsets/" + coords_name + "/values/x"].set(
        ...
        else if (coordsHandle.IsType<CoordsVec32>())
        {
            ...
            vtkm::Float32 *points_ptr =
                (vtkm::Float32 *)vtkh::GetVTKMPointer(points);
        ...
        else if (coordsHandle.IsType<CoordsVec64>())
        {
            ...
            const int num_vals = points.GetNumberOfValues();
            vtkm::Float64 *points_ptr =
                (vtkm::Float64 *)vtkh::GetVTKMPointer(points);
        ...
        {
            // Ok vtkm has handed us something we don't know about, and its
            // really hard to ask vtkm to tell us what it is. Before we give
            // up, we will attempt to copy the data to a known type and copy
            // that copy. We can't avoid the double copy since conduit can't
            // take ownership and we can't seem to write to a zero copied array
            ...
            vtkm::Float64 *points_ptr =
                (vtkm::Float64 *)vtkh::GetVTKMPointer(coords_copy);
            const int byte_size = sizeof(vtkm::Float64);
        ...
        if (is_structured)
        {
            ...
            if (dyn_cells.CanConvert<Structured2D>())
            {
                ...
                output["topologies/" + topo_name + "/elements/dims/i"] =
                    (int)cell_dims[0];
                output["topologies/" + topo_name + "/elements/dims/j"] =
                    (int)cell_dims[1];
            }
            else if (dyn_cells.CanConvert<Structured3D>())
            {
                ...
                output["topologies/" + topo_name + "/elements/dims/j"] =
                    (int)cell_dims[1];
                output["topologies/" + topo_name + "/elements/dims/k"] =
                    (int)cell_dims[2];
            }
            else
            {
        ...
        else
        {
            ...
            if (dyn_cells.CanConvert<SingleType>())
            {
                ...
                output["topologies/" + topo_name + "/elements/shape"] =
                    conduit_name;
                ...
                if (zero_copy)
                {
                    output["topologies/" + topo_name +
                           "/elements/connectivity"]
                        .set_external(vtkh::GetVTKMPointer(conn),
                                                              ...
                                                              shape_id))
            {
                ...
                auto cells =
                    dyn_cells.AsCellSet<vtkm::cont::CellSetExplicit<>>();
                auto shapes =
                    cells.GetShapesArray(vtkm::TopologyElementTagCell(),
                                         vtkm::TopologyElementTagPoint());
            ...
            else
            {
                ...
                Node &topo_ele =
                    output["topologies/" + topo_name + "/elements"];
                topo_ele["shape"] = "mixed";

                VTKmBlueprintShapeMap(topo_ele["shape_map"]);

                size_t num_cells =
                    static_cast<size_t>(cells.GetNumberOfCells());
                auto vtkm_shapes =
                    cells.GetShapesArray(vtkm::TopologyElementTagCell{},
                                         vtkm::TopologyElementTagPoint{});
                auto vtkm_conn = cells.GetConnectivityArray(
                    vtkm::TopologyElementTagCell{},
                    vtkm::TopologyElementTagPoint{});
                auto vtkm_offsets =
                    cells.GetOffsetsArray(vtkm::TopologyElementTagCell{},
                                          vtkm::TopologyElementTagPoint{});

                std::size_t conn_size =
                    static_cast<std::size_t>(vtkm_conn.GetNumberOfValues());

                if (zero_copy)
                {
                    topo_ele["shapes"].set_external(
                        vtkh::GetVTKMPointer(vtkm_shapes), num_cells);
                    topo_ele["connectivity"].set_external(
                        vtkh::GetVTKMPointer(vtkm_conn), conn_size);
                    topo_ele["offsets"].set_external(
                        vtkh::GetVTKMPointer(vtkm_offsets), num_cells);
                }
                else
                {
                    topo_ele["shapes"].set(vtkh::GetVTKMPointer(vtkm_shapes),
                                           num_cells);
                    topo_ele["connectivity"].set(
                        vtkh::GetVTKMPointer(vtkm_conn), conn_size);
                    topo_ele["offsets"].set(vtkh::GetVTKMPointer(vtkm_offsets),
                                            num_cells);
                }

                // bp requires sizes, so we have to compute them
                topo_ele["sizes"].set(DataType::index_t(num_cells));
                generate_sizes_from_shapes(topo_ele["shapes"],
                                           topo_ele["sizes"]);
            }
        }
    }
...
void SetFieldValuesFromVTKmUnknownArrayHandleVec(
    vtkm::cont::UnknownArrayHandle &dyn_handle,
    bool zero_copy,
    Node &output_values)
{
    ...
    for (index_t comp = 0; comp < N; comp++)
    {
        ...
        if (zero_copy)
        {
            ...
            {
                stride_handle =
                    dyn_handle.ExtractComponent<T>(comp, vtkm::CopyFlag::Off);
            }
            catch (...)
            {
                stride_handle =
                    dyn_handle.ExtractComponent<T>(comp, vtkm::CopyFlag::On);
                zero_copy = false;
            }
        }
        else
        ...
        vtkm::cont::ArrayHandleBasic<T> basic_array =
            stride_handle.GetBasicArray();

        if (zero_copy)
        {
            output_values_component.set_external(
                (T *)vtkh::GetVTKMPointer(basic_array),
                stride_handle.GetNumberOfValues(),
                sizeof(T) *
                    stride_handle.GetOffset(), // starting offset in bytes
                sizeof(T) * stride_handle.GetStride()); // stride in bytes
        }
        else
        {
            output_values_component.set(
                (T *)vtkh::GetVTKMPointer(basic_array),
                stride_handle.GetNumberOfValues(),
                sizeof(T) *
                    stride_handle.GetOffset(), // starting offset in bytes
...
//---------------------------------------------------------------------------//
template <typename T>
void SetFieldValuesFromVTKmUnknownArrayHandle(
    vtkm::cont::UnknownArrayHandle &dyn_handle,
    bool zero_copy,
    Node &output_values)
{
    ...
    if (zero_copy)
    {
        ...
        {
            stride_handle =
                dyn_handle.ExtractComponent<T>(0, vtkm::CopyFlag::Off);
        }
        catch (vtkm::cont::Error &e) // fall back to copy
        {
            stride_handle =
                dyn_handle.ExtractComponent<T>(0, vtkm::CopyFlag::On);
            zero_copy = false;
        }
    }
...
void VTKHDataAdapter::VTKmFieldToBlueprint(conduit::Node &output,
                                           ...
                                           bool zero_copy)
{
    ...
    bool assoc_points =
        vtkm::cont::Field::Association::Points == field.GetAssociation();
    bool assoc_cells =
        vtkm::cont::Field::Association::Cells == field.GetAssociation();
    // bool assoc_mesh  = vtkm::cont::Field::ASSOC_WHOLE_MESH ==
    ...
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Float32, 3>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Vec<vtkm::Float64, 3>>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Float64, 3>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Vec<vtkm::Int32, 3>>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Int32, 3>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Vec<vtkm::Float32, 2>>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Float32, 2>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Vec<vtkm::Float64, 2>>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Float64, 2>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Vec<vtkm::Int32, 2>>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandleVec<vtkm::Int32, 2>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Float32>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::Float32>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Float64>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::Float64>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Int8>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::Int8>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Int32>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::Int32>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::Int64>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::Int64>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::UInt32>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::UInt32>(
            dyn_handle, zero_copy, output_values);
    }
    else if (dyn_handle.IsValueType<vtkm::UInt8>())
    {
        SetFieldValuesFromVTKmUnknownArrayHandle<vtkm::UInt8>(
            dyn_handle, zero_copy, output_values);
    }
...
//-----------------------------------------------------------------------------
bool VTKHDataAdapter::CheckShapeMapVsVTKmShapeIds(const Node &shape_map)
{
    bool res = true;
    Node ref_map;
...
void VTKHDataAdapter::VTKmBlueprintShapeMap(conduit::Node &output)
{
    output.reset();
    output["tri"] = 5;
    output["quad"] = 9;
...
void VTKHDataAdapter::VTKHCollectionToBlueprintDataSet(
    ...
    bool zero_copy)
{
    ...
    try
    {
        for (auto domain_it : domain_map)
        {
            ...
            for (auto topo_it : domain_it.second)
            {
                ...
                VTKHDataAdapter::VTKmToBlueprintDataSet(
                    &dataset, dom, topo_name, zero_copy);
            }
        }
    }
    ...
    if (!success)
    {
        ...
        ASCENT_ERROR(
            "Failed to convert VTK-m data set to blueprint: " << err_msg);
    }
}

void VTKHDataAdapter::VTKHToBlueprintDataSet(vtkh::DataSet *dset,
                                             conduit::Node &node,
                                             bool zero_copy)
{
    node.reset();
...
void VTKHDataAdapter::VTKmToBlueprintDataSet(const vtkm::cont::DataSet *dset,
                                             conduit::Node &node,
                                             const std::string &topo_name,
                                             bool zero_copy)
{
