...
std::shared_ptr<dray::Collection> DataObject::as_dray_collection()
{
    ...
    else
    {
        ...
        {
            std::shared_ptr<dray::Collection> collection(
                new dray::Collection());
            const int num_domains = m_high_bp->number_of_children();
            for (int i = 0; i < num_domains; ++i)
            {
                dray::DataSet dset = dray::BlueprintReader::blueprint_to_dray(
                    m_high_bp->child(i));
...
std::shared_ptr<VTKHCollection> DataObject::as_vtkh_collection()
{
    ...
    else
    {
        ...
        {
            std::shared_ptr<conduit::Node> low_order(
                Transmogrifier::low_order(*m_high_bp));
            m_low_bp = low_order;
        }
        ...
        // convert to vtkh
        std::shared_ptr<VTKHCollection> vtkh_dset(
            VTKHDataAdapter::BlueprintToVTKHCollection(*to_vtkh, zero_copy));
...
std::shared_ptr<conduit::Node> DataObject::as_low_order_bp()
{
    ...
    else if (m_source == Source::VTKH)
    {
        ...
        bool zero_copy = true;
        VTKHDataAdapter::VTKHCollectionToBlueprintDataSet(
            m_vtkh.get(), *out_data, true);
...
std::shared_ptr<conduit::Node> DataObject::as_high_order_bp()
{
    ...
    ASCENT_ERROR(
        "converting from low order to high order is not currenlty supported");
#else
    ASCENT_ERROR(
        "Cannot provide high order blueprint. MFEM support not enabled.");
#endif
