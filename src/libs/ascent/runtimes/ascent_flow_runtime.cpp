...
//-----------------------------------------------------------------------------
void FlowRuntime::Initialize(const conduit::Node &options)
{
    int rank = 0;
    ...
    if (options.has_path("web/stream") &&
        options["web/stream"].as_string() == "true" && rank == 0)
    {
...
//-----------------------------------------------------------------------------
void FlowRuntime::Publish(const conduit::Node &data)
{
...
//-----------------------------------------------------------------------------
void FlowRuntime::ConnectSource()
{
    ...
    {
        w.registry().add<Node>("_ascent_input_data", &m_data);
    }
...
//-----------------------------------------------------------------------------
void FlowRuntime::Execute(const conduit::Node &actions)
{
    ResetInfo();
    ...
    for (int i = 0; i < actions.number_of_children(); ++i)
    {
        ...
        else if (action_name == "connect")
        {
            ...
            else
            {
                ...
                w.graph().connect(
                    action["src"].as_string(), action["dest"].as_string(), 0);
            }
        }
