...
public:
    static int m_rank;
    static void
    info_handler(const std::string &msg, const std::string &file, int line)
    {
        if (m_rank == 0)
        {
            std::cout << "[" << file << " : " << line << "]"
...
AscentRuntime::AscentRuntime()
    : Runtime(),
      ...
      m_rank(0), m_default_output_dir("."), m_session_name("ascent_session"),
...
#    endif
    ...
    if (host_alloc_id != -1)
    {
        ...
        {
            ASCENT_ERROR("Error setting host allocator id to "
                         << host_alloc_id);
        }
    }
    ...
    {
        if (!ascent::AllocationManager::set_device_allocator_id(
                device_alloc_id))
        {
            ASCENT_ERROR("Error setting device allocator id to "
                         << device_alloc_id);
        }
    }
...
#    endif     // end ASCENT_DRAY_ENABLED
    ...
    if (options.has_path("ghost_field_name"))
    {
        ...
        else if (options["ghost_field_name"].dtype().is_list())
        {
            ...
            const int num_children =
                options["ghost_field_name"].number_of_children();
            for (int i = 0; i < num_children; ++i)
            {
                const conduit::Node &child =
                    options["ghost_field_name"].child(i);
    ...
    if (options.has_path("ghost_field_names"))
    {
        ...
        m_ghost_fields.reset();
        const int num_children =
            options["ghost_field_names"].number_of_children();
        for (int i = 0; i < num_children; ++i)
        {
            const conduit::Node &child = options["ghost_field_names"].child(i);
            if (!child.dtype().is_string())
            {
                ASCENT_ERROR("ghost_field_names list child "
                             << i << " is not a string");
            }
            m_ghost_fields.append().set(child);
        }
    ...
    if (options.has_path("web/stream") &&
        options["web/stream"].as_string() == "true" && m_rank == 0)
    {
#ifdef ASCENT_WEBSERVER_ENABLED
        if (options.has_path("web/document_root"))
        {
            m_web_interface.SetDocumentRoot(
                options["web/document_root"].as_string());
        }
...
//-----------------------------------------------------------------------------
void AscentRuntime::AddPublishedMeshInfo()
{
    ...
    index_t num_domains =
        blueprint::mpi::mesh::number_of_domains(m_source, mpi_comm);
    m_info["published_mesh_info/number_of_domains"] = num_domains;

    // if we have domains, then also provde the index
    if (num_domains > 0)
    {
        blueprint::mpi::mesh::generate_index(
            m_source, "", m_info["published_mesh_info/index"], mpi_comm);
    }
    ...
    if (num_domains > 0)
    {
        ...
        blueprint::mesh::generate_index(
            m_source[0], "", 1, m_info["published_mesh_info/index"]);
    }
...
void AscentRuntime::Cleanup()
{
        ...
        m_runtime_options["timings"].as_string() == "true")
    {
        ...
        file_name =
            conduit::utils::join_file_path(m_default_output_dir, file_name);
...
#    endif
    ...
    {
        ASCENT_ERROR(
            "Inconsistent domain ids: all domains must either have an id "
    ...
    MPI_Allgather(
        &num_domains, 1, MPI_INT, domains_per_rank, 1, MPI_INT, mpi_comm);
    ...
    MPI_Allgather(
        &unique_ids, 1, MPI_INT, unique_ids_array, 1, MPI_INT, mpi_comm);
    std::stringstream ss;
    for (int i = 0; i < comm_size; i++)
    {
    ...
    local_check_timer_log << "Local Uniqueness Check: "
                          << local_check_timer_time << "\n";
...
#    endif
    ...
    {
        conduit::Node &dom = m_source.child(i);
        domain_ids_per_rank[domain_offset + i] =
            dom["state/domain_id"].to_int32();
        domain_rank[domain_offset + i] = m_rank;
    }

    MPI_Allreduce(domain_ids_per_rank,
                  global_domain_ids,
                  total_domains,
                  MPI_INT,
                  MPI_MAX,
                  mpi_comm);
    MPI_Allreduce(domain_rank,
                  global_domain_rank,
                  total_domains,
                  MPI_INT,
                  MPI_MAX,
    ...
    if (global_ids.size() != total_domains)
    {
        ...
        for (int i = 0; i < total_domains; i++)
        {
            map_global_ids[global_domain_ids[i]].push_back(
                global_domain_rank[i]);
        }
        for (auto itr = map_global_ids.begin(); itr != map_global_ids.end();
             ++itr)
        {
...
#    endif
    ...
    ensureDomainIDs_log << "AscentRuntime::EnsureDomainIDs [TOTAL]: "
                        << ensureDomainIDs_time << "\n";
...
//-----------------------------------------------------------------------------
conduit::Node AscentRuntime::CreateDefaultFilters()
{
    ...
    m_workspace.graph().add_filter(
        "blueprint_verify", // registered filter name
        "verify",           // "unique" filter name
        params);

    m_workspace.graph().connect("source", "verify",
                                0); // default port
    ...
    for (int i = 0; i < num_ghosts; ++i)
    {
        ...
        m_workspace.graph().add_filter(
            "vtkh_ghost_stripper", filter_name, threshold_params);
    ...
    m_workspace.graph().add_filter("alias", queries_endpoint);

    m_workspace.graph().connect(prev_filter,      // src
                                queries_endpoint, // dest
                                0);               // default port

    m_workspace.graph().add_filter("dependent_alias", endpoint);
                                          ...
                                          const std::string pipeline_name)
{
    ...
    {
        ASCENT_ERROR("Pipeline " << pipeline_name
                                 << " empty. Must specify a filter.");
    }
    ...
    for (int i = 0; i < pipeline.number_of_children(); ++i)
    {
        ...
        if (type.find("exa") == (size_t)0 && type.size() > (size_t)3)
        {
            type = type.substr(3);
        }

        const conduit::Node &n_transforms =
            registered_filter_types()["transforms"];
        ...
        {
            ASCENT_ERROR("Unrecognized transform filter "
                         << filter["type"].as_string());
        }
        ...
        m_workspace.graph().add_filter(filter_name, name, filter["params"]);
    ...
    m_workspace.graph().add_filter("alias", pipeline_name);
...
//-----------------------------------------------------------------------------
void AscentRuntime::ConvertExtractToFlow(const conduit::Node &extract,
                                         const std::string extract_name)
{
    ...
    if (extract_type == "python")
    {
        ...
        if (params.has_path("file"))
        {
            ...
            if (!n_py_src.dtype().is_string())
            {
                ASCENT_ERROR("failed to read python script file "
                             << script_fname << " and broadcast source");
            }
        ...
        py_src_final
            << "# ascent mpi comm helper function" << std::endl
    ...
    else if (extract_type == "jupyter")
    {
        ...
        py_src_final
            ...
            << "from ascent.mpi import jupyter_bridge " << std::endl;
#else
        // import our code that connects to the bridge kernel server
    ...
    {
        ASCENT_ERROR("Unrecognized extract type "
                     << extract["type"].as_string());
    }
    ...
    m_workspace.graph().add_filter(filter_name, extract_name, params);
                                         ...
                                         const std::string trigger_name)
{
    ...
    m_workspace.graph().add_filter("basic_trigger", trigger_name, params);
                                       ...
                                       const std::string prev_name)
{
    ...
    m_workspace.graph().add_filter("basic_query", query_name, params);
    ...
    m_workspace.graph().connect(conn_port, query_name, "dummy");
...
//-----------------------------------------------------------------------------
void AscentRuntime::ConvertCommandToFlow(const conduit::Node &command,
                                         const std::string command_name)
{
    ...
    m_workspace.graph().add_filter("command", command_name, params);
                                      ...
                                      const std::string plot_name)
{
    ...
    m_workspace.graph().add_filter(filter_name, plot_name, plot);
    ...
    {
        m_workspace.graph().add_filter("alias", endpoint_name);
...
//-----------------------------------------------------------------------------
void AscentRuntime::CreateTriggers(const conduit::Node &triggers)
{
...
//-----------------------------------------------------------------------------
void AscentRuntime::CreateCommands(const conduit::Node &commands)
{
...
//-----------------------------------------------------------------------------
void AscentRuntime::ConnectGraphs()
{
    ...
    for (int i = 0; i < m_connections.number_of_children(); ++i)
    {
        ...
        {
            ASCENT_ERROR(names[i] << "' references unknown pipeline: "
                                  << pipeline);
        }
...
std::string AscentRuntime::GetDefaultImagePrefix(const std::string scene)
{
...
void AscentRuntime::CreateScenes(const conduit::Node &scenes)
{
    ...
    for (int i = 0; i < num_scenes; ++i)
    {
        ...
        {
            ASCENT_ERROR(
                "Scene must have at least one plot: " << scene.to_yaml());
        }
        ...
        if (scene.has_path("image_prefix") || scene.has_path("image_name"))
        {
            ...
            {
                render_params["image_prefix"] =
                    scene["image_prefix"].as_string();
            }
            else
            {
        ...
        if (!flow::Workspace::supports_filter_type("default_render"))
        {
            ...
            ASCENT_ERROR("Cannot create scene ("
                         << names[i]
                         << ")"
        ...
        m_workspace.graph().add_filter(
            "default_render", renders_name, render_params);
        ...
        m_workspace.graph().add_filter("exec_scene", exec_name);
        ...
        for (int p = 0; p < plot_count; ++p)
        {
            ...
            conduit::Node empty;
            m_workspace.graph().add_filter("vtkh_bounds", bounds_name, empty);
            ...
            if (p > 0)
            {
                std::string union_bounds_name =
                    plot_names[p] + "_union_bounds";
                m_workspace.graph().add_filter(
                    "vtkh_union_bounds", union_bounds_name, empty);
                ...
                else
                {
                    ...
                    // previous union
                    //
                    m_workspace.graph().connect(
                        union_bounds_names[p - 2], // src
                        union_bounds_name,         // dest
                        0);                        // default port
                }
            ...
            m_workspace.graph().add_filter("add_plot", add_name);
...
void AscentRuntime::BuildGraph(const conduit::Node &actions)
{
    ...
    for (int i = 0; i < actions.number_of_children(); ++i)
    {
        ...
        if (action_name == "add_pipelines")
        {
            ...
            {
                ASCENT_ERROR(
                    "action 'add_pipelines' missing child 'pipelines'");
            }
        }
        ...
        else if (action_name == "execute" || action_name == "reset")
        {
        ...
        else if (action_name == "open_log")
        {
            ...
            {
                ascent::Logger::instance().set_log_threshold(
                    action["log_threshold"].as_string());
            }
            else
            {
#if defined(ASCENT_MPI_ENABLED)
            ...
            std::string file_pattern =
                action.has_path("file_pattern")
                    ? action["file_pattern"].as_string()
                    : "ascent_log_output_rank_{rank:05d}.yaml";
            ...
            ASCENT_LOG_DEBUG(conduit_fmt::format(
                "mpi info: rank={}, size={}", m_rank, comm_size));
#else
            std::string file_pattern = action.has_path("file_pattern")
                                           ? action["file_pattern"].as_string()
                                           : "ascent_log_output.yaml";
        ...
        else if (action_name == "set_log_threshold")
        {
            ...
            {
                ASCENT_WARN("No Log Threshold level given. No changes to "
                            "logging behavior made.");
            }
        }
        else if (action_name == "set_echo_threshold")
        {
            ...
            {
                ascent::Logger::instance().set_echo_threshold(
                    action["echo_threshold"].as_string());
            }
            else
            {
                ASCENT_WARN("No Echo Threshold level given. No changes to "
                            "echo output behavior made.");
            }
        }
...
void AscentRuntime::Execute(const conduit::Node &actions)
{
    ...
    try
    {
        ...
        m_workspace.registry().add<DataObject>(
            "source_object", &m_data_object, 1);
        ...
        {
            Node *extracts_list =
                m_workspace.registry().fetch<Node>("extract_list");
        ...
        {
            runtime::expressions::ExpressionEval::get_last(
                m_info["expressions"]);
        }
...
//-----------------------------------------------------------------------------
void AscentRuntime::DisplayError(const std::string &msg)
{
    if (m_rank == 0)
    {
        std::cerr << msg;
...
//-----------------------------------------------------------------------------
void AscentRuntime::RegisterFilterType(const std::string &role_path,
                                       ...
                                       const std::string &filter_type_name)
{
    ...
    {
        // ASCENT_ERROR("Filter " << f_name << " already registered at " <<
        // path);
    }
    else
    {
...
void AscentRuntime::PaintNestsets()
{
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        if (dom.has_path("nestsets"))
        {
            ...
            for (int n = 0; n < num_nests; ++n)
            {
                ...
                {
                    topo_nestsets[nestset["topology"].as_string()] =
                        nest_names[n];
                }
                else
                {
        ...
        for (int f = 0; f < num_ghosts; ++f)
        {
            ...
            if (dom.has_path(fpath))
            {
                ...
                {
                    topo_ghosts[dom[fpath + "/topology"].as_string()] =
                        ghosts[f];
                }
                else
                {
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        for (auto topo_name : topo_names)
        {
            ...
            bool has_nest =
                topo_nestsets.find(topo_name) != topo_nestsets.end();
            ...
            if (has_ghost)
            {
                ...
                if (dom.has_path("fields/" + ghost_name))
                {
                    ...
                    dom["fields/"].rename_child(ghost_name + "_bananas",
                                                ghost_name);

                    conduit::Node &ghost_field = dom[ghost_path];

                    runtime::expressions::paint_nestsets(
                        nest_name, topo_name, dom, ghost_field);
                }
                else
                {
                ...
                if (m_source.number_of_children() > 0)
                {
                    if (m_source.child(0).has_path("fields"))
                    {
                        std::vector<string> names =
                            m_source.child(0)["fields"].child_names();
                        for (auto name : names)
                        {
                            ss << " '" << name << "'";
                        }
                    ...
                    else
                    {
                        ss << "can't deduce possible fields. "
                           << "Published data does not contain fields in dom "
                              "0";
                    }
                }
                ASCENT_ERROR("User specified Ghost field '"
                             << ghost_name
...
//---------------------------------------------------------------------------//
void AscentRuntime::SetStatus(const std::string &msg)
{
    ...
    if (m_rank == 0)
    {
        ...
        while (itr.has_next())
        {
            ...
            if (Metadata::n_metadata.has_path("cycle"))
            {
                ...
                filename =
                    conduit_fmt::format("out_ascent_info_{:06d}.yaml", cycle);
            }
    ...
    for (int a = 0; a < num_actions; ++a)
    {
        ...
        if (action.has_path("expressions"))
        {
            ...
            runtime::expressions::ExpressionEval::save_cache(
                filename, expressions_selection);
        }
        else
        {
