...
void Cache::save(const std::string &filename)
{
    // the session file can be blank during testing,
    // since its not actually opening ascent
    if (m_rank == 0 && !m_data.dtype().is_empty())
    {
                 ...
                 const std::vector<std::string> &selection)
{
    ...
    if (m_rank == 0 && !data.dtype().is_empty())
    {
...
void register_builtin()
{
    ...
    flow::Workspace::register_filter_type<
        expressions::ExprArrayReductionMax>();
    flow::Workspace::register_filter_type<
        expressions::ExprArrayReductionMin>();
    flow::Workspace::register_filter_type<
        expressions::ExprArrayReductionAvg>();
    flow::Workspace::register_filter_type<
        expressions::ExprArrayReductionSum>();
    ...
    flow::Workspace::register_filter_type<
        expressions::ExprHistoryGradientRange>();
    ...
    flow::Workspace::register_filter_type<
        expressions::ExprHistogramCDFQuantile>();
    flow::Workspace::register_filter_type<
        expressions::ExprHistogramBinByValue>();
    flow::Workspace::register_filter_type<
        expressions::ExprHistogramBinByIndex>();
    ...
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionMax>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionMin>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionAvg>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionSum>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionNanCount>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshFieldReductionInfCount>();
    ...
    flow::Workspace::register_filter_type<
        expressions::ExprMeshBinningBinByIndex>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshBinningPointAndAxis>();
    flow::Workspace::register_filter_type<
        expressions::ExprMeshBinningMaxFromPoint>();
...
ExpressionEval::ExpressionEval(DataObject &dataset) : m_data_object(dataset) {}

void ExpressionEval::load_cache(const std::string &dir,
            ...
            for (int a = 0; a < num_args; ++a)
            {
                ...
                else
                {
                    ...
                    if (seen_opt)
                    {
                        function.print();
                        ASCENT_ERROR(
                            "Function: optional parameters must come after "
                            "required params");
                    }
                }
            }
...
void initialize_functions()
{
    ...
    array_replace_sig["filter_name"] =
        "expr_array_replace"; // matches the filter's type_name
    array_replace_sig["args/arg1/type"] = "array";
    array_replace_sig["args/find/type"] = "double";
    array_replace_sig["args/find/description"] =
        "Value in the array to find and replace.";
    array_replace_sig["args/replace/type"] = "double";
    array_replace_sig["args/replace/description"] = "Replacement value.";
    array_replace_sig["description"] =
        "Find and replace zero or more values in an array.";
    ...
    // gradient is an interesting case. Currently function matching picks the
    // first match and breaks so this if we don't list the most restrictive
    // first, bad things will happen (get a syntax error or something).
    ...
    scalar_gradient_sig["args/expr_name/description"] =
        "`expr_name` should be the name of an expression that was evaluated "
        "in "
        "the past.";

    scalar_gradient_sig["args/window_length/type"] = "scalar";
    scalar_gradient_sig["args/window_length/optional"];
    scalar_gradient_sig["args/window_length/description"] =
        "The number of time points ago to use as the x0 for the gradient calculation. defaults to ``1`` \
    ...
    scalar_gradient_sig["args/window_length_unit/description"] =
        "Can be one of three values: ``\"index\"``, ``\"time\"`` or ``\"cycle\".`` \
    ...
    scalar_gradient_sig["description"] =
        "Return the temporal gradient of the given expression for the current "
        "point in time.";

    // -------------------------------------------------------------
    // NOTE: 2023-07-13 CYRUS
    // `history_gradient` is a new user facing name for what was `gradient`
    // applied to history
    conduit::Node &hist_gradient_sig =
        (*functions)["history_gradient"].append();
    // same sig as old "gradient_range"
    hist_gradient_sig.set(scalar_gradient_sig);

    // -------------------------------------------------------------

    conduit::Node &array_gradient_sig =
        (*functions)["gradient_range"].append();
    ...
    array_gradient_sig["args/first_relative_index/description"] =
        "The the first number of evaluations ago for which to calculate the temporal gradient. \
  ...
  gradient_range(pressure, first_relative_index=1, last_relative_index=10). This will calculate the temporal gradient for the previous 10 evaluations.";
    ...
    array_gradient_sig["args/last_relative_index/description"] =
        "The the last number of evaluations ago for which to calculate the temporal gradient. \
  ...
  pressure from the first two cycles.";
    ...
    array_gradient_sig["description"] =
        "As the simulation progresses the expressions \
  ...
  first_relative_index and last_relative_index, 3). first_absolute_time and last_absolute_time, or 4). first_absolute_cycle and last_absolute_cycle.";
    ...
    // -------------------------------------------------------------
    // NOTE: 2023-07-13 CYRUS
    // `history_gradient_range` is a new user facing name for what was
    // `gradient_range` applied to history
    conduit::Node &hist_gradient_range_sig =
        (*functions)["history_gradient_range"].append();
    // same sig as old "gradient_range"
    ...
    pow_sig["description"] = "Returns base raised to the power exponent."
                             " pow(base, exponent)";
    ...
    log_sig["description"] = "Returns the natural logarithm of the argument";
    ...
    field_nan_sig["args/arg1/type"] =
        "field"; // arg names match input port names
    ...
    array_avg_sig["filter_name"] =
        "expr_array_reduction_avg"; // matches the filter's type_name
    array_avg_sig["args/arg1/type"] =
        "array"; // arg names match input port names
    ...
    history_sig["args/relative_index/description"] =
  ...
  used.";
    ...
    history_range_sig["args/first_relative_index/description"] =
        "The the first number of evaluations ago for which to retrieve past expression values. \
  The index is relative, with ``first_relative_index=1`` corresponding to one evaluation ago. Example usage: \
  ...
  for the previous 10 evaluations.";
    ...
    history_range_sig["args/last_relative_index/description"] =
        "The the last number of evaluations ago for which to retrieve past expression values. \
  The index is relative, with ``last_relative_index=1`` corresponding to one evaluation ago. Example usage: \
  history_range(pressure, first_relative_index=1, last_relative_index=10). This will retrieve the value \
  for the previous 10 evaluations.";
  ...
  - midpoint: ``(i + j) / 2``";
    ...
    quantile_sig["description"] =
        "Return the `q`-th quantile of the data along \
  ...
  x-axis which 50 percent of the data lies below.";
    ...
    // point_and_axis()
    //---------------------------------------------------------------------------
    conduit::Node &point_and_axis_sig =
        (*functions)["point_and_axis"].append();
    ...
    // max_from_point()
    //---------------------------------------------------------------------------
    conduit::Node &max_from_point_sig =
        (*functions)["max_from_point"].append();
    ...
    axis_sig["args/name/description"] =
        ...
        "string `''` if `reduction_op` is either `sum` or `pdf` to mean we "
        "want "
        "to count the number of elements in the bin as our reduction "
        "variable.";
    // rectilinear binning
    ...
    axis_sig["args/max_val/description"] =
        "Maximum value of the axis (i.e. the value of the last tick).Defaults "
        "to "
    ...
    axis_sig2["args/var/description"] =
        "One of the strings ``'x', 'y', 'z'`` "
    ...
    axis_sig2["description"] = "Same as the above function except that "
                               "``reduction_var`` should be one "
  ...
  - rms: root mean square of values in a bin";
    ...
    //      "\"bin the count\" if ``reduction_op`` is one of ``sum``, ``pdf``,
    //      or "
    //      "``cdf``.";
    ...
    //      "all the fields used for the axes of ``binning``. It only makes
    //      sense " "to specify this when ``bin_axes`` and ``reduction_var``
    //      are a " "subset of ``x``, ``y``, ``z``.";
    //  binning_sig2["args/assoc/type"] = "string";
    //  binning_sig2["args/assoc/optional"];
    //  binning_sig2["args/assoc/description"] =
    //      "The association of the resultant field. Defaults to the
    //      association " "infered from the bin axes and and reduction
    //      variable. It only " "makes sense to specify this when ``bin_axes``
    //      and ``reduction_var`` are " "a subset of ``x``, ``y``, ``z``.";
    //  binning_sig2["description"] =
    //      "Returns a multidimensional data binning. Same as the above
    //      function " "except that ``reduction_var`` should be one of the
    //      strings ``'x', 'y', "
    //      "'z'`` and the association and topology can be explicitely
    //      specified.";
    //
    //---------------------------------------------------------------------------
    ...
    // conduit::Node &paint_binning_sig =
    // (*functions)["paint_binning"].append(); paint_binning_sig["return_type"]
    // = "field"; paint_binning_sig["filter_name"] = "paint_binning";
    ...
    //     " "is automatically generated and the field is treated as a
    //     temporary and " "removed from the dataset when the expression is
    //     done executing.";
    ...
    //     "topology associated with the bin axes. This topology must have "
    //     "all the fields used for the axes of ``binning``. It only makes
    //     sense " "to specify this when the ``bin_axes`` are a subset of
    //     ``x``, ``y``, "
    //     "``z``. Additionally, it must be specified in this case since there
    //     is " "not enough info to infer the topology assuming there are
    //     multiple " "topologies in the dataset.";
    ...
    //     "topology must have all the fields used for the axes of ``binning``.
    //     It " "only makes sense to specify this when the ``bin_axes`` are a
    //     subset of "
    //     "``x``, ``y``, ``z``.";
    // paint_binning_sig["description"] =
    //     "Paints back the bin values onto an existing mesh by binning the "
    //     "elements of the mesh and creating a new field there the value at
    //     each " "element is the value in the bin it falls into.";
    //
    // //---------------------------------------------------------------------------
    //
    ...
    //     "The name of the new field to be generated, the corresponding
    //     topology " "topology and coordinate sets will be named
    //     '``name``_topo' and "
    ...
    //     "A binning with 3 or fewer dimensions will be output as a new
    //     element " "associated field on a new topology on the dataset. This
    //     is useful for " "directly visualizing the binning.";

    //---------------------------------------------------------------------------
    // Jitable Functions
    //---------------------------------------------------------------------------
    // Functions below this line call JitFilter
    // All jitable functions need to have ["jitable"] in order for this to
    // happen filter_name is passed to JitFilter so that it can determine which
    // function to execute
    ...
    field_magnitude_sig["filter_name"] =
        "expr_jit_mesh_field_vector_magnitude";
    ...
    const_field2["description"] =
        "Used to explicitly specify the topology and association of a "
        "constant "
    ...
    recenter_sig["description"] =
        "Recenter a field from vertex association to "
    ...
    binning_value_sig["filter_name"] =
        "expr_jit_mesh_binning_value"; // still not a great name
    ...
    binning_value_sig["description"] =
        "Get the value of a vertex or cell in a given binning. In other "
        "words, "
...
void initialize_objects()
{
    ...
    cell["dx/description"] =
        "Cell dx, only defined for rectilinear topologies.";
    cell["dy/type"] = "jitable";
    cell["dy/description"] =
        "Cell dy, only defined for rectilinear topologies.";
    cell["dz/type"] = "jitable";
    cell["dz/description"] =
        "Cell dz, only defined for rectilinear topologies.";
    ...
    vertex["min/description"] =
        "Min coordinate of an axis-aligned bounding box (aabb)";
    aabb["max/type"] = "vector";
    vertex["max/description"] =
        "Max coordinate of an axis-aligned bounding box (aabb)";
...
conduit::Node ExpressionEval::evaluate(const std::string expr,
                                       std::string expr_name)
{
    ...
    int cycle =
        get_state_var(*m_data_object.as_node().get(), "cycle").to_int32();
    ...
    catch (std::exception &e)
    {
        delete root_node;
        w.reset();
        ASCENT_ERROR("Error while executing expression '"
                     << expr << "': " << e.what());
    }
    ...
    // add the sim time
    conduit::Node n_time =
        get_state_var(*m_data_object.as_node().get(), "time");
    double time = 0;
    ...
    w.reset();
#ifdef ASCENT_JIT_ENABLED
    ASCENT_DATA_ADD("Device high water mark",
...
void ExpressionEval::jit_root(conduit::Node &root,
