...
//-----------------------------------------------------------------------------
std::string fused_kernel_type(const std::vector<std::string> kernel_types)
{
...
//-----------------------------------------------------------------------------
void topo_to_jitable(const std::string &topology,
...
//-----------------------------------------------------------------------------
void ExprJitFilter::declare_interface(Node &i)
{
    stringstream ss;
...
//-----------------------------------------------------------------------------
bool ExprJitFilter::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
...
// expr: the main expression being transformed (e.g topo_volume *
// density[item]) for_body: for-loop body that holds code needed for expr
// kernel_body: code that we already generated but aren't touching (i.e. past
// for-loops). for_body gets wrapped in a for-loop and added to kernel_body
// when we need to generate an temporary field.
//-----------------------------------------------------------------------------
void ExprJitFilter::execute()
{
    ...
    try
    {
        ...
        // convert filter's inputs (numbers, topos, fields, binnings, etc.) to
        // jitables
        for (int i = 0; i < num_inputs; ++i)
        {
            const std::string input_fname =
                inputs.child(i)["filter_name"].as_string();
            const std::string type = inputs.child(i)["type"].as_string();
            // A jitable at "compile time" may have been executed at runtime so
            // we need check_type to get the runtime type to make sure it
            // hasn't executed
            ...
            else
            {
                ...
                if (type == "topology")
                {
                    // topo is special because it can build different kernels
                    // for each domain (kernel types)
                    detail::topo_to_jitable(
                        (*inp)["value"].as_string(), *dataset, jitable);
                    jitable.obj = *inp;
                }
                else
                {
                    ...
                    // During kernel fusion the type of the output kernel is
                    // the "union" of the types of the input kernels.
                    ...
                    if (type == "int" || type == "double" || type == "bool")
                    {
                        ...
                        for (int i = 0; i < num_domains; ++i)
                        {
                            jitable.dom_info.child(i)["args/" + input_fname] =
                                (*inp)["value"];
                        }
                    ...
                    else if (type == "field" || type == "jitable")
                    {
                        ...
                        for (int i = 0; i < num_domains; ++i)
                        {
                            ...
                            const conduit::Node &field =
                                dom["fields/" + field_name];
                            const std::string &topo_name =
                                field["topology"].as_string();
                            const std::string &assoc_str =
                                field["association"].as_string();

                            conduit::Node &cur_dom_info =
                                jitable.dom_info.child(i);

                            std::string values_path = "values";
                            if (inp->has_path("component"))
                            {
                                const std::string &component =
                                    (*inp)["component"].as_string();
                            ...
                            else
                            {
                                const int num_children =
                                    field[values_path].number_of_children();
                                default_kernel.num_components =
                                    std::max(1, num_children);
                            }

                            bool is_float64;
                            if (field[values_path].number_of_children() > 1)
                            {
                                is_float64 = field[values_path]
                                                 .child(0)
                                                 .dtype()
                                                 .is_float64();
                            }
                            ...
                            {
                                is_float64 =
                                    field[values_path].dtype().is_float64();
                            }
                            ...
                            // Used to determine if we need to generate an
                            // entire derived field beforehand for things like
                            // gradient(field). obj["value"] will have the name
                            // of the field, "value" goes away as soon as we do
                            // something like field + 1, indicating we are no
                            // longer dealing with an original field and will
                            // have to generate it via a for-loop.
                            jitable.obj["value"] = field_name;
                            jitable.obj["type"] = "field";
                        }
                        // We assume that fields don't have different
                        // strides/offsets in different domains (otherwise we
                        // might need to compile a kernel for every domain) so
                        // just use the first domain array for codegen
                        if (default_kernel.num_components == 1)
                        {
                            default_kernel.expr =
                                jitable.arrays[0].index(field_name, "item");
                        }
                        else
                        {
                            default_kernel.for_body.insert(
                                ...
                                "];\n");
                            for (int i = 0; i < default_kernel.num_components;
                                 ++i)
                            {
                                default_kernel.for_body.insert(
                                    field_name + "_item[" + std::to_string(i) +
                                    "] = " +
                                    jitable.arrays[0].index(
                                        field_name, "item", i) +
                                    ";\n");
                            }
                    ...
                    {
                        // we need to put the binning in the registry,
                        // otherwise it may get deleted
                        conduit::Node &binning_value =
                            (*remove)["temporaries"].append();
                        ...
                        for (int i = 0; i < num_domains; ++i)
                        {
                            ...
                            // TODO this is the same for every domain and it's
                            // getting copied...
                            pack_array(binning_value,
                                       input_fname + "_value",
                                       args,
                                       jitable.arrays[i]);
                            ...
                            for (int i = 0; i < axes.number_of_children(); ++i)
                            {
                                ...
                                {
                                    args[axis_prefix + "min_val"] =
                                        axis["min_val"];
                                    args[axis_prefix + "max_val"] =
                                        axis["max_val"];
                                    args[axis_prefix + "num_bins"] =
                                        axis["num_bins"];
                                }
                                else
                                {
                                ...
                                // we may not need the topology associated with
                                // the binning if we are painting to a
                                // different topology so don't pack it here
                            }
                        }
                    }
                    else if (type == "string")
                    {
                        // strings don't get converted to jitables, they are
                        // used as arguments to jitable functions
                        jitable.obj = *inp;
                    }
                    else
                    {
                        ASCENT_ERROR("Cannot convert object of type '"
                                     << type << "' to jitable.");
                    }
                }
            }
        ...
        // argument. hack: add a new input jitable to the end with the topology
        // and fuse it
        if (func == "expr_jit_mesh_field_gradient" ||
            func == "expr_jit_mesh_field_curl" ||
            func == "expr_jit_mesh_field_recenter" ||
            (func == "expr_jit_mesh_binning_value" &&
             !inputs.has_path("topo")))
        {
            new_jitables.emplace_back(num_domains);
            Jitable &jitable = new_jitables.back();
            input_jitables.push_back(&jitable);
            ...
            {
                // if a topology wasn't passed in get the one associated with
                // the binning
                const int binning_port = inputs["binning/port"].to_int32();
                const conduit::Node &binning =
                    *input<conduit::Node>(binning_port);
        ...
        else
        {
            ...
            {
                // get the input kernels with the right kernel_type for this
                // domain and determine the type of the fused kernel
                std::vector<const Kernel *> input_kernels;
                std::vector<std::string> input_kernel_types;
                for (const Jitable *input_jitable : input_jitables)
                {
                    const std::string kernel_type =
                        input_jitable->dom_info.child(dom_idx)["kernel_type"]
                            .as_string();
                    input_kernel_types.push_back(kernel_type);
                    input_kernels.push_back(
                        &(input_jitable->kernels.at(kernel_type)));
                }
                const std::string out_kernel_type =
                    detail::fused_kernel_type(input_kernel_types);
                (*out_jitable).dom_info.child(dom_idx)["kernel_type"] =
                    out_kernel_type;
                Kernel &out_kernel = out_jitable->kernels[out_kernel_type];
                const bool not_fused =
                    fused_kernel_types.find(out_kernel_type) ==
                    fused_kernel_types.cend();

                // this class knows how to combine kernels and generate jitable
                // functions
                ...
                // Map filter_name from the function signature to function name
                // in OCCA
                // ---------------------------------------------------------------------
...
public:
    ...
    //---------------------------------------------------------------------------
    static Filter *JitFilterFactoryFunctor(const std::string &filter_type_name)
    {
...
std::shared_ptr<const JitExecutionPolicy>
    ExprJitFilterFactoryFunctor::m_exec_policy;
...
ExprExpressionList::ExprExpressionList(int num_inputs)
    : Filter(), m_num_inputs(num_inputs)
{
    // empty
}

//-----------------------------------------------------------------------------
ExprExpressionList::ExprExpressionList() : Filter(), m_num_inputs(256)
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprExpressionList::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprExpressionList::verify_params(const conduit::Node &params,
                                       conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprExpressionList::execute()
{
...
ExprExpressionListFilterFactoryMethod(const std::string &filter_type_name)
{
    ...
    const std::string num_inputs_str = filter_type_name.substr(
        prefix_size, filter_type_name.size() - prefix_size);
                                            ...
                                            const int num_inputs)
{
    ...
    {
        flow::Workspace::register_filter_type(
            ss.str(), ExprExpressionListFilterFactoryMethod);
    }
    return ss.str();
}
