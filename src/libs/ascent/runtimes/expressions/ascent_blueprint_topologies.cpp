                        ...
                        const conduit::Node &domain)
{
    ...
    if (error)
    {
        ...
        ASCENT_ERROR("Coords array from topo '"
                     << topo_name << "' is neither float or double."
                     << " type is '" << type_name << "'."
                     << " Contact someone.");
    }
...
template <>
std::array<size_t, 1> logical_index(const size_t index,
...
template <>
std::array<size_t, 2> logical_index(const size_t index,
...
template <>
std::array<size_t, 3> logical_index(const size_t index,
    ...
    : Topology(topo_name, domain, N)
{
    ...
    if (this->coords_type == "uniform")
    {
        const conduit::Node &n_coords =
            domain["coordsets/" + this->coords_name];
    ...
    else if (this->coords_type == "rectilinear")
    {
        ...
        for (size_t i = 0; i < N; ++i)
        {
            const conduit::Node &coord_values =
                values.fetch(std::string(1, 'x' + i));
    ...
    else
    {
        ASCENT_ERROR("Unknown coordinate type '"
                     << this->coords_type << "' for point topology '"
                     << topo_name << "' in domain " << domain.name() << ".");
    }
}
...
template <typename T, size_t N>
size_t PointTopology<T, N>::get_num_cells() const
{
    ...
    : Topology(topo_name, domain, N)
{
    ...
    {
        ASCENT_ERROR(
            "Cannot initialize a UniformTopology class from topology '"
            << topo_name << "' in domain " << domain.name()
            << " which has type '" << this->topo_type << "'.");
    }
    ...
    : Topology(topo_name, domain, N)
{
    ...
    {
        ASCENT_ERROR(
            "Cannot initialize a RectilinearTopology class from topology '"
            << topo_name << "' in domain " << domain.name()
            << " which has type '" << this->topo_type << "'.");
    }
    ...
    : Topology(topo_name, domain, N)
{
    ...
    {
        ASCENT_ERROR(
            "Cannot initialize a StructuredTopology class from topology '"
            << topo_name << "' in domain " << domain.name()
            << " which has type '" << this->topo_type << "'.");
    }
    const conduit::Node &values =
    ...
    for (size_t i = 0; i < N; ++i)
    {
        ...
        const std::string &dim = std::string(1, 'i' + i);
        coords[i] = coord_values.value();
        // the blueprint gives structured dims in terms of elements not
        // vertices so we change it to vertices so that it's consistent with
        // uniform
    ...
    {
        ASCENT_ERROR("StructuredTopology ("
                     << topo_name << "): The number of points calculated ("
                     << num_points
                     << ") differs from the number of vertices in "
                        "corresponding coordset ("
...
constexpr size_t constexpr_pow(size_t x, size_t y)
{
...
StructuredTopology<T, N>::element_location(const size_t index) const
{
    ...
    {
        vertices[0] =
            (element_index[2] * dims[1] + element_index[1]) * dims[0] +
            element_index[0];
        vertices[1] = vertices[0] + 1;
        vertices[2] = vertices[1] + dims[0];
        vertices[3] = vertices[2] - 1;
...
template <typename T, size_t N>
size_t UnstructuredTopology<T, N>::get_num_points() const
{
    // number of unique elements in connectivity
    const conduit::int32 *conn_begin =
        (conduit::int32 *)connectivity.data_ptr();
    const conduit::int32 *conn_end =
        conn_begin + connectivity.dtype().number_of_elements();
    // points used in the topology
    const size_t num_points =
        std::unordered_set<T>(conn_begin, conn_end).size();
...
UnstructuredTopology<T, N>::element_location(const size_t index) const
{
    ...
    else if (shape == "polyhedral")
    {
        ...
        ASCENT_ERROR(
            "element_location for polyhedral shapes is not implemented.");
    }
    else
    {
                                          ...
                                          const conduit::Node &domain)
{
    ...
    if (topo_type == "uniform")
    {
        if (type == "double")
        {
            ...
            {
            case 1:
                return my_make_unique<UniformTopology<conduit::float64, 1>>(
                    topo_name, domain);
                break;
            case 2:
                return my_make_unique<UniformTopology<conduit::float64, 2>>(
                    topo_name, domain);
                break;
            case 3:
                return my_make_unique<UniformTopology<conduit::float64, 3>>(
                    topo_name, domain);
                break;
            }
        }
        else
        {
            ...
            {
            case 1:
                return my_make_unique<UniformTopology<conduit::float32, 1>>(
                    topo_name, domain);
                break;
            case 2:
                return my_make_unique<UniformTopology<conduit::float32, 2>>(
                    topo_name, domain);
                break;
            case 3:
                return my_make_unique<UniformTopology<conduit::float32, 3>>(
                    topo_name, domain);
                break;
            }
        }
    ...
    else if (topo_type == "rectilinear")
    {
        if (type == "double")
        {
            ...
            {
            case 1:
                return my_make_unique<
                    RectilinearTopology<conduit::float64, 1>>(topo_name,
                                                              domain);
                break;
            case 2:
                return my_make_unique<
                    RectilinearTopology<conduit::float64, 2>>(topo_name,
                                                              domain);
                break;
            case 3:
                return my_make_unique<
                    RectilinearTopology<conduit::float64, 3>>(topo_name,
                                                              domain);
                break;
            }
        }
        else
        {
            ...
            case 1:
                return my_make_unique<
                    RectilinearTopology<conduit::float32, 1>>(topo_name,
                                                              domain);
                break;
            case 2:
                return my_make_unique<
                    RectilinearTopology<conduit::float32, 2>>(topo_name,
                                                              domain);
                break;
            case 3:
                return my_make_unique<
                    RectilinearTopology<conduit::float32, 3>>(topo_name,
                                                              domain);
                break;
            }
        }
    ...
    else if (topo_type == "unstructured")
    {
        if (type == "double")
        {
            ...
            case 1:
                return my_make_unique<
                    UnstructuredTopology<conduit::float64, 1>>(topo_name,
                                                               domain);
                break;
            case 2:
                return my_make_unique<
                    UnstructuredTopology<conduit::float64, 2>>(topo_name,
                                                               domain);
                break;
            case 3:
                return my_make_unique<
                    UnstructuredTopology<conduit::float64, 3>>(topo_name,
                                                               domain);
                break;
            }
        }
        else
        {
            ...
            case 1:
                return my_make_unique<
                    UnstructuredTopology<conduit::float32, 1>>(topo_name,
                                                               domain);
                break;
            case 2:
                return my_make_unique<
                    UnstructuredTopology<conduit::float32, 2>>(topo_name,
                                                               domain);
                break;
            case 3:
                return my_make_unique<
                    UnstructuredTopology<conduit::float32, 3>>(topo_name,
                                                               domain);
                break;
            }
        }
    ...
    ASCENT_ERROR(
        "topologyFactory returning nullptr, this should never happen.");
    return nullptr;
}
