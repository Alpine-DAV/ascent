...
void FieldCode::field_idx(InsertionOrderedSet<std::string> &code,
                          ...
                          const bool declare) const
{
    ...
    {
        res +=
            " + " + index_name + "[1] * (" + topo_code->topo_name + "_dims_i";
    ...
    {
        res +=
            " + " + index_name + "[2] * (" + topo_code->topo_name + "_dims_i";
        if (association == "element")
        {
            res += " - 1";
        }
...
void FieldCode::element_vertex_values(InsertionOrderedSet<std::string> &code,
                                      const std::string &res_name,
                                      const int component,
                                      const bool declare) const
{
    if (topo_code->topo_type == "unstructured")
    {
        ...
        if (topo_code->shape_size == -1)
        {
            // multiple shapes
            ASCENT_ERROR(
                "element_vertex_values is not implemented for multi-shape "
    ...
    {
        const std::string &vertex = array_code.index(
            topo_code->topo_name + "_vertices", std::to_string(i));
...
// 3 : 2
// 0 : 3
void FieldCode::quad_gradient(InsertionOrderedSet<std::string> &code,
                              const std::string &res_name) const
{
    ...
    code.insert(
        {"double " + res_name + "_x[3];\n",
         res_name + "_x[0] = .5 * (" + vertex_locs + "[3][0] + " +
             vertex_locs + "[0][0] - " + vertex_locs + "[1][0] - " +
             vertex_locs + "[2][0]);\n",
         res_name + "_x[1] = .5 * (" + vertex_locs + "[0][0] + " +
             vertex_locs + "[1][0] - " + vertex_locs + "[2][0] - " +
             vertex_locs + "[3][0]);\n",
         "double " + res_name + "_y[3];\n",
         res_name + "_y[0] = .5 * (" + vertex_locs + "[3][1] + " +
             vertex_locs + "[0][1] - " + vertex_locs + "[1][1] - " +
             vertex_locs + "[2][1]);\n",
         res_name + "_y[1] = .5 * (" + vertex_locs + "[0][1] + " +
             vertex_locs + "[1][1] - " + vertex_locs + "[2][1] - " +
             vertex_locs + "[3][1]);\n",
...
// 6 : 5
// 7 : 6
void FieldCode::hex_gradient(InsertionOrderedSet<std::string> &code,
                             const std::string &res_name) const
{
  ...
  // vk = .25 * ( (v[7] + v[4] + v[5] + v[6]) - (v[3] + v[0] + v[1] + v[2]) );
    ...
    code.insert({
        ...
        res_name + "_x[0] = .25 * ( (" + vertex_locs + "[3][0] + " +
            vertex_locs + "[0][0] + " + vertex_locs + "[7][0] + " +
            vertex_locs + "[4][0]) - (" + vertex_locs + "[2][0] + " +
            vertex_locs + "[1][0] + " + vertex_locs + "[5][0] + " +
            vertex_locs + "[6][0]) );\n",
        res_name + "_x[1] = .25 * ( (" + vertex_locs + "[0][0] + " +
            vertex_locs + "[1][0] + " + vertex_locs + "[5][0] + " +
            vertex_locs + "[4][0]) - (" + vertex_locs + "[3][0] + " +
            vertex_locs + "[2][0] + " + vertex_locs + "[6][0] + " +
            vertex_locs + "[7][0]) );\n",
        res_name + "_x[2] = .25 * ( (" + vertex_locs + "[7][0] + " +
            vertex_locs + "[4][0] + " + vertex_locs + "[5][0] + " +
            vertex_locs + "[6][0]) - (" + vertex_locs + "[3][0] + " +
            vertex_locs + "[0][0] + " + vertex_locs + "[1][0] + " +
            vertex_locs + "[2][0]) );\n",
        "double " + res_name + "_y[3];\n",
        res_name + "_y[0] = .25 * ( (" + vertex_locs + "[3][1] + " +
            vertex_locs + "[0][1] + " + vertex_locs + "[7][1] + " +
            vertex_locs + "[4][1]) - (" + vertex_locs + "[2][1] + " +
            vertex_locs + "[1][1] + " + vertex_locs + "[5][1] + " +
            vertex_locs + "[6][1]) );\n",
        res_name + "_y[1] = .25 * ( (" + vertex_locs + "[0][1] + " +
            vertex_locs + "[1][1] + " + vertex_locs + "[5][1] + " +
            vertex_locs + "[4][1]) - (" + vertex_locs + "[3][1] + " +
            vertex_locs + "[2][1] + " + vertex_locs + "[6][1] + " +
            vertex_locs + "[7][1]) );\n",
        res_name + "_y[2] = .25 * ( (" + vertex_locs + "[7][1] + " +
            vertex_locs + "[4][1] + " + vertex_locs + "[5][1] + " +
            vertex_locs + "[6][1]) - (" + vertex_locs + "[3][1] + " +
            vertex_locs + "[0][1] + " + vertex_locs + "[1][1] + " +
            vertex_locs + "[2][1]) );\n",
        "double " + res_name + "_z[3];\n",
        res_name + "_z[0] = .25 * ( (" + vertex_locs + "[3][2] + " +
            vertex_locs + "[0][2] + " + vertex_locs + "[7][2] + " +
            vertex_locs + "[4][2]) - (" + vertex_locs + "[2][2] + " +
            vertex_locs + "[1][2] + " + vertex_locs + "[5][2] + " +
            vertex_locs + "[6][2]) );\n",
        "double " + res_name + "_z[3];\n",
        res_name + "_z[1] = .25 * ( (" + vertex_locs + "[0][2] + " +
            vertex_locs + "[1][2] + " + vertex_locs + "[5][2] + " +
            vertex_locs + "[4][2]) - (" + vertex_locs + "[3][2] + " +
            vertex_locs + "[2][2] + " + vertex_locs + "[6][2] + " +
            vertex_locs + "[7][2]) );\n",
        res_name + "_z[2] = .25 * ( (" + vertex_locs + "[7][2] + " +
            vertex_locs + "[4][2] + " + vertex_locs + "[5][2] + " +
            vertex_locs + "[6][2]) - (" + vertex_locs + "[3][2] + " +
            vertex_locs + "[0][2] + " + vertex_locs + "[1][2] + " +
            vertex_locs + "[2][2]) );\n",
        "double " + res_name + "_v[3];\n",
...
// if_body is executed if the target element/vertex (e.g. upper, lower,
// current) is within the mesh boundary otherwise else_body is executed
void FieldCode::visit_current(InsertionOrderedSet<std::string> &code,
                              const std::string &index_name,
                              ...
                              const int dim) const
{
    ...
    if_code.insert({"if(" + index_name + "[" + std::to_string(dim) +
                        "] > 0 && " + index_name + "[" + std::to_string(dim) +
...
// bounds of the mesh
void FieldCode::visit_upper(InsertionOrderedSet<std::string> &code,
                            const std::string &index_name,
                            const std::string &if_body,
                            const std::string &else_body,
                            const int dim) const
...
void FieldCode::visit_lower(InsertionOrderedSet<std::string> &code,
                            const std::string &index_name,
                            const std::string &if_body,
                            const std::string &else_body,
                            const int dim) const
...
void FieldCode::gradient(InsertionOrderedSet<std::string> &code) const
{
                                    ...
                                    topo_code->topo_type == "unstructured"))
    {
        ...
        if (topo_code->topo_type == "structured")
        {
            ...
            {
                ASCENT_ERROR(
                    "Gradient is not implemented for 1D structured meshes.");
            }
        }
        else if (topo_code->topo_type == "unstructured")
        {
            ...
            {
                ASCENT_ERROR(
                    "Gradient of unstructured vertex associated fields only "
    ...
    for (int i = 0; i < 3; ++i)
    {
        if (i < topo_code->num_dims)
        {
            ...
            field_idx(
                upper_body, index_name, association, upper + "_idx", false);
            upper_body.insert(
                upper + " = " +
            ...
            field_idx(
                lower_body, index_name, association, lower + "_idx", false);
            lower_body.insert(
                lower + " = " +
            ...
            // calculate delta
            code.insert(gradient_name + "_delta = " + upper + "_loc - " +
                            lower + "_loc;\n",
                        false);

            // calculate gradient
            code.insert(gradient_name + "[" + std::to_string(i) + "] = (" +
                        upper + " - " + lower + ") / " + gradient_name +
                        "_delta;\n");
        }
        else
        {
...
void FieldCode::curl(InsertionOrderedSet<std::string> &code) const
{
...
// structured topology
void FieldCode::visit_vertex_elements(InsertionOrderedSet<std::string> &code,
                                      const std::string &index_name,
                                      const std::string &if_body,
                                      const std::string &else_body,
                                      const int dim) const
...
void FieldCode::recenter(InsertionOrderedSet<std::string> &code,
                         ...
                         const std::string &res_name) const
{
    ...
    else
    {
            ...
            topo_code->topo_type == "structured")
        {
            topo_code->vertex_idx(code);
            const std::string index_name =
                topo_code->topo_name + "_vertex_idx";
            ...
            if (component == -1 && num_components > 1)
            {
                ...
                for (int i = 0; i < num_components; ++i)
                {
                    ...
                    avg_code.insert(res_name + "[" + i_str +
                                    "] = " + res_name + "_sum[" + i_str +
                                    "] / " + res_name + "_num_adj;\n");
                }
            }
