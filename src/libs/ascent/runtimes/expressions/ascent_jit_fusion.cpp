        ...
        else
        {
            ...
            if (error)
            {
                ASCENT_ERROR("Unsupported binary_op: (field with "
                             << lhs_kernel.num_components << " components) "
                             << op_str << " (field with "
                             << rhs_kernel.num_components << " components).");
            }
        }
    }
    else
...
void JitableFusion::builtin_functions(const std::string &function_name)
{
    if (not_fused)
    {
...
bool available_component(const std::string &axis, const int num_axes)
{
...
bool available_axis(const std::string &axis,
                    const int num_axes,
...
void JitableFusion::topo_attrs(const conduit::Node &obj,
    ...
    if (obj.has_path("attr"))
    {
        if (not_fused)
        {
            ...
            if (assoc.name() == "cell")
            {
                ...
                {
                    topo_code.element_coord(out_kernel.for_body,
                                            name,
                                            "",
                                            topo_name + "_cell_" + name);
                         ...
                         available_axis(name, topo->num_dims, topo_name))
                {
                    ...
                    {
                        ASCENT_ERROR("Can only get dx, dy, dz for uniform or "
                                     "rectilinear "
                ...
                else if (name == "volume")
                {
                    if (topo->num_dims != 3)
                    {
                        ASCENT_ERROR("Cell volume is only defined for "
                                     "topologies with 3 "
                                     "dimensions. The specified topology '"
                                     << topo->topo_name << "' has "
                                     << topo->num_dims << " dimensions.");
                    }
                ...
                else if (name == "area")
                {
                    if (topo->num_dims < 2)
                    {
                        ASCENT_ERROR("Cell area is only defined for "
                                     "topologies at most 2 "
                                     "dimensions. The specified topology '"
                                     << topo->topo_name << "' has "
                                     << topo->num_dims << " dimensions.");
                    }
                ...
                else if (name == "surface_area")
                {
                    if (topo->num_dims != 3)
                    {
                        ASCENT_ERROR("Cell surface area is only defined for "
                                     "topologies with 3 "
                                     "dimensions. The specified topology '"
                                     << topo->topo_name << "' has "
                                     << topo->num_dims << " dimensions.");
            ...
            else if (assoc.name() == "vertex")
            {
                if (is_xyz(name) &&
                    available_axis(name, topo->num_dims, topo_name))
                {
                    topo_code.vertex_coord(out_kernel.for_body,
                                           name,
                                           "",
                                           topo_name + "_vertex_" + name);
        ...
        if (name == "cell")
        {
            if (topo->topo_type == "points")
            {
                ASCENT_ERROR("Point topology '"
                             << topo_name << "' has no cell attributes.");
            }
...
void JitableFusion::expr_if()
{
    if (not_fused)
    {
        ...
        {
            ASCENT_ERROR(
                "Jitable if-else: The if-branch results in "
...
void JitableFusion::constant_field()
{
    ...
    if (inputs.has_path("assoc"))
    {
        ...
        {
            ASCENT_ERROR(
                "constant_field: Unknown association '"
                << new_association
    ...
    if (inputs.has_path("topo"))
    {
            ...
            out_jitable.association != "none")
        {
            ...
            std::unique_ptr<Topology> topo =
                topologyFactory(new_topology, domain);
            ...
            if (cur_dom_info.has_child("entries"))
            {
                ...
                if (new_entries != cur_entries)
                {
                    ASCENT_ERROR(
                        ...
                        << "-associated field on the topology '"
                        << new_topology
...
void JitableFusion::temporary_field(const Kernel &field_kernel,
                                    const std::string &field_name)
{
    ...
    if (not_fused)
    {
        // not a regular kernel_fuse because we have to generate a for-loop and
        // add it to kernel_body instead of fusing for_body
...
std::string JitableFusion::possible_temporary(const int field_port)
{
...
void JitableFusion::gradient(const int field_port, const int component)
{
    const Kernel &field_kernel = *input_kernels[field_port];
...
void JitableFusion::curl()
{
    ...
    {
        ASCENT_ERROR(
            "Vorticity is only implemented for fields with at least 2 "
...
void JitableFusion::recenter()
{
    if (not_fused)
    {
        ...
        const std::string res_name =
            field_name + "_recenter_" + target_association;
...
void JitableFusion::vector()
{
    ...
    if (arg1_jitable.obj.has_path("type") &&
        arg2_jitable.obj.has_path("type") && arg3_jitable.obj.has_path("type"))
    {
    ...
    if (not_fused)
    {
        ...
        if (arg1_kernel.num_components != 1 ||
            arg2_kernel.num_components != 1 || arg2_kernel.num_components != 1)
        {
            ASCENT_ERROR(
                "Vector arguments must all have exactly one component.");
        }
        out_kernel.fuse_kernel(arg1_kernel);
...
void JitableFusion::binning_value(const conduit::Node &binning)
{
  ...
  const std::string uniform_bin =
    ...
    const conduit::Node &topo_and_assoc = final_topo_and_assoc(
        dataset, bin_axes, out_jitable.topology, assoc_str_);
    ...
    if (not_fused)
    {
        ...
        const std::string &binning_name =
            inputs["binning/filter_name"].as_string();
        ...
        for (int axis_index = 0; axis_index < num_axes; ++axis_index)
        {
            ...
            if (axis.has_path("num_bins"))
            {
                // uniform axis
                stride_multiplier =
                    bin_axes.child(axis_index)["num_bins"].as_int32();
                ...
                code.insert("int " + axis_prefix + "bin_index = uniform_bin(" +
                            ...
                            axis_prefix + "max_val, " + axis_prefix +
                            "num_bins, " + axis_prefix + "clamp);\n");
            }
            else
            {
                // rectilinear axis
                stride_multiplier = bin_axes.child(axis_index)["bins"]
                                        .dtype()
                                        .number_of_elements() -
                                    1;
                ...
                code.insert("int " + axis_prefix +
                            "bin_index = rectilinear_bin(" + axis_value +
                            ", " + axis_prefix + "bins, " + axis_prefix +
                            "bins_len, " + axis_prefix + "clamp);\n");
            }

            // update the current item's home
            code.insert("if(" + axis_prefix + "bin_index != -1 && " +
                        filter_name + "_home != -1)\n{\n" + filter_name +
                        "_home += " + axis_prefix + "bin_index * " +
                        filter_name + "_stride;\n}\n");
            // update stride
            code.insert(filter_name + "_stride *= " +
                        std::to_string(stride_multiplier) + ";\n");
        }
