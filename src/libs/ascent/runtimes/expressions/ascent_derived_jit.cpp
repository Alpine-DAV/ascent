...
std::string type_string(const conduit::DataType &dtype)
{
    std::string type;
    if (dtype.is_float64())
    {
...
void host_realloc_array(const conduit::Node &src_array,
                        const conduit::Schema &dest_schema,
                        conduit::Node &dest_array)
{
    // This check isn't strong enough, it will pass if the objects have
    // different child names
    if (!dest_schema.compatible(src_array.schema()))
    {
        ASCENT_ERROR(
            "JIT: failed to allocate host array because the source and "
...
void device_alloc_temporary(const std::string &array_name,
                            const conduit::Schema &dest_schema,
                            conduit::Node &args,
                            ...
                            unsigned char *host_ptr)
{
    ...
    {
        // instead of using a temporary array for output and copying, we can
        // point Array<> at the destination conduit array
...
void device_alloc_array(const conduit::Node &array,
                        const conduit::Schema &dest_schema,
                        conduit::Node &args,
                        std::vector<Array<unsigned char>> &array_memories,
                        std::vector<slice_t> &slices)
{
    ...
    {
        unsigned char *start_ptr = static_cast<unsigned char *>(
            const_cast<void *>(res_array.data_ptr()));
        ...
        const std::string param = "const " +
                                  detail::type_string(array.dtype()) + " *" +
                                  array.name();
        array_memories.push_back(mem);
        slices.push_back(slice_t(array_memories.size() - 1, 0, size));
        args[param + "/index"] = slices.size() - 1;
    ...
    else
    {
        ...
        //       what this code fully does. Right now, its either block
        //       copyable or not. It should not matter if the block is in some
        //       larger region. I am just a cave man.
        //
        for (const std::string &component : res_array.child_names())
        {
            const conduit::Node &n_component = res_array[component];
            ...
            const std::string param =
                "const " + detail::type_string(n_component.dtype()) + " *" +
...
std::string indent_code(const std::string &input_code, const int num_spaces)
{
...
MemoryRegion::MemoryRegion(const void *start, const size_t size)
    : start(static_cast<const unsigned char *>(start)),
      end(this->start + size), allocated(false)
{
}
...
//-----------------------------------------------------------------------------
// {{{
bool is_compact_interleaved(const conduit::Node &array)
{
        ...
        for (const std::string &component : array.child_names())
        {
            ...
            else
            {
                const unsigned char *new_start =
                    static_cast<const unsigned char *>(n_component.data_ptr());
                min_start = std::min(
                    min_start, new_start, std::less<const unsigned char *>());
                ...
                ArrayCode &array_code)
{
    ...
    else
    {
        ...
        {
            type_id =
                static_cast<conduit::DataType::TypeID>(array.dtype().id());
            size = array.dtype().number_of_elements();
        }
        else
        {
            type_id = static_cast<conduit::DataType::TypeID>(
                array.child(0).dtype().id());
        ...
        array_code.array_map.insert(
            std::make_pair(name, SchemaBool(s, false)));
    }
}
                   ...
                   ArrayCode &array_code)
{
    ...
    if (topo_type == "uniform")
    {
        for (size_t i = 0; i < num_dims; ++i)
        {
            ...
            args[topo_name + "_spacing_d" + coord] =
                coords["spacing"].child(i);
    ...
    else if (topo_type == "unstructured")
    {
        ...
        {
            pack_array(
                elements["sizes"], topo_name + "_sizes", args, array_code);
            pack_array(
                elements["offsets"], topo_name + "_offsets", args, array_code);
        }
...
bool AlwaysExecutePolicy::should_execute(const Jitable &jitable) const
{
    return true;
}
...
std::string Jitable::generate_kernel(const int dom_idx,
                                     const conduit::Node &args) const
{
    ...
    for (int i = 0; i < num_args; ++i)
    {
        ...
        kernel_string +=
            type + arg.name() + (i == num_args - 1 ? ")\n{\n" : ",\n");
        first = false;
    }
    kernel_string += kernel.kernel_body.accumulate();
    kernel_string +=
        kernel.generate_loop("output", arrays[dom_idx], "entries");
    kernel_string += "}";
    return detail::indent_code(kernel_string, 0);
}

void Jitable::fuse_vars(const Jitable &from)
{
    ...
    for (int dom_idx = 0; dom_idx < num_domains; ++dom_idx)
    {
        ...
        if (src_dom_info.has_path("entries"))
        {
            if (dest_dom_info.has_path("entries"))
            {
                ...
                {
                    ASCENT_ERROR(
                        "JIT: Failed to fuse kernels due to an incompatible "
                        "number of entries: "
        ...
        {
            conduit::NodeConstIterator arg_itr =
                src_dom_info["args"].children();
...
bool Jitable::can_execute() const
{
...
void Jitable::execute(conduit::Node &dataset, const std::string &field_name)
{
#ifdef ASCENT_JIT_ENABLED
    ...
    try
    {
        ...
        {
            ASCENT_ERROR(
                "Error while executing derived field: Could not infer the "
        ...
        {
            ASCENT_ERROR(
                "Error while executing derived field: Could not determine the "
        ...
        for (int dom_idx = 0; dom_idx < num_domains; ++dom_idx)
        {
            ...
            const Kernel &kernel =
                kernels.at(cur_dom_info["kernel_type"].as_string());

            if (kernel.expr.empty())
            {
                ASCENT_ERROR(
                    "Cannot compile a kernel with an empty expr field. This "
            ...
            for (const auto &array : arrays[dom_idx].array_map)
            {
                ...
                {
                    detail::device_alloc_array(
                        cur_dom_info["args/" + array.first],
                        array.second.schema,
                        new_args,
                        array_buffers,
                        slices);
                ...
                {
                    // not in args so doesn't point to any data, allocate a
                    // temporary
                    ...
                    if (array.first == "output" &&
                        (device.mode() == "Serial" ||
                         device.mode() == "OpenMP"))
                    {
                        // in Serial and OpenMP we don't need a separate output
                        // array for the device, so just pass it conduit's
                        // array
            ...
            const int original_num_args =
                cur_dom_info["args"].number_of_children();
            ...
            const std::string kernel_string =
                generate_kernel(dom_idx, new_args);

            // std::cout << kernel_string << std::endl;

            // store kernels so that we don't have to recompile, even loading a
            // cached kernel from disk is slow
            ...
            try
            {
                ...
                {
                    occa_kernel =
                        device.buildKernelFromString(kernel_string, "map");
                ...
                ASCENT_DATA_ADD("kernel compile",
                                kernel_compile_timer.elapsed());
            }
            ...
            {
                ASCENT_ERROR(
                    "Jitable: Expression compilation failed with an unknown "
                    "error.\n\n"
                    << kernel_string);
            }
            ...
            for (int i = 0; i < num_new_args; ++i)
            {
                ...
                {
                    occa_kernel.pushArg(
                        array_memories[arg["index"].to_int32()]);
                }
                else
                {
                    ASCENT_ERROR("JIT: Unknown argument type of argument: "
                                 << arg.name());
                }
            }
...
int Jitable::num_devices()
{
    ...
    if (res != cudaSuccess)
    {
        ...
        msg << "Failed to get CUDA device count" << std::endl
            << "CUDA Error Message: " << cudaGetErrorString(res);
    ...
    if (res != hipSuccess)
    {
        ...
        msg << "Failed to get HIP device count" << std::endl
            << "HIP Error Message: " << hipGetErrorString(res);
        ASCENT_ERROR(msg.str());
    }
#endif
    return device_count;
