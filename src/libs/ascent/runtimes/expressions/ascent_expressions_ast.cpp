...
bool is_field_type(const std::string &type)
{
...
void ASTNamedExpression::accept(ASTVisitor *visitor) const
{
    visitor->visit(*this);
}
void ASTMethodCall::accept(ASTVisitor *visitor) const
{
    visitor->visit(*this);
}
...
void PrintVisitor::visit(const ASTBlock &block)
{
...
void PrintVisitor::visit(const ASTExpression &expr)
{
    std::cout << "placeholder expression" << std::endl;
}

void PrintVisitor::visit(const ASTInteger &expr)
{
    std::cout << "Creating integer: " << expr.m_value << std::endl;
}

void PrintVisitor::visit(const ASTDouble &expr)
{
    std::cout << "Creating double: " << expr.m_value << std::endl;
}

void PrintVisitor::visit(const ASTIdentifier &expr)
{
    std::cout << "Creating identifier reference: " << expr.m_name << std::endl;
}

void PrintVisitor::visit(const ASTNamedExpression &expr)
{
    expr.key->accept(this);
    expr.value->accept(this);
}

void PrintVisitor::visit(const ASTMethodCall &call)
{
...
void PrintVisitor::visit(const ASTIfExpr &expr)
{
...
void PrintVisitor::visit(const ASTBinaryOp &expr)
{
    ...
    case TNOT:
        ...
        break;
    default:
        ASCENT_ERROR("unknown binary op " << expr.m_op);
    }
...
void PrintVisitor::visit(const ASTString &expr)
{
    std::cout << "Creating string " << expr.m_name << std::endl;
}

void PrintVisitor::visit(const ASTBoolean &expr)
{
    std::string bool_str = "";
    switch (expr.tok)
    {
    ...
    case TFALSE:
        ...
        break;
    default:
        std::cout << "unknown bool literal " << expr.tok << "\n";
    }
    std::cout << "Creating bool literal " << bool_str << std::endl;
}

void PrintVisitor::visit(const ASTArrayAccess &expr)
{
...
void PrintVisitor::visit(const ASTDotAccess &expr)
{
...
void PrintVisitor::visit(const ASTExpressionList &list)
{
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTBlock &block)
{
    for (auto stmt : *block.stmts)
    {
        stmt->accept(this);
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTExpression &expr)
{
    ...
    if (!w.graph().has_filter("expr_binary_op_placeholder"))
    {
        ...
        w.graph().add_filter(
            "expr_bool", "expr_binary_op_placeholder", placeholder_params);
    }
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTInteger &expr)
{
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTDouble &expr)
{
    // create a unique name for each ast leaf so we can reuse subexpressions
    std::stringstream ss;
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTIdentifier &expr)
{
    ...
    {
        ASCENT_ERROR(
            "Expression identifier: needs a non-zero number of entries: "
            << entries);
    }
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTNamedExpression &expr)
{
    return expr.value->accept(this);
}

//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTMethodCall &call)
{
    ...
    conduit::Node *f_table =
        w.registry().fetch<conduit::Node>("function_table");
    ...
    for (int i = 0; i < overload_list.number_of_children(); ++i)
    {
        ...
        const std::vector<std::string> func_arg_names =
            func["args"].child_names();
    ...
    if (matched_index != -1)
    {
        ...
        if (func.has_path("jitable"))
        {
            ...
            for (auto const &arg : args_map)
            {
                ...
                ss << arg.first << "="
                   << (*arg.second)["filter_name"].as_string();
                first = false;
            }
            ss << ")";
            verbose_name = ss.str();
            ...
            w.graph().add_filter(expressions::register_jit_filter(
                                     w, args_map.size(), exec_policy),
                                 name,
                                 params);
            ...
            for (auto const &arg : args_map)
            {
                // src, dest, port
                w.graph().connect(
                    (*arg.second)["filter_name"].as_string(), name, port);
                ++port;
            }
        }
        else
        {
            ...
            conduit::Node params;
            w.graph().add_filter(
                func["filter_name"].as_string(), name, params);

            // connect up all the arguments
            for (auto const &arg : args_map)
            {
                std::string inp_filter_name =
                    (*arg.second)["filter_name"].as_string();
                // we must to execute inputs that are jitables if the function
                // is not jitable
                    ...
                    if (!subexpr_cache.has_path(jit_execute_name))
                    {
                        ...
                        w.graph().add_filter(
                            expressions::register_jit_filter(
                                ...
                                1,
                                std::make_shared<
                                    const expressions::AlwaysExecutePolicy>()),
                            jit_execute_name,
                            params);
                        // src, dest, port
                        w.graph().connect(
                            inp_filter_name, jit_execute_name, 0);
            ...
            for (std::unordered_set<std::string>::iterator it =
                     opt_args.begin();
                 it != opt_args.end();
                 ++it)
            {
        ...
        // we will need special code (per function) to determine the correct
        // return type
        if (output_type == "anytype")
        {
            // the history function's return type is the type of its first
            // argument
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTIfExpr &expr)
{
    expr.m_condition->accept(this);
    ...
    if (detail::is_field_type(condition_type))
    {
        if ((!detail::is_scalar(if_type) && !detail::is_field_type(if_type)) ||
            (!detail::is_scalar(else_type) &&
             !detail::is_field_type(else_type)))
        {
            ASCENT_ERROR(
                "If the if-condition is a field type then the if and else "
                "branches "
                "must return scalars or field types. condition_type: '"
                << condition_type << "', if_type: '" << if_type
                << "', else_type: '" << else_type << "'.");
        }
        output_type = "jitable";
    }
    else if (condition_type == "bool")
    {
        ...
        {
            ASCENT_ERROR("The return types of the if ("
                         << if_type << ") and else (" << else_type
                         << ") branches must match");
        }
        output_type = if_type;
    }
    ...
    {
        std::stringstream ss;
        ss << "expr_jit_if_" << n_condition["filter_name"].as_string()
           << "_then_" << n_if["filter_name"].as_string() << "_else_"
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTBinaryOp &expr)
{
    // std::cout << "Creating binary operation " << m_op << endl;
    std::string op_str;
    switch (expr.m_op)
    ...
    if (detail::is_math(op_str))
    {
        ...
        {
            ASCENT_ERROR("Unsupported math operation: " << "'" << l_type << " "
                                                        << op_str << " "
                                                        << r_type << "'");
        }
    }
    else if (detail::is_logic(op_str))
    {
        ...
        {
            ASCENT_ERROR("logical operators are only supported on bools and "
                         "field types: "
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTString &expr)
{
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTBoolean &expr)
{
    bool value = false;
    switch (expr.tok)
    {
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTArrayAccess &expr)
{
    expr.array->accept(this);
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTDotAccess &expr)
{
    expr.obj->accept(this);
    ...
    if (o_table->has_path(expr.name + "/jitable") || output_type == "jitable")
    {
        ...
        w.graph().add_filter(
            expressions::register_jit_filter(w, 1, exec_policy),
            f_name,
            params);
        // src, dest, port
    ...
    {
        std::stringstream ss;
        ss << "expr_dot_(" << input_obj["filter_name"].as_string() << ")__"
           << expr.name;
        verbose_name = ss.str();
        if (subexpr_cache.has_path(verbose_name))
        {
...
//-----------------------------------------------------------------------------
void BuildGraphVisitor::visit(const ASTExpressionList &list)
{
    ...
    conduit::Node params;
    w.graph().add_filter(
        expressions::register_expression_list_filter(w, list_size),
        name,
        params);
