    ...
    if (topo_type == "unstructured")
    {
        ...
        else if (shape == "polyhedral")
        {
            const std::string &subelement_shape =
                domain["topologies/" + topo_name + "/subelements/shape"]
                    .as_string();
            if (subelement_shape != "polygonal")
            {
                // shape_size becomes the number of vertices for the
                // subelements
...
void TopologyCode::element_idx(InsertionOrderedSet<std::string> &code) const
{
...
void TopologyCode::structured_vertices(
    ...
    if (num_dims == 1)
    {
        code.insert(
            {topo_name + "_vertices[0] = " + topo_name + "_element_idx[0];\n",
             topo_name + "_vertices[1] = " + topo_name +
                 "_vertices[0] + 1;\n"});
    }
    else if (num_dims == 2)
    {
        code.insert(
            {topo_name + "_vertices[0] = " + topo_name + "_element_idx[1] * " +
             ...
             topo_name + "_vertices[2] = " + topo_name + "_vertices[1] + " +
                 topo_name + "_dims_i;\n",
             topo_name + "_vertices[3] = " + topo_name +
                 "_vertices[2] - 1;\n"});
    }
    else if (num_dims == 3)
    {
...
void TopologyCode::structured_vertex_locs(
    InsertionOrderedSet<std::string> &code) const
{
    if (topo_type != "structured")
    {
        ASCENT_ERROR(
            "The function structured_vertex_locs only supports structured "
            "topologies.");
    }
...
void TopologyCode::unstructured_vertices(
    ...
    const std::string &index_name) const
{
    ...
    if (shape_size == -1)
    {
        // TODO generate vertices array for multi-shapes case, it's variable
        // length so might have to find the max shape size before hand and pass
        // it in
    }
    else
    {
...
void TopologyCode::unstructured_vertex_locs(
    InsertionOrderedSet<std::string> &code,
    const std::string &index_name) const
{
    if (topo_type != "unstructured")
    ...
    {
        // multiple shapes
        code.insert({"int " + topo_name + "_shape_size = " + topo_name +
                         "_sizes[" + index_name + "];\n",
                     "int " + topo_name + "_offset = " + topo_name +
                         "_offsets[" + index_name + "];\n",
    ...
    else
    {
        ...
        for (int i = 0; i < shape_size; ++i)
        {
            vertex_xyz(
                ...
                array_code.index(topo_name + "_vertices", std::to_string(i)),
                false,
                array_code.index(topo_name + "_vertex_locs",
                                 std::to_string(i)),
                false);
        }
    }
...
void TopologyCode::element_coord(InsertionOrderedSet<std::string> &code,
                                 ...
                                 const bool declare) const
{
    ...
    else if (topo_type == "rectilinear")
    {
        code.insert(
            ...
            " + " +
            array_code.index(
                topo_name + "_coords", my_index_name + " + 1", coord) +
            ") / 2.0;\n");
    }
    ...
    else if (topo_type == "unstructured")
    {
        ...
        {
            // multiple shapes
            // This will generate 3 for loops if we want to calculate
            // element_xyz If this is an issue we can make a special case for
            // it in element_xyz
            InsertionOrderedSet<std::string> for_loop;
            for_loop.insert(
                {"for(int i = 0; i < " + topo_name + "_shape_size; ++i)\n",
    ...
    {
        ASCENT_ERROR("Cannot get element_coord for topology of type '"
                     << topo_type << "'.");
    }
}

void TopologyCode::element_xyz(InsertionOrderedSet<std::string> &code) const
{
    ...
    {
        ASCENT_ERROR(
            "Cannot get element location for unstructured topology with "
            << num_dims << " dimensions.");
    }
}

void TopologyCode::vertex_idx(InsertionOrderedSet<std::string> &code) const
{
    if (topo_type == "uniform" || topo_type == "rectilinear" ||
        topo_type == "structured")
    {
        code.insert({"int " + topo_name + "_vertex_idx[" +
                         std::to_string(num_dims) + "];\n",
                     topo_name + "_vertex_idx[0] = item % (" + topo_name +
                         "_dims_i);\n"});
        if (num_dims >= 2)
        {
    ...
    else
    {
        ...
        // vertex_idx[2] = item
        ASCENT_ERROR(
            "vertex_idx does not need to be calculated for unstructured "
            "topologies.");
    }
}

void TopologyCode::vertex_coord(InsertionOrderedSet<std::string> &code,
                                const std::string &coord,
                                const std::string &index_name,
                                const std::string &res_name,
                                const bool declare) const
{
    ...
    if (index_name.empty())
    {
        ...
        {
            vertex_idx(code);
            my_index_name = topo_name + "_vertex_idx[" +
                            std::to_string(coord[0] - 'x') + "]";
        }
        else
        {
            my_index_name = "item";
...
void TopologyCode::vertex_xyz(InsertionOrderedSet<std::string> &code) const
{
    if (topo_type == "uniform" || topo_type == "rectilinear")
    {
        vertex_idx(code);
...
void TopologyCode::dxdydz(InsertionOrderedSet<std::string> &code) const
{
    ...
    element_idx(code);
    code.insert(
        "const double " + topo_name + "_dx = " +
        array_code.index(
            topo_name + "_coords", topo_name + "_element_idx[0] + 1", "x") +
        " - " +
        array_code.index(
...
void TopologyCode::hexahedral_volume(InsertionOrderedSet<std::string> &code,
...
// ||(p3-p0) (p2-p0) (p1-p0)|| / 6
void TopologyCode::tetrahedral_volume(InsertionOrderedSet<std::string> &code,
                                      const std::string &vertex_locs,
                                      const std::string &res_name) const
{
    math_code.vector_subtract(
...
void print_vector(InsertionOrderedSet<std::string> &code,
                  const std::string v,
                  const int num_dims)
{
    std::stringstream ss;
...
void print_int_vector(InsertionOrderedSet<std::string> &code,
                      const std::string v,
                      const int num_dims)
{
    std::stringstream ss;
...
// 1/2 * |(p2 - p0) X (p3 - p1)|
void TopologyCode::quadrilateral_area(InsertionOrderedSet<std::string> &code,
                                      const std::string &p0,
                                      const std::string &p1,
                                      const std::string &p2,
                                      const std::string &p3,
                                      const std::string &res_name) const
{
    ...
    if (num_dims == 3)
    {
        ...
        math_code.cross_product(code,
                                ...
                                num_dims);
        math_code.magnitude(
            code, res_name + "_cross", res_name + "_cross_mag", 3);
        code.insert("const double " + res_name + " = " + res_name +
                    "_cross_mag / 2.0;\n");
    }
    else if (num_dims == 2)
    {
        ...
        code.insert("const double " + res_name + " = abs((" + res_name +
                    "_2m0[0] * " + res_name + "_3m1[1] - " + res_name +
                    "_2m0[1] * " + res_name + "_3m1[0] ) / 2.0); \n");
    }
}
...
// 1/2 * |(p1 - p0) X (p2 - p0)|
void TopologyCode::triangle_area(InsertionOrderedSet<std::string> &code,
                                 const std::string &p0,
                                 const std::string &p1,
                                 const std::string &p2,
                                 const std::string &res_name) const
{
    ...
    else if (num_dims == 2)
    {
        ...
        // since I am not currently sure how to ensure the winding order
        code.insert("const double " + res_name + " = abs((" + res_name +
                    "_1m0[0] * " + res_name + "_2m0[1] - " + res_name +
                    "_1m0[1] * " + res_name + "_2m0[0] ) / 2.0); \n");
    }
}
...
void TopologyCode::polygon_area_vec(InsertionOrderedSet<std::string> &code,
                                    const std::string &vertex_locs,
                                    const std::string &res_name) const
{
...
void TopologyCode::polygon_area(InsertionOrderedSet<std::string> &code,
                                const std::string &vertex_locs,
                                const std::string &res_name) const
{
...
void TopologyCode::volume(InsertionOrderedSet<std::string> &code) const
{
    if (topo_type == "uniform")
    {
        code.insert("const double " + topo_name + "_volume = " + topo_name +
                    "_spacing_dx * " + topo_name + "_spacing_dy * " +
                    topo_name + "_spacing_dz;\n");
    }
    else if (topo_type == "rectilinear")
    {
        dxdydz(code);
    ...
    {
        ASCENT_ERROR("Unsupported topo_type '" << topo_type
                                               << "' for volume calculation");
    }
}

void TopologyCode::hexahedral_surface_area(
    InsertionOrderedSet<std::string> &code,
    const std::string &vertex_locs,
    const std::string &res_name) const
{
    ...
    code.insert("const double " + res_name + " = " + res_name + "_nx + " +
                res_name + "_px + " + res_name + "_ny + " + res_name +
                "_py + " + topo_name + "_area_nz + " + res_name + "_pz;\n");
}

void TopologyCode::tetrahedral_surface_area(
    InsertionOrderedSet<std::string> &code,
    const std::string &vertex_locs,
    const std::string &res_name) const
{
    ...
    code.insert("const double " + res_name + " = " + res_name + "_f0 + " +
                res_name + "_f1 + " + res_name + "_f2 + " + res_name +
                "_f3;\n");
}

void TopologyCode::area(InsertionOrderedSet<std::string> &code) const
{
    if (num_dims != 2)
    {
...
void TopologyCode::surface_area(InsertionOrderedSet<std::string> &code) const
{
    if (topo_type == "uniform")
    {
        code.insert("const double " + topo_name + "_area = 2.0 * (" +
                    topo_name + "_spacing_dx * " + topo_name +
                    "_spacing_dy + " + topo_name + "_spacing_dx * " +
                    topo_name + "_spacing_dz + " + topo_name +
    ...
    else if (topo_type == "rectilinear")
    {
        dxdydz(code);
        code.insert("const double " + topo_name + "_area = 2.0 * (" +
                    topo_name + "_dx * " + topo_name + "_dy + " + topo_name +
                    "_dx * " + topo_name + "_dz + " + topo_name + "_dy * " +
                    topo_name + "_dz);\n");
    }
    else if (topo_type == "structured")
    {
    ...
    {
        ASCENT_ERROR("surface_area for topology type '"
                     << topo_type << "' is not implemented.");
    }
}
// }}}
