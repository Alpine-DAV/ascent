...
void vector_op(const double lhs[3],
               const double rhs[3],
               const std::string &op,
               double res[3])
{
...
template <> int math_op(const int lhs, const int rhs, const std::string &op)
{
    int res;
    if (op == "+")
    {
                              ...
                              int &last_index)
{
    ...
    if (absolute || relative)
    {
        ...
        if (first_index < 0 || last_index < 0)
        {
            ASCENT_ERROR(operator_name +
                         ": the first index and last index must both be "
        ...
        if (first_index > last_index)
        {
            ASCENT_ERROR(
                operator_name +
                ": the first index must not be greater than the last index.");
        }
        ...
        if (relative)
        {
            ...
            // when retrieving from m to n cycles ago, where m < n, n will have
            // a lower history index than m
    ...
    else if (simulation_time)
    {
        ...
        if (first_time < 0 || last_time < 0)
        {
            ASCENT_ERROR(
                operator_name +
                    ": the first_absolute_time and last_absolute_time "
        ...
        {
            ASCENT_ERROR(
                operator_name + ": the first_absolute_time must not be "
        ...
        for (int index = 0; index < entries; index++)
        {
            ...
            else
            {
                ASCENT_ERROR(
                    operator_name
                    << ": internal error. missing " << time_path
                    << " value for time point in retrieval window (for the"
                    << " calculation at absolute index: " + to_string(index) +
                           ").");
            }

            // I am not totally sure about this logic. Part of the problem is
            // that we haven't fully specified what we want this behavior to
            // be.
    ...
    else if (simulation_cycle)
    {
        ...
        {
            ASCENT_ERROR(operator_name +
                         ": the first_absolute_cycle and last_absolute_cycle "
                         "must both be non-negative.");
        }
        ...
        {
            ASCENT_ERROR(operator_name +
                         ": the first_absolute_cycle must not be greater than "
                         "the last_absolute_cycle.");
        }
        ...
        {
            ASCENT_ERROR(
                operator_name + ": internal error. number of history entries: "
                << to_string(entries)
                             ...
                             conduit::Node *output)
{
    ...
    {
        ASCENT_ERROR(operator_name + " internal error. first index does not "
    ...
    {
        ASCENT_ERROR(operator_name + ": unsupported array type "
                     << dtype.to_string());
    }
    (*output)["type"] = "array";
    ...
    if (gradient)
    {
        ...
        else if (return_simulation_time)
        {
            ...
            for (int i = 0; i < return_size - 1; ++i)
            {
                simulation_time_array[i] =
                    history.child(first_index + i + 1)["time"].to_float64() -
                    history.child(first_index + i)["time"].to_float64();
            }
        ...
        else if (return_simulation_cycle)
        {
            ...
            for (int i = 0; i < return_size - 1; ++i)
            {
                cycle_array[i] = stoll(child_names[first_index + i + 1]) -
                                 stoll(child_names[first_index + i]);
            }
...
conduit::Node *range_values_helper(
    const conduit::Node &history,
    ...
    const std::string time_units = "") // 'cycle' 'index' 'time'
{
    ...
    bool absolute = (!n_first_absolute_index->dtype().is_empty() ||
                     !n_last_absolute_index->dtype().is_empty());
    bool relative = (!n_first_relative_index->dtype().is_empty() ||
                     !n_last_relative_index->dtype().is_empty());
    bool simulation_time = (!n_first_absolute_time->dtype().is_empty() ||
                            !n_last_absolute_time->dtype().is_empty());
    bool simulation_cycle = (!n_first_absolute_cycle->dtype().is_empty() ||
                             !n_last_absolute_cycle->dtype().is_empty());
    ...
    {
        ASCENT_ERROR(operator_name
                     << ": Must specify a selection range, providing either "
    ...
    if (count > 1)
    {
        ASCENT_ERROR(
            operator_name
            << ": Must specify exactly one selection range, providing either "
            << "1). first_absolute_index and last_absolute_index, "
            << "2). first_relative_index and last_relative_index, "
            << "3). first_absolute_time and last_absolute_time, or "
            << "4). first_absolute_cycle and last_absolute_cycle.");
    ...
    if (entries <= 0)
    {
        ASCENT_ERROR(operator_name + ": no entries collected for expression.");
    }
    ...
    if (!n_first_index->has_path("value"))
    {
        ASCENT_ERROR(
            operator_name +
            ": internal error. first_index does not have child value");
    }
    if (!n_last_index->has_path("value"))
    {
        ASCENT_ERROR(operator_name +
                     ": internal error. last_index does not have child value");
    }
                       ...
                       conduit::Node &n_output_axes)
{
    ...
    // verify reduction_op
    if (reduction_op != "sum" && reduction_op != "min" &&
        reduction_op != "max" && reduction_op != "avg" &&
        reduction_op != "pdf" && reduction_op != "std" &&
    ...
    //   #ifdef ASCENT_MPI_ENABLED
    //     MPI_Comm mpi_comm =
    //     MPI_Comm_f2c(flow::Workspace::default_mpi_comm());
...
//-----------------------------------------------------------------------------
ExprBoolean::ExprBoolean() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprBoolean::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprBoolean::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprInteger::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprInteger::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprDouble::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprDouble::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprString::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprString::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprNan::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprNan::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    bool res = true;
    return res;
...
//-----------------------------------------------------------------------------
void ExprNull::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprNull::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    bool res = true;
    return res;
...
//-----------------------------------------------------------------------------
ExprIdentifier::ExprIdentifier() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprIdentifier::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprIdentifier::verify_params(const conduit::Node &params,
                                   conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprObjectDotAccess::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprObjectDotAccess::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
    bool res = true;
    ...
    {
        info["errors"].append() =
            "DotAccess: Missing required parameter 'name'";
        res = false;
    }
    return res;
}
...
//-----------------------------------------------------------------------------
void ExprObjectDotAccess::execute()
{
...
//-----------------------------------------------------------------------------
void ExprIf::declare_interface(Node &i)
{
    i["type_name"] = "expr_if";
...
//-----------------------------------------------------------------------------
bool ExprIf::verify_params(const conduit::Node &params, conduit::Node &info)
{
    info.reset();
    bool res = true;
    return res;
...
//-----------------------------------------------------------------------------
ExprBinaryOp::ExprBinaryOp() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprBinaryOp::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprBinaryOp::verify_params(const conduit::Node &params,
                                 conduit::Node &info)
{
    info.reset();
    bool res = true;
    ...
    {
        info["errors"].append() =
            "Missing required string parameter 'op_string'";
        res = false;
    }
    return res;
}
    ...
    if (detail::is_math(op_str))
    {
        ...
        else
        {
            if (detail::is_scalar(l_type) != detail::is_scalar(r_type))
            {
                ...
                ASCENT_ERROR("Mixed vector and scalar quantities not "
...
//-----------------------------------------------------------------------------
void ExprScalarMin::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarMin::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprScalarMax::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarMax::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprScalarAbs::ExprScalarAbs() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprScalarAbs::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarAbs::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprScalarExp::ExprScalarExp() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprScalarExp::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarExp::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprScalarLog::ExprScalarLog() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprScalarLog::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarLog::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprScalarPow::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprScalarPow::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprVector::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprVector::verify_params(const conduit::Node &params,
                               conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprVectorMagnitude::ExprVectorMagnitude() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprVectorMagnitude::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprVectorMagnitude::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprVectorMagnitude::execute()
{
    const conduit::Node *arg1 = input<Node>("arg1");
...
//-----------------------------------------------------------------------------
ExprArrayAccess::ExprArrayAccess() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayAccess::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayAccess::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
    bool res = true;
...
void ExprArrayAccess::execute()
{
    ...
    if (index > length - 1)
    {
        ASCENT_ERROR("ArrayAccess: array index out of bounds: [0,"
                     << length - 1 << "]");
    }
...
//-----------------------------------------------------------------------------
ExprArrayReplace::ExprArrayReplace() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayReplace::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayReplace::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprArrayReductionMin::ExprArrayReductionMin() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayReductionMin::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayReductionMin::verify_params(const conduit::Node &params,
                                          conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprArrayReductionMin::execute()
{
    conduit::Node *output = new conduit::Node();
    std::string exec = ExecutionManager::preferred_cpu_policy();
    (*output)["value"] =
        array_min((*input<Node>("arg1"))["value"], exec)["value"];
    (*output)["type"] = "double";
...
//-----------------------------------------------------------------------------
ExprArrayReductionMax::ExprArrayReductionMax() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayReductionMax::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayReductionMax::verify_params(const conduit::Node &params,
                                          conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprArrayReductionMax::execute()
{
    conduit::Node *output = new conduit::Node();
    std::string exec = ExecutionManager::preferred_cpu_policy();
    (*output)["value"] =
        array_max((*input<Node>("arg1"))["value"], exec)["value"];
    (*output)["type"] = "double";
...
//-----------------------------------------------------------------------------
ExprArrayReductionAvg::ExprArrayReductionAvg() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayReductionAvg::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayReductionAvg::verify_params(const conduit::Node &params,
                                          conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprArrayReductionAvg::execute()
{
    conduit::Node *output = new conduit::Node();
    std::string exec = ExecutionManager::preferred_cpu_policy();
...
//-----------------------------------------------------------------------------
ExprArrayReductionSum::ExprArrayReductionSum() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprArrayReductionSum::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprArrayReductionSum::verify_params(const conduit::Node &params,
                                          conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprArrayReductionSum::execute()
{
    conduit::Node *output = new conduit::Node();
    std::string exec = ExecutionManager::preferred_cpu_policy();
    (*output)["value"] =
        array_sum((*input<Node>("arg1"))["value"], exec)["value"];
    (*output)["type"] = "double";
...
//-----------------------------------------------------------------------------
ExprHistory::ExprHistory() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistory::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistory::verify_params(const conduit::Node &params,
                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistory::execute()
{
    conduit::Node *output = new conduit::Node();

    const std::string expr_name =
        (*input<Node>("expr_name"))["name"].as_string();
    ...
    if (!n_relative_index->dtype().is_empty())
    {
        ...
        {
            ASCENT_ERROR(
                "History: relative_index must be a non-negative integer.");
        }
    ...
    else
    {
        ...
        {
            ASCENT_ERROR("History: internal error. absolute index does not "
                         "have child value");
        }
        ...
        {
            ASCENT_ERROR("History: found only "
                         << entries << " entries, cannot get entry at "
                         << absolute_index);
        }
        if (absolute_index < 0)
        {
            ASCENT_ERROR(
                "History: absolute_index must be a non-negative integer.");
        }
...
//-----------------------------------------------------------------------------
ExprHistoryRange::ExprHistoryRange() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistoryRange::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistoryRange::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistoryRange::execute()
{
    const string operator_name = "HistoryRange";
    const std::string expr_name =
        (*input<conduit::Node>("expr_name"))["name"].as_string();
    ...
    const conduit::Node *n_first_absolute_index =
        input<conduit::Node>("first_absolute_index");
    const conduit::Node *n_last_absolute_index =
        input<conduit::Node>("last_absolute_index");
    const conduit::Node *n_first_relative_index =
        input<conduit::Node>("first_relative_index");
    const conduit::Node *n_last_relative_index =
        input<conduit::Node>("last_relative_index");
    const conduit::Node *n_first_absolute_time =
        input<conduit::Node>("first_absolute_time");
    const conduit::Node *n_last_absolute_time =
        input<conduit::Node>("last_absolute_time");
    const conduit::Node *n_first_absolute_cycle =
        input<conduit::Node>("first_absolute_cycle");
    const conduit::Node *n_last_absolute_cycle =
        input<conduit::Node>("last_absolute_cycle");
...
//-----------------------------------------------------------------------------
ExprHistoryGradient::ExprHistoryGradient() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistoryGradient::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistoryGradient::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistoryGradient::execute()
{
    conduit::Node *output = new conduit::Node();
    const std::string expr_name =
        (*input<Node>("expr_name"))["name"].as_string();
    ...
    {
        ASCENT_ERROR(
            "HistoryGradient: if a ``window_length_unit`` value is provided,"
            << " it must be set to either: 1). \"index\", 2). \"time\", or "
               "3). \"cycle\".");
    }

    if ((execution_points || cycles) && window_length < 1)
    {
        ASCENT_ERROR("HistoryGradient: window_length must be at least 1 if "
                     "the window length unit is \"index\" or \"cycle\".");
    }
    ...
    else if (time)
    {
        ...
        {
            ASCENT_ERROR("HistoryGradient: internal error. current time point "
                         "does not have the child " +
                         time_path);
        }
        const double current_time =
            history.child(current_index)[time_path].to_float64();
        ...
        for (int index = 0; index < entries; index++)
        {
            ...
            {
                ASCENT_ERROR(
                    "HistoryGradient: a time point in evaluation window (for "
                    "the calculation at absolute index: " +
                    to_string(index) + ") does not have the child " +
                    time_path);
            }
            if (time >= first_time)
            {
                first_index = index;
                // adjust so our window length is accurate (since we may not
                // have performed a calculation at precisely the requested
                // time)
    ...
    else if (cycles)
    {
        ...
        const unsigned long long current_cycle =
            stoull(child_names[current_index]);
    ...
    double current_value =
        history.child(current_index)[value_path].to_float64();
...
//-----------------------------------------------------------------------------
ExprHistoryGradientRange::ExprHistoryGradientRange() : Filter()
{
    // empty
}
...
// -----------------------------------------------------------------------------
void ExprHistoryGradientRange::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistoryGradientRange::verify_params(const conduit::Node &params,
                                             conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistoryGradientRange::execute()
{
    ...
    conduit::Node gradient =
        history_gradient_range((*output)["value"], (*output)["time"]);
...
//-----------------------------------------------------------------------------
ExprHistogram::ExprHistogram() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistogram::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogram::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprHistogramEntropy::ExprHistogramEntropy() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistogramEntropy::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramEntropy::verify_params(const conduit::Node &params,
                                         conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistogramEntropy::execute()
{
...
//-----------------------------------------------------------------------------
void ExprHistogramPDF::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramPDF::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistogramCDF::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramCDF::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprHistogramCDFQuantile::ExprHistogramCDFQuantile() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistogramCDFQuantile::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramCDFQuantile::verify_params(const conduit::Node &params,
                                             conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistogramCDFQuantile::execute()
{
    const conduit::Node *n_cdf = input<conduit::Node>("cdf");
    const conduit::Node *n_val = input<conduit::Node>("q");
    // optional inputs
    const conduit::Node *n_interpolation =
        input<conduit::Node>("interpolation");
    ...
    if (!n_interpolation->dtype().is_empty())
    {
        ...
        {
            ASCENT_ERROR(
                "Known interpolation types are: linear, lower, higher, "
                "midpoint, nearest");
        }
    }
...
//-----------------------------------------------------------------------------
ExprHistogramBinByIndex::ExprHistogramBinByIndex() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistogramBinByIndex::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramBinByIndex::verify_params(const conduit::Node &params,
                                            conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistogramBinByIndex::execute()
{
    ...
    {
        ASCENT_ERROR(
            "BinByIndex: bin index must be within the bounds of hist [0, "
            << num_bins - 1 << "]");
    }
...
//-----------------------------------------------------------------------------
ExprHistogramBinByValue::ExprHistogramBinByValue() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprHistogramBinByValue::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprHistogramBinByValue::verify_params(const conduit::Node &params,
                                            conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprHistogramBinByValue::execute()
{
...
//-----------------------------------------------------------------------------
ExprMeshCycle::ExprMeshCycle() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshCycle::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshCycle::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshTime::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshTime::verify_params(const conduit::Node &params,
                                 conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprMeshField::ExprMeshField() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshField::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshField::verify_params(const conduit::Node &params,
                                  conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshTopology::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshTopology::verify_params(const conduit::Node &params,
                                     conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
ExprMeshBounds::ExprMeshBounds() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBounds::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBounds::verify_params(const conduit::Node &params,
                                   conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshLineout::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshLineout::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
    bool res = true;
...
void ExprMeshLineout::execute()
{
    ...
    for (int v = 0; v < var_size; ++v)
    {
        ...
        (*output)["attrs/vars/" + var + "/value"] =
            conduit::DataType::float64(size);
        (*output)["attrs/vars/" + var + "/type"] = "array";
        float64_array var_array =
            (*output)["attrs/vars/" + var + "/value"].value();
        for (int i = 0; i < size; ++i)
        {
...
//-----------------------------------------------------------------------------
ExprMeshFieldReductionMin::ExprMeshFieldReductionMin() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionMin::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionMin::verify_params(const conduit::Node &params,
                                              conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionMin::execute()
{
    const conduit::Node *arg1 = input<Node>("arg1");
...
//-----------------------------------------------------------------------------
ExprMeshFieldReductionMax::ExprMeshFieldReductionMax() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionMax::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionMax::verify_params(const conduit::Node &params,
                                              conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionMax::execute()
{
    ...
    if (!is_scalar_field(*dataset, field))
    {
        ASCENT_ERROR("FieldMax: field '" << field
...
//-----------------------------------------------------------------------------
ExprMeshFieldReductionAvg::ExprMeshFieldReductionAvg() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionAvg::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionAvg::verify_params(const conduit::Node &params,
                                              conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionAvg::execute()
{
    const conduit::Node *arg1 = input<Node>("arg1");
    ...
    if (!is_scalar_field(*dataset, field))
    {
        ASCENT_ERROR("FieldAvg: field '" << field
                                         << "' is not a scalar field");
    }
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionSum::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionSum::verify_params(const conduit::Node &params,
                                              conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionSum::execute()
{
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionNanCount::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionNanCount::verify_params(const conduit::Node &params,
                                                   conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionNanCount::execute()
{
    std::string field = (*input<Node>("arg1"))["value"].as_string();
...
//-----------------------------------------------------------------------------
ExprMeshFieldReductionInfCount::ExprMeshFieldReductionInfCount() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionInfCount::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshFieldReductionInfCount::verify_params(const conduit::Node &params,
                                                   conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshFieldReductionInfCount::execute()
{
    std::string field = (*input<Node>("arg1"))["value"].as_string();
...
//-----------------------------------------------------------------------------
ExprMeshBinning::ExprMeshBinning() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBinning::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBinning::verify_params(const conduit::Node &params,
                                    conduit::Node &info)
{
    info.reset();
    bool res = true;
...
void ExprMeshBinning::execute()
{
    ...
    // optional arguments
    const conduit::Node *n_empty_bin_val =
        input<conduit::Node>("empty_bin_val");
...
//-----------------------------------------------------------------------------
ExprMeshBinningAxis::ExprMeshBinningAxis() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBinningAxis::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBinningAxis::verify_params(const conduit::Node &params,
                                        conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshBinningAxis::execute()
{
    ...
    if (!is_scalar_field(*dataset, name) && !is_xyz(name))
    {
        ...
        ASCENT_ERROR("Binning Axis: Axes must be scalar fields or x/y/z. "
                     "Dataset does not "
    ...
    if (!n_bins_list_obj->dtype().is_empty())
    {
        ...
        {
            ASCENT_ERROR("Binning Axis: Only pass in arguments for uniform or "
                         "rectilinear "
                         "binning, not both.");
        }
        ...
        for (int i = 0; i < bins_len; ++i)
        {
            ...
            {
                delete output;
                ASCENT_ERROR("Binning Axis: bin extents must be strictly "
                             "increasing scalars.");
            }
        }
    }
...
//-----------------------------------------------------------------------------
ExprMeshBinningBinByIndex::ExprMeshBinningBinByIndex() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBinningBinByIndex::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBinningBinByIndex::verify_params(const conduit::Node &params,
                                              conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshBinningBinByIndex::execute()
{
...
//-----------------------------------------------------------------------------
ExprMeshBinningPointAndAxis::ExprMeshBinningPointAndAxis() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBinningPointAndAxis::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBinningPointAndAxis::verify_params(const conduit::Node &params,
                                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshBinningPointAndAxis::execute()
{
    conduit::Node &in_binning = *input<Node>("binning");
...
//-----------------------------------------------------------------------------
ExprMeshBinningMaxFromPoint::ExprMeshBinningMaxFromPoint() : Filter()
{
    // empty
}
...
//-----------------------------------------------------------------------------
void ExprMeshBinningMaxFromPoint::declare_interface(Node &i)
{
...
//-----------------------------------------------------------------------------
bool ExprMeshBinningMaxFromPoint::verify_params(const conduit::Node &params,
                                                conduit::Node &info)
{
    info.reset();
    bool res = true;
...
//-----------------------------------------------------------------------------
void ExprMeshBinningMaxFromPoint::execute()
{
    conduit::Node &in_binning = *input<Node>("binning");
    conduit::Node &in_axis = *input<Node>("axis");
    conduit::Node &in_point = *input<Node>("point");
