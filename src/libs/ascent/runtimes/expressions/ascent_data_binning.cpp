                        ...
                        const std::string component)
{
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        if (dom.has_path("fields/" + field_name))
        {
            ...
            {
                n_error.append() =
                    "specified a component name but no name exists";
            }
            else
            {
                std::vector<std::string> comp_names =
                    field["values"].child_names();
                for (int name_idx = 0; name_idx < comp_names.size();
                     ++name_idx)
                {
                            ...
                            int &total_bins)
{
    ...
    {
        num_bins *=
            axes.child(axis_index)["bins"].dtype().number_of_elements() - 1;
    }
    ...
    if (reduction_op == "var" || reduction_op == "std")
    {
        num_bin_vars = 3;
             ...
             reduction_op == "pdf" || reduction_op == "count")
    {
        num_bin_vars = 1;
    }
...
conduit::Node verify_topo_and_assoc(const conduit::Node &dataset,
                                    const std::vector<std::string> var_names)
{
    ...
    if (assoc_str.empty())
    {
        ...
        error_msg.append() =
            "Could not determine the association from the given "
...
conduit::Node create_bins_axes(conduit::Node &bin_axes,
                               ...
                               std::string topo_name)
{
    ...
    for (int i = 0; i < num_axes; ++i)
    {
        ...
        if (min_val > max_val)
        {
            ASCENT_ERROR("Binning: axis '"
                         << axis_name << "' has invalid bounds "
                 ...
                 Exec)
{
    ...
    double *bins_node_ptr =
        const_cast<double *>(axis["bins"].as_float64_ptr());
    Array<double> bins(bins_node_ptr,
    ...
    ascent::forall<for_policy>(
        ...
        [=] ASCENT_LAMBDA(index_t i)
        {
            ...
            bindex_ptr[i] = bindex;
            // std::cout << "final bindex for " << i << " " << bindex_ptr[i] <<
            // std::endl;
        });
...
Array<double> cast_to_float64(conduit::Node &field,
                              const std::string component)
{
    const std::string mem_space = Exec::memory_space;
...
template <typename Exec>
Array<double>
cast_field_values(conduit::Node &field, const std::string component, Exec)
{
    Array<double> res;
    ...
    {
        // This error is a bad idea, one rank might have this field and others
        // not
                       ...
                       Exec)
{
    ...
    ascent::forall<for_policy>(0,
                               ...
                               [=] ASCENT_LAMBDA(index_t i)
                               {
                                   ...
                                   // calc location of bin result
                                   const int src_index =
                                       i * src_stride + src_offset;
                                   const int dest_index =
                                       i * des_stride + des_offset;
...
struct BinningFunctor
{
    ...
    BinningFunctor(std::map<int, Array<int>> &bindexes,
                   ...
                   Array<double> &bins,
                   const std::string op)
        : m_bindexes(bindexes), m_values(values), m_bins(bins), m_op(op)
    {
    ...
    template <typename Exec> void operator()(const Exec &)
    {
        ...
        for (auto dom_id : m_domain_ids)
        {
            ...
            const int *bindex_ptr =
                m_bindexes[dom_id].get_ptr_const(Exec::memory_space);
            const double *values_ptr =
                m_values[dom_id].get_ptr_const(Exec::memory_space);
                ...
                ascent::forall<for_policy>(
                    ...
                    [=] ASCENT_LAMBDA(index_t i)
                    {
                        ...
                        if (bindex >= 0)
                        {
                            ...
                            ascent::atomic_min<atomic_policy>(
                                bins_ptr + bindex, value);
                        }
                    });
            }
            else if (m_op == "max")
            {
                ascent::forall<for_policy>(
                    ...
                    [=] ASCENT_LAMBDA(index_t i)
                    {
                        ...
                        {
                            const double value = values_ptr[i];
                            ascent::atomic_max<atomic_policy>(
                                bins_ptr + bindex, value);
                        }
                    });
            }
            else if (m_op == "count")
            {
                ascent::forall<for_policy>(
                    ...
                    [=] ASCENT_LAMBDA(index_t i)
                    {
                        ...
                        // bindex" << bindex << std::endl;
                        if (bindex >= 0)
                        {
                            ascent::atomic_add<atomic_policy>(
                                bins_ptr + bindex, 1.);
                        }
                    });
            }
            ...
            {
                // use basic 2-pass, since single pass sum of squares method is
                ...
                ascent::forall<for_policy>(
                    ...
                    [=] ASCENT_LAMBDA(index_t i)
                    {
                        ...
                        if (bindex >= 0)
                        {
                            const double value = values_ptr[i];
                            const int offset = bindex * 3;
                            const double val_avg =
                                bins_ptr[offset] / bins_ptr[offset + 1];
                            // (v - mean) ^ 2
                            double val_cal = (value - val_avg);
                            val_cal = val_cal * val_cal;
                            ascent::atomic_add<atomic_policy>(
                                bins_ptr + offset + 2, val_cal);
                        }
                    });
            }
            ...
            // {
            //   std::cout<<"bin accumulate results index "<<i<<"
            //   "<<host_ptr[i]<<"\n";
            // }
        }
    }
...
struct SamplesFunctor
{
    ...
    SamplesFunctor(conduit::Node &dataset,
        ...
        : m_axes(axes), m_dataset(dataset), m_topo_name(topo_name),
          m_assoc(assoc), m_component(component),
          m_reduction_var(reduction_var), m_sample_dims(-1), m_sample_points(),
          m_sample_values()
    {
    }
    ...
    template <typename Exec> void operator()(const Exec &)
    {
        ...
        for (int dom_index = 0; dom_index < num_domains; ++dom_index)
        {
            ...
            std::string coords_name =
                dom["topologies"][m_topo_name]["coordset"].as_string();
            // find the coordset dims
            int coords_dims = conduit::blueprint::mesh::coordset::dims(
                dom["coordsets"][coords_name]);
            ...
            for (int axis_index = 0; axis_index < num_axes; ++axis_index)
            {
                ...
                // axis_name << std::endl; case where bin axis is a field
                ...
                {
                    // this is a coordinate axis so we need the spatial
                    // information
                    if (do_once)
                    {
                        do_once = false;
                        ...
                        else
                        {
                            ...
                            // double *sval_ptr =
                            // spatial_values.get_host_ptr();
                            // std::cout<<"*** spatial values
                            // "<<spatial_values.size()<<"\n";
                        }
                    }
            ...
            // // we need to extract the values if we haven't already pulled
            // them out if(!reduction_op_values_found)
            // {
            //    // empty reduction var is supported for count and pdf
            //    if(m_reduction_var != "")
            //    {
            //      conduit::Node &field = dom["fields/"+m_reduction_var];
            //      m_sample_values = cast_field_values(field, m_component,
            //      Exec());
            //    }
            // }
        ...
        for (int dom_index = 0; dom_index < num_domains; ++dom_index)
        {
                ...
                else // this is a spatial axis
                {
                    ...
                    if (do_once)
                    {
                        ...
                        else
                        {
                            ...
                            index_t sidx = 0;
                            for (index_t e_idx = 0; e_idx < homes_size;
                                 e_idx++)
                            {
            ...
            if (!reduction_op_values_found)
            {
                ...
                if (m_reduction_var != "")
                {
                    conduit::Node &field = dom["fields/" + m_reduction_var];
                    m_values[domain_id] =
                        cast_field_values(field, m_component, Exec());
                }
            }
...
void exchange_bins(Array<double> &bins, const std::string op)
{
    ...
    {
        MPI_Allreduce(
            bins_ptr, global_ptr, bins_size, MPI_DOUBLE, MPI_SUM, mpi_comm);
    }
    else if (op == "min")
    {
        MPI_Allreduce(
            bins_ptr, global_ptr, bins_size, MPI_DOUBLE, MPI_MIN, mpi_comm);
    }
    else if (op == "max")
    {
        MPI_Allreduce(
            bins_ptr, global_ptr, bins_size, MPI_DOUBLE, MPI_MAX, mpi_comm);
    }
    bins = global_bins;
#endif
}
...
struct BinningReductionFunctor
{
    ...
    BinningReductionFunctor(conduit::Node &res,
        ...
        : m_res(res), m_bins(bins), m_num_bins(num_bins), m_op(op),
    ...
    template <typename Exec> void operator()(const Exec &)
    {
        ...
        else if (m_op == "pdf")
        {
            ...
            ascent::forall<for_policy>(
                0, size, [=] ASCENT_LAMBDA(index_t i) { sum += bins_ptr[i]; });
            pdf_total = sum.get();
                           ...
                           std::map<int, Array<int>> &bindexes)
{
    ...
    conduit::Node axes =
        detail::create_bins_axes(bin_axes, dataset, topo_name);
    ...
    detail::BindexingFunctor bindexer(
        dataset, axes, topo_name, assoc_str, component, reduction_var);
    ...
    detail::BinningFunctor binner(
        bindexer.m_bindexes, bindexer.m_values, bins, reduction_op);
    ...
    conduit::Node res;
    detail::BinningReductionFunctor reducer(
        res, bins, num_bins, reduction_op, empty_bin_val);
