...
struct UniformCoords
{
    ...
    UniformCoords(const conduit::Node &n_coords) { populate(n_coords); }
...
int get_num_indices(const std::string &shape_type)
{
    int num = 0;
    if (shape_type == "tri")
    {
                         ...
                         std::vector<int> &indices)
{
    ...
    else
    {
        ...
        else
        {
            ...
            indices[0] = (element_index[2] * vert_dims[1] + element_index[1]) *
                             vert_dims[0] +
                         element_index[0];
            indices[1] = indices[0] + 1;
...
conduit::Node get_uniform_vert(const conduit::Node &n_coords, const int &index)
{
...
conduit::Node get_explicit_vert(const conduit::Node &n_coords,
                                const int &index)
{
...
conduit::Node get_rectilinear_vert(const conduit::Node &n_coords,
                                   const int &index)
{
    bool is_float64 = true;
...
// ----------------------  element locations ---------------------------------
conduit::Node get_uniform_element(const conduit::Node &n_coords,
                                  const int &index)
{
...
conduit::Node get_rectilinear_element(const conduit::Node &n_coords,
                                      const int &index)
{
    bool is_float64 = true;
    ...
    if (is_float64)
    {
        ...
        if (dims[2] != 0)
        {
            ...
            vert[2] =
                (z_a[logical_index[2]] + z_a[logical_index[2] + 1]) * 0.5;
        }
    }
    else
    {
...
conduit::Node get_explicit_element(const conduit::Node &n_coords,
...
struct VertexFunctor
{
    ...
    void operator()(MeshType &mesh, const Exec &)
    {
        ...
        // std::cout<<"**** VertexFunctor Mem space
        // "<<Exec::memory_space<<"\n";
        using for_policy = typename Exec::for_policy;
        ascent::forall<for_policy>(0,
                                   size,
                                   [=] ASCENT_LAMBDA(index_t point_idx)
                                   {
                                       double vert[3];
...
struct CentroidFunctor
{
    ...
    void operator()(MeshType &mesh, const Exec &)
    {
        ...
        ascent::forall<for_policy>(
            0,
            size,
            [=] ASCENT_LAMBDA(index_t cell_idx)
            {
                                ...
                                const std::string &field_name)
{
    ...
    if (dataset.number_of_children() > 0)
    {
        ...
        {
            if (dom["fields/" + field_name + "/values"].number_of_children() >
                0)
            {
...
int num_components(const conduit::Node &dataset, const std::string &field_name)
{
    int comps = -1;
    for (int i = 0; i < dataset.number_of_children(); ++i)
    {
        ...
        {
            const conduit::Node &vals =
                dom["fields/" + field_name + "/values"];
                           ...
                           const int component_id)
{
    if (component_id <= 0)
    {
        ASCENT_ERROR("Invalid component_id "
                     << component_id
    ...
    if (component_id >= comps)
    {
        ASCENT_ERROR("Field '" << field_name << "' invalid component_id "
                               << component_id << ". Number of components "
                               << comps);
    }
    ...
    for (int i = 0; i < dataset.number_of_children(); ++i)
    {
        ...
        if (dom.has_path("fields/" + field_name + "/values"))
        {
            res = dom["fields/" + field_name + "/values"]
                      .child(component_id)
                      .name();
        }
    }
    ...
    if (res_set.size() != 1)
    {
        ASCENT_ERROR("Mismatched component names res_set size "
                     << res_set.size());
    }
                              ...
                              const int &num_bins)
{
    ...
    for (int i = 0; i < dataset.number_of_children(); ++i)
    {
        ...
        if (dom.has_path("fields/" + field))
        {
            ...
            conduit::Node res;
            res = field_reduction_histogram(
                dom[path], min_val, max_val, num_bins);
        ...
        for (int field_idx = 0; field_idx < num_fields; field_idx++)
        {
            ...
            if (dom.has_path(path)) // has both
            {
                if (!dom.has_path(output_path)) // setup output path
                {
                    ...
                    {
                        const int vals =
                            dom[path]["values"].dtype().number_of_elements();
                        dom[output_path]["values"].set(
                            conduit::DataType::float32(vals));
                    }
                    else
                    {
                        const int vals =
                            dom[path]["values"].dtype().number_of_elements();
                        dom[output_path]["values"].set(
                            conduit::DataType::float64(vals));
                    }
                }
                else // has output path already
                {
                    // check that the field assoc and topo
                    std::string out_assoc =
                        dom[output_path]["association"].to_string();
                    std::string out_topo =
                        dom[output_path]["topology"].to_string();
                ...
                conduit::Node n_add_res =
                    derived_field_binary_add(tmp_a, tmp_b);
                            ...
                            const std::string &topo_name)
{
    ...
    for (int dom_index = 0; dom_index < dataset.number_of_children();
         ++dom_index)
    {
        ...
        if (topo_type == "uniform")
        {
            ...
            for (int i = 0; i < num_dims; ++i)
            {
                ...
                double spacing =
                    n_coords["spacing/" + axes[i][2]].to_float64();

                min_coords[i] = std::min(min_coords[i], origin);
                max_coords[i] =
                    std::max(min_coords[i], origin + (dim - 1) * spacing);
            }
        }
                 ...
                 topo_type == "unstructured" || topo_type == "points")
        {
            ...
            for (int i = 0; i < num_dims; ++i)
            {
                ...
                min_coords[i] =
                    std::min(min_coords[i],
                             field_reduction_min(n_coords, axes[i][0])["value"]
                                 .as_double());
                max_coords[i] =
                    std::max(max_coords[i],
                             field_reduction_max(n_coords, axes[i][0])["value"]
                                 .as_double());
                // min_coords[i] = std::min(
                //     min_coords[i],
                //     field_reduction_min(n_coords[axis_path])["value"].as_double());
                // max_coords[i] = std::max(
                //     max_coords[i],
                //     field_reduction_max(n_coords[axis_path])["value"].as_double());
            }
        }
        else
        {
                                    ...
                                    const std::vector<std::string> var_names)
{
         ...
         ++dom_index)
    {
        ...
        for (const std::string &var_name : var_names)
        {
            if (dom.has_path("fields/" + var_name))
            {
                ...
                {
                    error_msg.append() =
                        "All Binning fields must have the same association.";
                    error = true;
                }
                ...
                {
                    error_msg.append() =
                        "All Binning fields must have the same topology.";
                    error = true;
                }
            }
        }
    ...
    if (assoc_str.empty())
    {
        ...
        error_msg.append() =
            "Could not determine the associate from the given "
...
conduit::index_t get_bin_index(const conduit::float64 value,
                               const conduit::Node &axis)
{
    ...
    const conduit::index_t bin_index = static_cast<conduit::index_t>(
        (value - axis["min_val"].to_float64()) * inv_delta);
...
//
void populate_homes(const conduit::Node &dom,
                    ...
                    conduit::Node &res)
{
    ...
    for (int axis_index = 0; axis_index < num_axes; ++axis_index)
    {
        ...
        {
            // rectilinear axis
            stride *= bin_axes.child(axis_index)["bins"]
                          .dtype()
                          .number_of_elements() -
                      1;
        }
...
void init_bins(double *bins, const int size, const std::string reduction_op)
{
                      ...
                      const std::string &component)
{
    ...
    for (int axis_index = 0; axis_index < num_axes; ++axis_index)
    {
        ...
        {
            axis["min_val"] =
                field_min(dataset, axis_name)["value"].to_float64();
        }
        ...
        {
            // rectilinear axis
            num_bins *= bin_axes.child(axis_index)["bins"]
                            .dtype()
                            .number_of_elements() -
                        1;
        }
    ...
    if (reduction_op == "sum" || reduction_op == "pdf" ||
        reduction_op == "avg" || reduction_op == "std" ||
        reduction_op == "var" || reduction_op == "rms")
    {
        MPI_Allreduce(
            bins, global_bins, bins_size, MPI_DOUBLE, MPI_SUM, mpi_comm);
    }
    else if (reduction_op == "min")
    {
        MPI_Allreduce(
            bins, global_bins, bins_size, MPI_DOUBLE, MPI_MIN, mpi_comm);
    }
    else if (reduction_op == "max")
    {
        MPI_Allreduce(
            bins, global_bins, bins_size, MPI_DOUBLE, MPI_MAX, mpi_comm);
    }
    delete[] bins;
    bins = global_bins;
#endif
                   ...
                   const std::string field_name)
{
    ...
    if (all_xyz)
    {
        // pick the first topology from the first domain and use the
        // association from the binning
         ...
         ++dom_index)
    {
        ...
        std::string fname = "painted_" + reduction_var + "_" +
                  ...
                  const std::string field_name)
{
    ...
    for (int i = 0; i < num_axes; ++i)
    {
        ...
        {
            // rectilinear
            mesh["coordsets/binning_coords/values/" + axes[i][0]] =
                axis["bins"];
        }
        else
        {
...
conduit::Node histogram_entropy(const conduit::Node &hist)
{
    ...
    const int num_bins =
        hist.fetch_existing("attrs/num_bins/value").to_int32();
    std::string exec = ExecutionManager::preferred_cpu_policy();
    double sum =
        array_sum(hist.fetch_existing("attrs/value/value"), exec)["value"]
            .to_float64();
    double entropy = 0;
...
conduit::Node field_min(const conduit::Node &dataset, const std::string &field)
{
    ...
    if (domain != -1)
    {
        assoc_str = dataset.child(domain)["fields/" + field + "/association"]
                        .as_string();
    ...
    MinLoc minloc_res;
    MPI_Allreduce(
        &minloc, &minloc_res, 1, MPI_DOUBLE_INT, MPI_MINLOC, mpi_comm);
...
conduit::Node field_sum(const conduit::Node &dataset, const std::string &field)
{
    ...
    MPI_Allreduce(
        &count, &global_count, 1, MPI_LONG_LONG_INT, MPI_SUM, mpi_comm);
...
conduit::Node field_avg(const conduit::Node &dataset, const std::string &field)
{
...
conduit::Node field_max(const conduit::Node &dataset, const std::string &field)
{
    ...
    {
        const std::string assoc_str =
            dataset.child(domain)["fields/" + field + "/association"]
                .as_string();
                    ...
                    conduit::Node &field)
{
    ...
    else
    {
        ...
        else if (coords["type"].as_string() == "rectilinear")
        {
            ...
            if (coords.has_path("values/z"))
            {
                is_3d = true;
                el_dims[1] =
                    coords["values/z"].dtype().number_of_elements() - 1;
            }
        }
        else
        {
    ...
    {
        // we already init'd it to zero if its a new field, so we are good to
        // go if it wasn't' a new field, it should already have the correct
        // values
        return;
    }
    ...
    for (int i = 0; i < windows; ++i)
    {
            ...
            for (index_t z = origin[2]; z < origin[2] + dims[2]; ++z)
            {
                ...
                for (int y = origin[1]; y < origin[1] + dims[1]; ++y)
                {
                    ...
                    for (int x = origin[0]; x < origin[0] + dims[0]; ++x)
                    {
                        ...
                        conduit::index_t value =
                            levels[z_offset + y_offset + x];
                        if (value == 0)
                        {
        ...
        else
        {
            ...
            {
                const conduit::index_t y_offset = y * el_dims[0];
                for (conduit::index_t x = origin[0]; x < origin[0] + dims[0];
                     ++x)
                {
                    // this might a ghost field, but we only want to
                    // mask real zones that are masked by finer grids
                                   ...
                                   const std::string &assoc_str)
{
    ...
    else
    {
        ...
        {
            ASCENT_ERROR("The specified topology '"
                         << topo_name
                         << "' does not have the required fields specified in "
                            "the bin axes: "
                         << bin_axes.to_yaml() << "\n Did you mean to use '"
                         << new_topo_name << "'?");
        }
        ...
        {
            ASCENT_ERROR("The specified association '"
                         << assoc_str
                         << "' conflicts with the association of the fields "
                            "of the bin axes:"
                         << bin_axes.to_yaml() << ". Did you mean to use '"
                         << new_assoc_str << "'?");
        }
    }
    conduit::Node res;
