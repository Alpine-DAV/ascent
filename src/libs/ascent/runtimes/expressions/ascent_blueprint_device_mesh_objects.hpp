...
//---------------------------------------------------------------------------//
//---------------------------------------------------------------------------//
template <typename T> inline bool is_conduit_type(const conduit::Node &values);
...
//---------------------------------------------------------------------------//
//---------------------------------------------------------------------------//
template <typename T> inline T *conduit_ptr(const conduit::Node &values);

//---------------------------------------------------------------------------//
template <>
inline conduit::float64 *
conduit_ptr<conduit::float64>(const conduit::Node &values)
{
...
//---------------------------------------------------------------------------//
template <>
inline conduit::float32 *
conduit_ptr<conduit::float32>(const conduit::Node &values)
{
...
// DeviceAccessor
//---------------------------------------------------------------------------//
template <typename T> struct DeviceAccessor
{
    const T *m_values;
    ...
    DeviceAccessor(const T *values, const conduit::DataType &dtype)
        : m_values(values), m_size(dtype.number_of_elements()),
...
//---------------------------------------------------------------------------//
template <typename T> class MCArray
{
private:
    // src node
...
public:
    ...
    //==---------------------------------------------------------------------==//
    // Main constructor
    MCArray(const conduit::Node &node) : m_src_node(node)
    {
        ...
        if (!types_match)
        {
            ...
            ASCENT_ERROR(
                "MCArray type does not match conduit type: " << schema);
        }
    }
    ...
    T value(const index_t idx, const std::string &component)
    {
        ...
        //{
        //  std::cout<<"[MCarray] Invalid index "<<idx<<" size
        //  "<<m_sizes[comp_idx]<<"\n";
        //}
    ...
    T value(const index_t idx, int component)
    {
        ...
        // elemen_index is extremely missleading. Its actually the byte offset
        // to the element. 2 hours to learn this
        T val;
        index_t el_idx = 0;
        if (path != "")
        {
    ...
    std::string component_path(int component)
    {
        ...
        {
            ASCENT_ERROR("Invalid component " << component
    ...
    const T *device_ptr_const(int component)
    {
        ...
        else
        {
            ...
            // std::cout<<"leaf_path '"<<leaf_path<<"' device _path
                               ...
                               const std::string comp = "")
    {
        ...
        if (comp == "")
        {
            if (m_components != 1)
            {
                ASCENT_ERROR("Ambiguous component: node has more than one "
                             "component but no"
        ...
        const T *ptr = location == "device" ? device_ptr_const(comp_idx)
                                            : host_ptr_const(comp_idx);
        if (leaf_path != "")
        {
...
//---------------------------------------------------------------------------//
ASCENT_EXEC
void mesh_logical_index_2d(int *idx, const int vert_index, const int *dims)
{
...
//---------------------------------------------------------------------------//
ASCENT_EXEC
void mesh_logical_index_3d(int *idx, const int vert_index, const int *dims)
{
    idx[0] = vert_index % dims[0];
                             ...
                             int indices[8])
{
    const int element_dims[3] = {
        point_dims[0] - 1, point_dims[1] - 1, point_dims[2] - 1};
    int element_index[3];
    if (dims == 2)
    {
    ...
    else
    {
        ...
        indices[0] = (element_index[2] * point_dims[1] + element_index[1]) *
                         point_dims[0] +
                     element_index[0];
        indices[1] = indices[0] + 1;
...
struct UniformMesh
{
    ...
    {
        structured_cell_indices(cell_index, m_point_dims, m_dims, indices);
    }
    ...
    {
        int logical_idx[3];
        int pdims[3] = {m_point_dims[0], m_point_dims[1], m_point_dims[2]};
        if (m_dims == 2)
        {
...
// StructuredMesh
//---------------------------------------------------------------------------//
template <typename CoordsType> struct StructuredMesh
{
    ...
    StructuredMesh(const std::string mem_space,
        ...
        : m_coords_x(coords.accessor(mem_space, "x")),
          ...
          m_dims(dims), m_point_dims({
                            {point_dims[0], point_dims[1], point_dims[2]}
    }),
          m_num_indices(m_dims == 2 ? 4 : 8),
          m_num_cells(m_dims == 2
                          ? (m_point_dims[0] - 1) * (m_point_dims[1] - 1)
                          : (m_point_dims[0] - 1) * (m_point_dims[1] - 1) *
                                (m_point_dims[2] - 1)),
          m_num_points(m_dims == 2 ? (m_point_dims[0]) * (m_point_dims[1])
                                   : (m_point_dims[0]) * (m_point_dims[1]) *
                                         (m_point_dims[2]))
    {
    }
...
// RectilinearMesh
//---------------------------------------------------------------------------//
template <typename CoordsType> struct RectilinearMesh
{
...
// UnstructuredMesh
//---------------------------------------------------------------------------//
template <typename CoordsType, typename ConnType> struct UnstructuredMesh
{
    ...
    UnstructuredMesh(const std::string mem_space,
        ...
        : m_coords_x(coords.accessor(mem_space, "x")),
          ...
          m_coords_z(dims == 3 ? coords.accessor(mem_space, "z") :
                               // just use a dummy in this case
                         coords.accessor(mem_space, "x")),
          m_conn(conn.accessor(mem_space)), m_shape_type(shape_type),
          m_dims(dims), m_num_indices(num_indices(shape_type)),
