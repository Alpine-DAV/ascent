...
struct DFAddFunctor
{
                             ...
                             const Exec &) const
    {
        ...
        ascent::forall<for_policy>(
            0, max_size, [=] ASCENT_LAMBDA(index_t i) { sums_ptr[i] = 0.0; });
        ASCENT_DEVICE_ERROR_CHECK();
        ...
        ascent::forall<for_policy>(
            ...
            [=] ASCENT_LAMBDA(index_t i)
            {
                ...
                // sums_ptr[i] = val;
                int old =
                    ascent::atomic_add<atomic_policy>(&(sums_ptr[i]), val);
            });
        ASCENT_DEVICE_ERROR_CHECK();
...
struct DFPowerFunctor
{
                             ...
                             const Exec &) const
    {
        ...
        // init device array
        ascent::forall<for_policy>(
            0, size, [=] ASCENT_LAMBDA(index_t i) { powers_ptr[i] = 0.0; });
        ASCENT_DEVICE_ERROR_CHECK();
        ...
        ascent::forall<for_policy>(
            ...
            [=] ASCENT_LAMBDA(index_t i)
            {
                ...
                // powers_ptr[i] = val;
                int old =
                    ascent::atomic_add<atomic_policy>(&(powers_ptr[i]), val);
                // if(f_accessor[i] < 0)
                //{
                //   std::cerr << "field val: " << f_accessor[i] << "^" <<
                //   exponent << " = " << val << std::endl; std::cerr << "field
                //   val: " << f_accessor[i] << "^" << exponent << " = " <<
                //   powers_ptr[i] << std::endl;
                // }
            });
        ASCENT_DEVICE_ERROR_CHECK();
...
struct InfFunctor
{
    ...
    conduit::Node operator()(const DeviceAccessor<float> accessor,
                             Exec &exec) const
    {
    ...
    template <typename Exec>
    conduit::Node operator()(const DeviceAccessor<double> accessor,
                             Exec &exec) const
    {
        return impl(accessor, exec);
    }
...
struct HistogramFunctor
{
    ...
    HistogramFunctor(const double &min_val,
                     ...
                     const int &num_bins)
        : m_min_val(min_val), m_max_val(max_val), m_num_bins(num_bins)
    {
    }
                             ...
                             const Exec &) const
    {
        ...
        // init device array
        ascent::forall<for_policy>(
            0, num_bins, [=] ASCENT_LAMBDA(index_t i) { bins_ptr[i] = 0.0; });
        ASCENT_DEVICE_ERROR_CHECK();
        ...
        ascent::forall<for_policy>(
            ...
            [=] ASCENT_LAMBDA(index_t i)
            {
                ...
                int old = ascent::atomic_add<atomic_policy>(
                    &(bins_ptr[bin_index]), 1.);
            });
        ASCENT_DEVICE_ERROR_CHECK();
...
struct HistoryGradientRangeFunctor
{
    ...
    conduit::Node operator()(const T *y_values,
                             const T2 *dx_values,
                             const int &size_y_values,
                             const int &size_dx_values) const
    {
        ...
        {
            ASCENT_ERROR(
                "Must either supply a single uniform delta_x value, or "
                "provide at least len(y_values)-1 delta_x values (indicating "
                "the delta_x from each y value to the next).");
        }
    ...
    #pragma omp parallel for
            ...
            {
                gradients[v] =
                    ((y_values[v + 1] - y_values[v]) / (double)*dx_values);
            }
        }
        else
        {
    ...
    #pragma omp parallel for
            ...
            {
                gradients[v] =
                    ((y_values[v + 1] - y_values[v]) / (double)dx_values[v]);
            }
        }
...
conduit::Node history_gradient_range(const conduit::Node &y_values,
                                     const conduit::Node &dx_values)
{
    // TODO THIS NEEDS TO BE PORTED TO RAJA ?
    return exec_dispatch_two_leaves(
        y_values, dx_values, detail::HistoryGradientRangeFunctor());
}

conduit::Node field_reduction_max(const conduit::Node &field,
                                  const std::string &component)
{
    return exec_dispatch_mcarray_component(
        field["values"], component, detail::MaxFunctor());
}

conduit::Node field_reduction_min(const conduit::Node &field,
                                  const std::string &component)
{
    return exec_dispatch_mcarray_component(
        field["values"], component, detail::MinFunctor());
}

conduit::Node field_reduction_sum(const conduit::Node &field,
                                  const std::string &component)
{
    return exec_dispatch_mcarray_component(
        field["values"], component, detail::SumFunctor());
}

conduit::Node field_reduction_nan_count(const conduit::Node &field,
                                        const std::string &component)
{
    return exec_dispatch_mcarray_component(
        field["values"], component, detail::NanFunctor());
}

conduit::Node field_reduction_inf_count(const conduit::Node &field,
                                        const std::string &component)
{
    return exec_dispatch_mcarray_component(
        field["values"], component, detail::InfFunctor());
}
                                        ...
                                        const std::string &component)
{
    ...
    return exec_dispatch_mcarray_component(
        field["values"], component, histogram);
}
...
conduit::Node derived_field_binary_add(const conduit::Node &l_field,
                                       const conduit::Node &r_field,
                                       const std::string &component)
{
    return exec_dispatch_binary_df(
        l_field, r_field, component, detail::DFAddFunctor());
}

conduit::Node derived_field_power(const conduit::Node &field,
                                  const double &exponent,
                                  const std::string &component)
{
    return exec_dispatch_unary_df(
        field, exponent, component, detail::DFPowerFunctor());
}
//-----------------------------------------------------------------------------
}; // namespace expressions
//-----------------------------------------------------------------------------
