...
conduit::Node exec_dispatch_two_leaves(const conduit::Node &values0,
                                       ...
                                       const Function &func)
{
    ...
    {
        ASCENT_ERROR(
            "exec_dispatch_two_leaves: Internal error: expected leaf arrays.");
    }
    const conduit::Node &vals0 =
        num_children0 == 0 ? values0 : values0.child(0);
    const conduit::Node &vals1 =
        num_children1 == 0 ? values1 : values1.child(0);
    ...
    if (vals0.dtype().is_float32())
    {
        ...
        {
            ASCENT_ERROR(
                "exec_dispatch_two_leaves: unsupported array type for array1: "
                << values1.schema().to_string());
        }
    }
    ...
    {
        ASCENT_ERROR(
            "exec_dispatch_two_leavesh: unsupported array type for array0: "
                                              ...
                                              const Exec &exec)
{
    ...
    if (mcarray_is_float32(node))
    {
        ...
        DeviceAccessor<conduit::float32> accessor =
            farray.accessor(mem_space, component);
        res = func(accessor, exec);
    }
    else if (mcarray_is_float64(node))
    {
        MCArray<conduit::float64> farray(node);
        DeviceAccessor<conduit::float64> accessor =
            farray.accessor(mem_space, component);
        res = func(accessor, exec);
    }
    else if (mcarray_is_int32(node))
    {
        ...
        DeviceAccessor<conduit::int32> accessor =
            farray.accessor(mem_space, component);
        res = func(accessor, exec);
    }
    else if (mcarray_is_int64(node))
    {
        ...
        DeviceAccessor<conduit::int64> accessor =
            farray.accessor(mem_space, component);
        res = func(accessor, exec);
    }
    else
                                              ...
                                              const Function &func)
{
    ...
    {
        ASCENT_ERROR(
            "exec_dispatch_mcarray_component: unsupported execution policy "
            << exec_policy);
    }
    return res;
}
...
//-----------------------------------------------------------------------------
template <typename Function, typename Exec>
void exec_dispatch_mesh(const conduit::Node &n_coords,
                        ...
                        const Exec &exec)
{
    ...
    if (mesh_type == "unstructured")
    {
        const int shape_type =
            cell_shape(n_topo["elements/shape"].as_string());
        ...
        if (is_conduit_type<conduit::float32>(n_coords["values/x"]))
        {
            ...
            if (is_conduit_type<conduit::int32>(n_topo[conn_path]))
            {
                ...
                UnstructuredMesh<conduit::float32, conduit::int32> mesh(
                    mem_space, coords, conn, shape_type, dims);
                func(mesh, exec);
            }
            else if (is_conduit_type<conduit::int64>(n_topo[conn_path]))
            {
                MCArray<conduit::int64> conn(n_topo[conn_path]);
                UnstructuredMesh<conduit::float32, conduit::int64> mesh(
                    mem_space, coords, conn, shape_type, dims);
                func(mesh, exec);
            }
            else
            {
                std::cout << "bad topo "
                          << n_topo[conn_path].to_summary_string() << "\n";
                // TODO: log error
            }
        }
        else if (is_conduit_type<conduit::float64>(n_coords["values/x"]))
        {
            ...
            {
                MCArray<conduit::int32> conn(n_topo[conn_path]);
                UnstructuredMesh<conduit::float64, conduit::int32> mesh(
                    mem_space, coords, conn, shape_type, dims);
                func(mesh, exec);
            }
            else if (is_conduit_type<conduit::int64>(n_topo[conn_path]))
            {
                MCArray<conduit::int64> conn(n_topo[conn_path]);
                UnstructuredMesh<conduit::float64, conduit::int64> mesh(
                    mem_space, coords, conn, shape_type, dims);
                func(mesh, exec);
            }
            else
    ...
    else if (mesh_type == "rectilinear")
    {
        ...
        if (is_conduit_type<conduit::float32>(n_coords["values/x"]))
        {
            ...
            RectilinearMesh<conduit::float32> mesh(
                mem_space, x_coords, y_coords, z_coords, dims);
            func(mesh, exec);
        }
        else if (is_conduit_type<conduit::float64>(n_coords["values/x"]))
        {
            ...
            RectilinearMesh<conduit::float64> mesh(
                mem_space, x_coords, y_coords, z_coords, dims);
            func(mesh, exec);
        }
        else
    ...
    else if (mesh_type == "structured")
    {
        ...
        {
            MCArray<conduit::float32> coords(n_coords["values"]);
            StructuredMesh<conduit::float32> mesh(
                mem_space, coords, dims, point_dims);
            func(mesh, exec);
        }
        else if (is_conduit_type<conduit::float64>(n_coords["values/x"]))
        {
    ...
    {
        std::cout << "exec_dispatch_mesh: mesh type not implemented:  "
                  << mesh_type << "\n";
    }
}
...
template <typename Function, typename Exec>
conduit::Node dispatch_memory_binary_df(const conduit::Node &l_field,
                                        ...
                                        const Exec &exec)
{
    ...
    if (field_is_float32(l_field))
    {
        ...
        DeviceAccessor<conduit::float32> l_accessor =
            l_farray.accessor(mem_space, component);
        DeviceAccessor<conduit::float32> r_accessor =
            r_farray.accessor(mem_space, component);
    ...
    else if (field_is_float64(l_field))
    {
        ...
        DeviceAccessor<conduit::float64> l_accessor =
            l_farray.accessor(mem_space, component);
        DeviceAccessor<conduit::float64> r_accessor =
            r_farray.accessor(mem_space, component);
        res = func(l_accessor, r_accessor, exec);
    }
    else if (field_is_int32(l_field))
    {
        ...
        DeviceAccessor<conduit::int32> l_accessor =
            l_farray.accessor(mem_space, component);
        DeviceAccessor<conduit::int32> r_accessor =
            r_farray.accessor(mem_space, component);
        res = func(l_accessor, r_accessor, exec);
    }
    else if (field_is_int64(l_field))
    {
        ...
        DeviceAccessor<conduit::int64> l_accessor =
            l_farray.accessor(mem_space, component);
        DeviceAccessor<conduit::int64> r_accessor =
            r_farray.accessor(mem_space, component);
        res = func(l_accessor, r_accessor, exec);
    }
    else
...
template <typename Function, typename Exec>
conduit::Node dispatch_memory_unary_df(const conduit::Node &field,
                                       const double &val,
                                       std::string component,
                                       const Function &func,
                                       const Exec &exec)
...
template <typename Function>
conduit::Node exec_dispatch_binary_df(const conduit::Node &l_field,
                                      const conduit::Node &r_field,
                                      std::string component,
                                      const Function &func)
{
    ...
    if (exec_policy == "serial")
    {
        SerialExec exec;
        res =
            dispatch_memory_binary_df(l_field, r_field, component, func, exec);
    }
#if defined(ASCENT_RAJA_OPENMP_ENABLED)
    else if (exec_policy == "openmp")
    {
...
template <typename Function>
conduit::Node exec_dispatch_unary_df(const conduit::Node &field,
                                     const double &val,
                                     std::string component,
                                     const Function &func)
{
...
void exec_dispatch_array(Array<T> &array, Function &func)
{
    const std::string exec_policy = ExecutionManager::execution_policy();
...
//-----------------------------------------------------------------------------
template <typename Function> void exec_dispatch_function(Function &func)
{
    const std::string exec_policy = ExecutionManager::execution_policy();
