...
public:
    ArrayInternals()
        : ArrayInternalsBase(), m_device(nullptr),

          m_host(nullptr), m_device_dirty(true), m_host_dirty(true), m_size(0),
          m_own_host(true), m_own_device(true)
    {
    ...
    ArrayInternals(T *data, const size_t size)
        : ArrayInternalsBase(), m_size(size)
    {
#if defined(ASCENT_DEVICE_ENABLED)
        m_device_enabled = true;
#else
    ...
    T get_value(const size_t i)
    {
        ...
        else
        {
            if (!m_host_dirty)
            {
                ...
                if (m_host == nullptr)
                {
                    std::cout << "get_value with null host ptr: this should "
                                 "not happen\n";
                }
                val = m_host[i];
            }
            else
            {
                ...
                cudaMemcpy(
                    &val, &m_device[i], sizeof(T), cudaMemcpyDeviceToHost);
#endif
            }
        }
    ...
    void status()
    {
        ...
        {
            if (m_device_dirty)
                std::cout << "[array] device dirty \n";
            else
                std::cout << "[array] device clean\n";
        }
        if (m_host_dirty)
            std::cout << "[array] host dirty \n";
        else
            std::cout << "[array] host clean\n";
        // bool m_own_host;
        // bool m_own_device;
    }
...
protected:
    ...
    void allocate_host()
    {
        ...
        if (m_host == nullptr)
        {
            m_host = static_cast<T *>(
                ascent::HostMemory::allocate(m_size * sizeof(T)));
    ...
    void allocate_device()
    {
        ...
        if (m_device_enabled)
        {
            if (m_device == nullptr)
            {
                m_device = static_cast<T *>(
                    ascent::DeviceMemory::allocate(m_size * sizeof(T)));
    ...
    void synch_to_host()
    {
        ...
        cudaMemcpy(
            m_host, m_device, m_size * sizeof(T), cudaMemcpyDeviceToHost);
...
#if defined(ASCENT_CUDA_ENABLED)
        cudaMemcpy(
            m_device, m_host, m_size * sizeof(T), cudaMemcpyHostToDevice);
#elif defined(ASCENT_HIP_ENABLED)
