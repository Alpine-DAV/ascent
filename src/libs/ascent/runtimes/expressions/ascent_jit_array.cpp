...
std::vector<std::string> component_names(const int num_components)
{
...
std::string ArrayCode::index(const std::string &idx,
...
std::string ArrayCode::index(const std::string &array_name,
                             ...
                             const int component) const
{
    ...
    if (array_it == array_map.end())
    {
        ...
        // This is not even used for temporary_field because their schema is
        // put into array_map explicitely
    ...
    else
    {
        const int num_components =
            array_it->second.schema.number_of_children();
        if (component == -1)
        {
                             ...
                             const std::string &component) const
{
    ...
    return index(
        array_name, idx, array_it->second.schema.child_index(component));
}
        ...
        if (schema_type == "contiguous")
        {
            for (size_t i = 0; i < component_names.size(); ++i)
            {
                const auto element_bytes =
                    conduit::DataType::default_bytes(type_id);
...
{
    std::vector<std::string> component_names =
        detail::component_names(num_components);
    schemaFactory(
        schema_type, type_id, component_size, component_names, out_schema);
}
