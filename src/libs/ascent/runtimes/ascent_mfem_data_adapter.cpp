...
MFEMDataSet::MFEMDataSet() : m_cycle(0) {}
...
MFEMDataSet::MFEMDataSet(mfem::Mesh *mesh)
    : m_mesh(mesh), m_cycle(0), m_time(0.0)
{
}
...
mfem::GridFunction *MFEMDataSet::get_field(const std::string &field_name)
{
                                        ...
                                        const std::string &topo_name)
{
    ...
    for (int i = 0; i < num_domains; ++i)
    {
        ...
        mesh = mfem::ConduitDataCollection::BlueprintMeshToMesh(
            dom, topo_name, zero_copy);
        dset->set_mesh(mesh);
        ...
        if (dom.has_path("fields"))
        {
            ...
            for (int f = 0; f < num_fields; ++f)
            {
                ...
                if (fnames[f] != nodes_gf_name &&
                    ...
                    !unsupported)
                {
                    mfem::GridFunction *gf = mfem::ConduitDataCollection::
                        BlueprintFieldToGridFunction(mesh, field, zero_copy);
...
bool MFEMDataAdapter::IsHighOrder(const conduit::Node &n)
{
...
// +------------+--------------------+------------------+
// | SC         | H1_FESpace         | 1                | scalar continous
// (nodes)
// +------------+--------------------+------------------+
// | SD         | L1                 | 1                | scalar discontinous
// (zonal sort of)
// +------------+--------------------+------------------+
...
// | ND         | NDColl             | 1                |
// +------------+--------------------+------------------+
void MFEMDataAdapter::Linearize(MFEMDomains *ho_domains,
                                conduit::Node &output,
                                const int refinement)
{
    ...
    for (int i = 0; i < n_doms; ++i)
    {
        ...
        const mfem::FiniteElementSpace *ho_fes_space =
            ho_mesh->GetNodalFESpace();
        const mfem::FiniteElementCollection *ho_fes_col =
            ho_fes_space->FEColl();
        // refine the mesh and convert to blueprint
        mfem::Mesh lo_mesh = mfem::Mesh::MakeRefined(
            *ho_mesh, refinement, mfem::BasisType::GaussLobatto);

        MeshToBlueprintMesh(&lo_mesh, n_dset);

        int conn_size = n_dset["topologies/main/elements/connectivity"]
                            .dtype()
                            .number_of_elements();
        ...
        for (auto it = field_map.begin(); it != field_map.end(); ++it)
        {
            ...
            {
                ASCENT_ERROR(
                    "Linearize: high order gf finite element space is null")
            }
            ...
            mfem::FiniteElementSpace *lo_fes = new mfem::FiniteElementSpace(
                &lo_mesh, lo_col, ho_fes->GetVDim());
        ...
        {
            info.print();
            ASCENT_ERROR("Linearize: failed to build a blueprint conforming "
                         "data set from mfem")
        }
    }
    // output.schema().print();
...
void MFEMDataAdapter::GridFunctionToBlueprintField(
    ...
    const std::string &main_topology_name)
{
    ...
    if (vdim == 1) // scalar case
    {
        ...
        //                                ndofs);
        n_field["values"].set(values, ndofs);
    }
    else // vector case
    {
        ...
        for (int d = 0; d < vdim; d++)
        {
            ...
            //                                           offset,
            //                                           stride);
            n_field["values"][comp_name].set(values, ndofs, offset, stride);
            offset += sizeof(double) * vdim_stride;
        }
    }
}

void MFEMDataAdapter::MeshToBlueprintMesh(
    mfem::Mesh *mesh,
    Node &n_mesh,
    ...
    const std::string &boundary_topology_name)
{
    ...
    n_mesh_coords["values/x"].set(coords_ptr, num_vertices, 0, stride);

    if (dim >= 2)
    {
        n_mesh_coords["values/y"].set(
            coords_ptr, num_vertices, sizeof(double), stride);
    }
    if (dim >= 3)
    {
        n_mesh_coords["values/z"].set(
            coords_ptr, num_vertices, sizeof(double) * 2, stride);
    }
    ...
    mfem::Element::Type ele_type =
        static_cast<mfem::Element::Type>(mesh->GetElement(0)->GetType());
    ...
    if (gf_mesh_nodes != NULL)
    {
        GridFunctionToBlueprintField(
            gf_mesh_nodes, n_mesh["fields/mesh_nodes"], main_topology_name);
    ...
    if (mesh->GetNBE() > 0)
    {
        ...
        n_bndry_topo["elements/connectivity"].set(
            DataType::c_int(num_bndry_conn_idxs));
        ...
        for (int i = 0; i < num_bndry_ele; i++)
        {
            ...
            memcpy(bndry_conn_ptr,
                   bndry_ele_verts,
                   bndry_idxs_per_ele * sizeof(int));
