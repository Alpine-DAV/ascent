...
class VTKH_API MeshConnectivityContainer : viskores::cont::ExecutionObjectBase
{
public:
  MeshConnectivityContainer();
  virtual ~MeshConnectivityContainer();

  virtual viskores::rendering::raytracing::MeshConnectivity PrepareForExecution(viskores::cont::DeviceAdapterId deviceId,
                                               viskores::cont::Token& token) const = 0;

  void FindEntry(viskores::rendering::raytracing::Ray<viskores::Float32>& rays);

  void FindEntry(viskores::rendering::raytracing::Ray<viskores::Float64>& rays);

protected:
  using Id4Handle = typename viskores::cont::ArrayHandle<viskores::Id4>;
  // Mesh Boundary
  Id4Handle Triangles;
  viskores::rendering::raytracing::TriangleIntersector Intersector;

private:
  template <typename T>
  VISKORES_CONT void FindEntryImpl(viskores::rendering::raytracing::Ray<T>& rays);
};
...
public:
  ...
  viskores::rendering::raytracing::MeshConnectivity PrepareForExecution(viskores::cont::DeviceAdapterId deviceId,
