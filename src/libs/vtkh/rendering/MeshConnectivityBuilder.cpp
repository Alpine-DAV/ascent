...
public:
  ...
  VISKORES_EXEC inline void operator()(const viskores::Id& index, TrianglePortalType& triangles) const
  {
    ...
    viskores::Int32 shapesFaceOffset =
      tables.FaceLookUp(tables.CellTypeLookUp(viskores::CELL_SHAPE_HEXAHEDRON), 0);
                                         ...
                                         viskores::cont::ArrayHandle<viskores::Int32>& uniqueFaces)
{
  ...
  viskores::worklet::DispatcherMapTopology<viskores::rendering::raytracing::MortonCodeFace>(viskores::rendering::raytracing::MortonCodeFace(inverseExtent, minPoint))
  ...
  viskores::rendering::raytracing::Logger::GetInstance()->AddLogData("gen_face_conn", time);
}

template <typename ShapeHandleType, typename OffsetsHandleType, typename ConnHandleType>
VISKORES_CONT viskores::cont::ArrayHandle<viskores::Vec<viskores::Id, 4>> ExtractFaces(
  ...
  const OffsetsHandleType& shapeOffsets)
{
  ...
  viskores::Float64 time = timer.GetElapsedTime();
  viskores::rendering::raytracing::Logger::GetInstance()->AddLogData("external_faces", time);
  ...
  viskores::cont::CellSetStructured<3>& cellSetStructured)
{
  ...
  viskores::Float64 time = timer.GetElapsedTime();
  viskores::rendering::raytracing::Logger::GetInstance()->AddLogData("structured_external_faces", time);
  ...
  const viskores::cont::CoordinateSystem& coordinates)
{
  ...
  else if (cellset.CanConvert<viskores::cont::CellSetSingleType<>>())
  {
    ...
    if (shapeType == viskores::CELL_SHAPE_HEXAHEDRON)
      type = UnstructuredSingle;
    if (shapeType == viskores::CELL_SHAPE_TETRA)
      type = UnstructuredSingle;
    if (shapeType == viskores::CELL_SHAPE_WEDGE)
      type = UnstructuredSingle;
    if (shapeType == viskores::CELL_SHAPE_PYRAMID)
      type = UnstructuredSingle;
  }
