...
TEST(ascent_triggers, simple_rick)
{
    ...
    string trigger_file = conduit::utils::join_file_path(output_path,"simple_trigger_actions.yaml");
...
TEST(ascent_triggers, complex_trigger)
{
    ...
    string output_path = prepare_output_dir();
    string trigger_file = conduit::utils::join_file_path(output_path,"complex_trigger_actions.yaml");
...
TEST(ascent_triggers, trigger_extract)
{
    ...
    string output_path = prepare_output_dir();
    string trigger_file = conduit::utils::join_file_path(output_path,"trigger_extract_actions.yaml");
    ...
    std::string msg = "A more complex trigger example using several functions"
                      " that evaluate positions on the mesh.";
    ASCENT_ACTIONS_DUMP(actions,output_file,msg);
}
...
TEST(ascent_triggers, trigger_multi_actions_files)
{
    ...
    string output_path = prepare_output_dir();
    string trigger_file_a = conduit::utils::join_file_path(output_path,"trigger_multi_action_a.yaml");
    string trigger_file_b = conduit::utils::join_file_path(output_path,"trigger_multi_action_b.yaml");
    ...
    {
      conduit::utils::remove_file(trigger_file_a);
    }

    if(conduit::utils::is_file(trigger_file_b))
    {
      conduit::utils::remove_file(trigger_file_b);
    }

    if(conduit::utils::is_file(output_root_file_a))
    {
      conduit::utils::remove_file(output_root_file_a);
    }

    //
    // Create multiple trigger actions files.
    //
    Node trigger_actions;
    conduit::Node &add_ext = trigger_actions.append();
    add_ext["action"] = "add_extracts";
    conduit::Node & extracts = add_ext["extracts"];
    extracts["e1/type"]  = "relay";
    extracts["e1/params/path"] = output_file_a;
    extracts["e1/params/protocol"] = "blueprint/mesh/hdf5";
    trigger_actions.save(trigger_file_a);
    trigger_actions.reset();
    ...
    scenes["s1/plots/p1/type"]  = "pseudocolor";
    scenes["s1/plots/p1/field"] = "radial";
    scenes["s1/image_prefix"]   = output_file_b;
    trigger_actions.save(trigger_file_b);
    ...
    triggers["t1/params/condition"] = condition;
    triggers["t1/params/actions_files"].append() = trigger_file_a;
    triggers["t1/params/actions_files"].append() = trigger_file_b;
    actions.print();
    ...
    // check that we created an image + bp hdf5 file from the trigger

    EXPECT_TRUE(conduit::utils::is_file(output_root_file_a));
    EXPECT_TRUE(check_test_image(output_file_b));
    std::string msg = "A trigger that executes actions from multiple actions files.";
    ASCENT_ACTIONS_DUMP(actions,output_file_b,msg);
}
